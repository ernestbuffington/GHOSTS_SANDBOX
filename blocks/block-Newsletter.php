...
if (is_user()) {
    ...
    if ($newsletter) {
        ...
        if (isset($_POST['nb_unsubscribe'])) {
            $nuke_db->sql_query("UPDATE ".$nuke_user_prefix."_users SET newsletter='0' WHERE user_id='$user_id'");
        ...
        if (isset($_POST['nb_subscribe'])) {
            $nuke_db->sql_query("UPDATE ".$nuke_user_prefix."_users SET newsletter='1' WHERE user_id='$user_id'");
            nuke_redirect($_SERVER['REQUEST_URI']);
        }
    }
} else {

...
$result = $nuke_db->sql_query("SELECT `ip_addr`, `reason` FROM `".$prefix."_nsnst_blocked_ips` ORDER BY `date` DESC LIMIT 30");
while (list($ip_addr, $ip_reason) = $nuke_db->sql_fetchrow($result)) {
  ...
  if((is_admin() AND $ab_config['display_reason']==1) OR ((is_user() OR is_admin()) AND $ab_config['display_reason']==2) OR $ab_config['display_reason']==3) {
    $result2 = $nuke_db->sql_query("SELECT `reason` FROM `".$prefix."_nsnst_blockers` WHERE `blocker`='$ip_reason'");

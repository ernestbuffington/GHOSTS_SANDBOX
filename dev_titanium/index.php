	...
	if (check_link_hash($request->variable('hash', ''), 'mark_notification_read'))
	{
		...
		if (isset($notification['notifications'][$mark_notification]))
		{
			...
			*/
			...
			{
				redirect(append_sid($phpbb3_root_path . $redirect));
			}
...
while ($row = $db->sql_fetchrow($result))
{
	...
	{
		$legend[] = '<a' . $colour_text . ' href="' . append_sid("{$phpbb3_root_path}memberlist.$phpEx", 'mode=group&amp;g=' . $row['group_id']) . '">' . $group_name . '</a>';
	}
}
...
$template->assign_vars(array(
	...
	'S_LOGIN_ACTION'			=> append_sid("{$phpbb3_root_path}ucp.$phpEx", 'mode=login'),
	...
	'U_MARK_FORUMS'		=> ($user->data['is_registered'] || $config['load_anon_lastread']) ? append_sid("{$phpbb3_root_path}index.$phpEx", 'hash=' . generate_link_hash('global') . '&amp;mark=forums&amp;mark_time=' . time()) : '',
	'U_MCP'				=> ($auth->acl_get('m_') || $auth->acl_getf_global('m_')) ? append_sid("{$phpbb3_root_path}mcp.$phpEx", 'i=main&amp;mode=front', true, $user->session_id) : '')
);

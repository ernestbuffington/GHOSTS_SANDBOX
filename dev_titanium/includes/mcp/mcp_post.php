...
{
	global $phpEx, $phpbb3_root_path, $config, $request;
	...
	$url = append_sid("{$phpbb3_root_path}mcp.$phpEx?" . phpbb_extra_url());
		...
		case 'whois':
			...
			if ($auth->acl_get('m_info', $post_info['forum_id']))
			{
				...
				$template->assign_vars(array(
					'RETURN_POST'	=> sprintf($user->lang['RETURN_POST'], '<a href="' . append_sid("{$phpbb3_root_path}mcp.$phpEx", "i=$id&amp;mode=$mode&amp;p=$post_id") . '">', '</a>'),
					'U_RETURN_POST'	=> append_sid("{$phpbb3_root_path}mcp.$phpEx", "i=$id&amp;mode=$mode&amp;p=$post_id"),
	...
	$mcp_post_template_data = array(
		...
		'U_FIND_USERNAME'		=> append_sid("{$phpbb3_root_path}memberlist.$phpEx", 'mode=searchuser&amp;form=mcp_chgposter&amp;field=username&amp;select_single=true'),
		...
		'RETURN_TOPIC'			=> sprintf($user->lang['RETURN_TOPIC'], '<a href="' . append_sid("{$phpbb3_root_path}viewtopic.$phpEx", "p=$post_id") . "#p$post_id\">", '</a>'),
		'RETURN_FORUM'			=> sprintf($user->lang['RETURN_FORUM'], '<a href="' . append_sid("{$phpbb3_root_path}viewforum.$phpEx", "f={$post_info['forum_id']}&amp;start={$start}") . '">', '</a>'),
		...
		'U_WHOIS'				=> ($auth->acl_get('m_info', $post_info['forum_id'])) ? append_sid("{$phpbb3_root_path}mcp.$phpEx", "i=$id&amp;mode=$mode&amp;action=whois&amp;p=$post_id&amp;ip={$post_info['poster_ip']}") : '',
	);
	...
	if ($auth->acl_get('m_info', $post_info['forum_id']))
	{
		...
		if (count($users_ary))
		{
			...
			foreach ($users_ary as $user_id => $user_row)
			{
				$template->assign_block_vars('userrow', array(
					...
					'U_SEARCHPOSTS' => append_sid("{$phpbb3_root_path}search.$phpEx", 'author_id=' . $user_id . '&amp;sr=topics'))
				);
			}
		}
		...
		while ($row = $db->sql_fetchrow($result))
		{
			...
			$template->assign_block_vars('iprow', array(
				...
				'U_WHOIS'		=> append_sid("{$phpbb3_root_path}mcp.$phpEx", "i=$id&amp;mode=$mode&amp;action=whois&amp;p=$post_id&amp;ip={$row['poster_ip']}"))
			);
		}
		$db->sql_freeresult($result);
...
{
	global $auth, $db, $config, $phpbb3_root_path, $phpEx, $user, $phpbb_log, $phpbb_dispatcher;

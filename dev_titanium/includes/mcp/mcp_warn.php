...
class mcp_warn
{
	...
	*/
	function mcp_warn_front_view()
	{
		...
		$template->assign_vars(array(
			...
			'U_POST_ACTION'		=> append_sid("{$phpbb3_root_path}mcp.$phpEx", 'i=warn&amp;mode=warn_user'),
		));
		...
		foreach ($highest as $row)
		{
			$template->assign_block_vars('highest', array(
				'U_NOTES'		=> append_sid("{$phpbb3_root_path}mcp.$phpEx", 'i=notes&amp;mode=user_notes&amp;u=' . $row['user_id']),
	...
	*/
	...
	{
		global $phpEx, $phpbb3_root_path, $config, $phpbb_container;
		...
		$base_url = append_sid("{$phpbb3_root_path}mcp.$phpEx", "i=warn&amp;mode=list&amp;st=$st&amp;sk=$sk&amp;sd=$sd");
	...
	*/
	function mcp_warn_post_view($action)
	{
		...
		if ($warning && $action == 'add_warning')
		{
			...
			if (!empty($message))
			{
				$redirect = append_sid("{$phpbb3_root_path}mcp.$phpEx", "i=notes&amp;mode=user_notes&amp;u=$user_id");
...
{
	global $phpEx, $phpbb3_root_path, $config, $phpbb_log;
	global $db, $user;
	...
	if ($send_pm)
	{
		...
		if ($user_row['user_lang'] != $user->lang_name)
		{
			...
			$user_row['user_lang'] = (file_exists($phpbb3_root_path . 'language/' . basename($user_row['user_lang']) . "/mcp." . $phpEx)) ? $user_row['user_lang'] : $config['default_lang'];
			include($phpbb3_root_path . 'language/' . basename($user_row['user_lang']) . "/mcp." . $phpEx);

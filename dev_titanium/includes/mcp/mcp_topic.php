...
function mcp_topic_view($id, $mode, $action)
{
	...
	if ($action == 'resync')
	{
		...
		{
			include($phpbb3_root_path . 'includes/mcp/mcp_forum.' . $phpEx);
		}
	...
	if (($action == 'restore' || $action == 'approve') && $auth->acl_get('m_approve', $topic_info['forum_id']))
	{
		...
		{
			include($phpbb3_root_path . 'includes/mcp/mcp_queue.' . $phpEx);
		}
	...
	*/
	...
	foreach ($rowset as $current_row_number => $row)
	{
		...
		$post_row = array(
			...
			'MESSAGE'		=> $message,
			'POST_ID'		=> $row['post_id'],
			'RETURN_TOPIC'	=> sprintf($user->lang['RETURN_TOPIC'], '<a href="' . append_sid("{$phpbb3_root_path}viewtopic.$phpEx", 't=' . $topic_id) . '">', '</a>'),
			...
			'U_MCP_APPROVE'		=> ($auth->acl_get('m_approve', $topic_info['forum_id'])) ? append_sid("{$phpbb3_root_path}mcp.$phpEx", 'i=queue&amp;mode=approve_details&amp;p=' . $row['post_id']) : '',
			'U_MCP_REPORT'		=> ($auth->acl_get('m_report', $topic_info['forum_id'])) ? append_sid("{$phpbb3_root_path}mcp.$phpEx", 'i=reports&amp;mode=report_details&amp;p=' . $row['post_id']) : '',
		);
	...
	$base_url = append_sid("{$phpbb3_root_path}mcp.$phpEx", "i=$id&amp;t={$topic_info['topic_id']}&amp;mode=$mode&amp;action=$action&amp;to_topic_id=$to_topic_id&amp;posts_per_page=$posts_per_page&amp;st=$sort_days&amp;sk=$sort_key&amp;sd=$sort_dir");
	if ($posts_per_page)
	{
	...
	$topic_row = [
		...
		'U_VIEW_TOPIC'		=> append_sid("{$phpbb3_root_path}viewtopic.$phpEx", 't=' . $topic_info['topic_id']),

		'TO_TOPIC_ID'		=> $to_topic_id,
		'TO_TOPIC_INFO'		=> ($to_topic_id) ? sprintf($user->lang['YOU_SELECTED_TOPIC'], $to_topic_id, '<a href="' . append_sid("{$phpbb3_root_path}viewtopic.$phpEx", 't=' . $to_topic_id) . '">' . $to_topic_info['topic_title'] . '</a>') : '',
		...
		'RETURN_TOPIC'		=> sprintf($user->lang['RETURN_TOPIC'], '<a href="' . append_sid("{$phpbb3_root_path}viewtopic.$phpEx", "t={$topic_info['topic_id']}&amp;start=$start") . '">', '</a>'),
		'RETURN_FORUM'		=> sprintf($user->lang['RETURN_FORUM'], '<a href="' . append_sid("{$phpbb3_root_path}viewforum.$phpEx", "f={$topic_info['forum_id']}&amp;start=$start") . '">', '</a>'),
...
{
	global $db, $template, $user, $phpEx, $phpbb3_root_path, $auth, $config, $phpbb_log, $request, $phpbb_dispatcher;
...
//					continue;
		...
		$return_link = sprintf($user->lang['RETURN_TOPIC'], '<a href="' . append_sid("{$phpbb3_root_path}viewtopic.$phpEx", 't=' . $post_info['topic_id']) . '">', '</a>') . '<br /><br />' . sprintf($user->lang['RETURN_NEW_TOPIC'], '<a href="' . append_sid("{$phpbb3_root_path}viewtopic.$phpEx", 't=' . $to_topic_id) . '">', '</a>');
...
function merge_posts($topic_id, $to_topic_id)
{
	...
	if (confirm_box(true))
	{
		...
		if ($row)
		{
			$return_link .= sprintf($user->lang['RETURN_TOPIC'], '<a href="' . append_sid("{$phpbb3_root_path}viewtopic.$phpEx", 't=' . $topic_id) . '">', '</a>');
		}
		else
		{

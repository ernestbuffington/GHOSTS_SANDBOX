...
class ucp_main
{
	...
	{
		global $config, $db, $user, $auth, $template, $phpbb3_root_path, $phpEx, $phpbb_dispatcher;
		global $request;
			...
			case 'front':
				...
				foreach ($topic_list as $topic_id)
				{
					...
					$topicrow = array(
						...
						'U_LAST_POST'			=> append_sid("{$phpbb3_root_path}viewtopic.$phpEx", "p=" . $row['topic_last_post_id']) . '#p' . $row['topic_last_post_id'],
						'U_LAST_POST_AUTHOR'	=> get_username_string('profile', $row['topic_last_poster_id'], $row['topic_last_poster_name'], $row['topic_last_poster_colour']),
						'U_NEWEST_POST'			=> append_sid("{$phpbb3_root_path}viewtopic.$phpEx", "t=$topic_id&amp;view=unread") . '#unread',
						'U_VIEW_TOPIC'			=> append_sid("{$phpbb3_root_path}viewtopic.$phpEx", "t=$topic_id"),
					);
...
//					'S_GROUP_OPTIONS'	=> $group_options,
					...
					'U_SEARCH_USER'		=> ($auth->acl_get('u_search')) ? append_sid("{$phpbb3_root_path}search.$phpEx", 'author_id=' . $user->data['user_id'] . '&amp;sr=posts') : '',
				));
			...
			case 'subscribed':
				...
				if ($unwatch)
				{
					...
					$message = $msg . '<br /><br />' . sprintf($user->lang['RETURN_UCP'], '<a href="' . append_sid("{$phpbb3_root_path}ucp.$phpEx", "i=$id&amp;mode=subscribed") . '">', '</a>');
					meta_refresh(3, append_sid("{$phpbb3_root_path}ucp.$phpEx", "i=$id&amp;mode=subscribed"));
			...
			case 'drafts':
				...
				include_once($phpbb3_root_path . 'includes/message_parser.' . $phpEx);
				...
				foreach ($draftrows as $draft)
				{
					...
					{
						$link_pm = true;
						$insert_url = append_sid("{$phpbb3_root_path}ucp.$phpEx", "i=$id&amp;mode=compose&amp;d=" . $draft['draft_id']);
					}
	...
	*/
	...
	{
		global $user, $db, $template, $config, $cache, $auth, $phpbb3_root_path, $phpEx, $phpbb_container, $request, $phpbb_dispatcher;
		...
		foreach ($topic_list as $topic_id)
		{
			...
			$view_topic_url = append_sid("{$phpbb3_root_path}viewtopic.$phpEx", $view_topic_url_params);
			...
			$template_vars = array(
				...
				'U_NEWEST_POST'			=> append_sid("{$phpbb3_root_path}viewtopic.$phpEx", $view_topic_url_params . '&amp;view=unread') . '#unread',
				'U_LAST_POST'			=> append_sid("{$phpbb3_root_path}viewtopic.$phpEx", $view_topic_url_params . '&amp;p=' . $row['topic_last_post_id']) . '#p' . $row['topic_last_post_id'],
			...
			$pagination->generate_template_pagination(append_sid("{$phpbb3_root_path}viewtopic.$phpEx", "t=$topic_id"), 'topicrow.pagination', 'start', $replies + 1, $config['posts_per_page'], 1, true, true);
		}
	}
}

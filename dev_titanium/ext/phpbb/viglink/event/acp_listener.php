...
class acp_listener implements EventSubscriberInterface
{
	...
	public function set_viglink_services()
	{
		...
		if (empty($this->config['viglink_ask_admin']) && $this->user->data['user_type'] == USER_FOUNDER && (time() - intval($this->config['viglink_ask_admin_last']) > 86400))
		{
			...
			redirect(append_sid($this->phpbb_root_path . 'admin/adm/index.' . $this->php_ext, 'i=acp_help_phpbb&mode=help_phpbb'));
		}
	}
	...
	public function update_viglink_settings($event)
	{
		...
		$this->template->assign_vars(array(
			...
			'ACP_VIGLINK_SETTINGS_CHANGE'	=> $this->language->lang('ACP_VIGLINK_SETTINGS_CHANGE', append_sid($this->phpbb_root_path . 'admin/adm/index.' . $this->php_ext, 'i=-phpbb-viglink-acp-viglink_module&mode=settings')),
		));
	}
}

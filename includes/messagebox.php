...
$result = $nuke_db->sql_query("SELECT `mid`, `title`, `content`, `date`, `expire`, `view`, `groups` FROM `".$prefix."_message` WHERE `active` = 1 ".$query." ORDER BY `date` DESC", true);

$query = '';

while (list($mid, $title, $content, $date, $expire, $view, $groups) = $nuke_db->sql_fetchrow($result)) 
{

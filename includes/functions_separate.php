...
function get_dividers($topics)
{
    ...
    $total_by_type = array (NUKE_POST_GLOBAL_ANNOUNCE => 0, NUKE_POST_ANNOUNCE => 0, NUKE_POST_STICKY => 0, NUKE_POST_NORMAL => 0);
    ...
    if ( ( $total_by_type[NUKE_POST_GLOBAL_ANNOUNCE] + $total_by_type[NUKE_POST_ANNOUNCE] + $total_by_type[NUKE_POST_STICKY] ) != 0 )
    {
        $count_topics = 0;
        
        $dividers[$count_topics] = $lang['Global_Announcements'];
        $count_topics += $total_by_type[NUKE_POST_GLOBAL_ANNOUNCE];

        $dividers[$count_topics] = $lang['Announcements'];
        $count_topics += $total_by_type[NUKE_POST_ANNOUNCE];

        $dividers[$count_topics] = $lang['Sticky_Topics'];
        $count_topics += $total_by_type[NUKE_POST_STICKY];

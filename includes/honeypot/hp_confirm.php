...
$result = $nuke_db->sql_query("SELECT usehp, check1, check2, check3, check4, check5, check6, c7opt1, c7opt2, c7amount, c8opt1, c8opt2, usebsapi, c8apikey, fs9opt1, fs9opt2, fs9apikey, check3time, check4question, check4answer, usefeedback, email FROM ".$prefix."_honeypot_config");
list($usehp, $check1, $check2, $check3, $check4, $check5, $check6, $c7opt1, $c7opt2, $c7amount, $c8opt1, $c8opt2, $usebsapi, $c8apikey, $fs9opt1, $fs9opt2, $fs9apikey, $check3time, $check4question, $check4answer, $usefeedback, $email) = $nuke_db->sql_fetchrow($result);
...
/************************************************************************/
$result1 = $nuke_db->sql_query("SELECT COUNT(ip) FROM `". $prefix ."_honeypot` WHERE ip = '$ip'");
list($resip) = $nuke_db->sql_fetchrow($result1);
$nuke_db->sql_freeresult($result1);

$result2 = $nuke_db->sql_query("SELECT COUNT(email) FROM `". $prefix ."_honeypot` WHERE email ='$ya_user_email'");
list($resemail) = $nuke_db->sql_fetchrow($result2);
...
$nuke_db->sql_query("INSERT INTO `" . $prefix . "_honeypot` VALUES (NULL, '$ya_username', '$ya_realname', '$ya_user_email', '$ip', '$date', '$potnum', '$reason')");
OpenTable();

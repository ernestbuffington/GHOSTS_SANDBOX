...
global $prefix, $nuke_db, $currentlang;
if (file_exists(NUKE_ADMIN_DIR.'language/Honeypot/lang-'.$currentlang.'.php')) {
	...
	$result1 = $nuke_db->sql_query("SELECT usehp, check3, check3time FROM ".$prefix."_honeypot_config");
	list($usehp, $check3, $check3time) = $nuke_db->sql_fetchrow($result1);

...
function spsave_config($config_name, $config_value){
  ...
  $nuke_db->sql_query("UPDATE `".$prefix."_nsnsp_config` SET `config_value`='$config_value' WHERE `config_name`='$config_name'");
/*****[BEGIN]******************************************
...
/*****[END]********************************************
 [ Base:    Caching System                     v3.0.0 ]
 ******************************************************/
      $configresult = $nuke_db->sql_query("SELECT `config_name`, `config_value` FROM `".$prefix."_nsnsp_config`");

 ...
 *
 *   Id: nuke_page_header.php,v 1.106.2.24 2005/03/26 14:15:59 acydburn Exp
 ***************************************************************************/
/***************************************************************************
...
|| ($nuke_userdata['user_id'] == NUKE_ANONYMOUS && $board_config['default_time_mode'] >= 4)):
    ...
    if(!isset($pc_dateTime['pc_timezoneOffset']) && !isset($HTTP_GET_VARS['pc_tzo'])):
        $template_nuke->assign_block_vars('switch_send_pc_dateTime', array());
		if($nuke_userdata['user_pc_timeOffsets'] != '0'):
        $template_nuke->assign_block_vars('switch_valid_time', array());
		endif;
	else:
        $template_nuke->assign_block_vars('switch_valid_time', array());
    endif;
...
if($board_config['quick_search_enable'] == 1):
    $template_nuke->assign_block_vars('switch_quick_search', array(
...
if($nuke_userdata['user_level'] == NUKE_ADMIN): 
  ...
  $template_nuke->assign_block_vars('boarddisabled', array());
endif;
# Mod: Disable Board Admin Override v0.1.1 END

# Login box?
if(!$nuke_userdata['session_logged_in']):
    $template_nuke->assign_block_vars('switch_user_logged_out', array());
    # Allow autologin?
    if(!isset($board_config['allow_autologin']) || $board_config['allow_autologin']):
        $template_nuke->assign_block_vars('switch_allow_autologin', array());
        $template_nuke->assign_block_vars('switch_user_logged_out.switch_allow_autologin', array());
    endif;
else:
    $template_nuke->assign_block_vars('switch_user_logged_in', array());
    if(!empty($nuke_userdata['user_popup_pm']))
    $template_nuke->assign_block_vars('switch_enable_pm_popup', array());
endif;
...
&& $nuke_userdata['user_birthday'] >= gmdate('md0000',$starttime) && $nuke_userdata['user_birthday'] <= gmdate('md9999',$current_time)):
   ...
   switch($nuke_userdata['birthday_greeting']):
	  ...
	  $template_nuke->assign_block_vars('switch_birthday_popup',array());
	endswitch;
endif;
# Mod: Birthdays v3.0.0 END
...
$template_nuke->pparse('overall_header');

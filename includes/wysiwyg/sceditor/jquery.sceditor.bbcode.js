		...
		html:		'<!DOCTYPE html>' +
				'<html>' +
					'<head>' +
						...
						// '	{' +
						// '		if(img.width > reimg_maxWidth) {' +
						// // '			console.log(reimg_maxWidth);' +
						// '			img.style.backgroundColor = "white";' +
						// '			img.style.border = "2px solid rgb(0, 102, 153)";' +
						// '			img.style.padding = "2px";' +
						// // '			img.style.height = auto;' +
						// '			img.style.width = "'+reimg_maxWidth+'px";' +
						// '		}' +

						// '		else if(img.height > reimg_maxHeight) {' +
						// // '			console.log(reimg_maxHeight);' +
						// '			img.style.backgroundColor = "white";' +
						// '			img.style.border = "2px solid rgb(0, 102, 153)";' +
						// '			img.style.padding = "2px";' +
						// '			img.style.height = "'+reimg_maxHeight+'px";' +
	...
	$.sceditor.commands = {
		...
		size: {
			_dropDown: function(editor, caller, callback) {
				...
				// for (var i=1; i<= 7; i++)
					// content.append(_tmpl('sizeOpt', {size: "8"}, true).click(clickFunc));
					// content.append(_tmpl('sizeOpt', {size: "10"}, true).click(clickFunc));
					// content.append(_tmpl('sizeOpt', {size: "12"}, true).click(clickFunc));
					// content.append(_tmpl('sizeOpt', {size: "18"}, true).click(clickFunc));
					// content.append(_tmpl('sizeOpt', {size: "24"}, true).click(clickFunc));
				for(i=8; i < 26; i+=2) 
				{
		...
		code: {
			...
			exec: function () {
				...
				this.wysiwygEditorInsertHtml('<code>', '</code>');
			},
			tooltip: 'Code'
		},
		// END_COMMAND
		...
		image: {
			exec: function (caller) 
			{
				...
				content.find('.button').click(function (e) 
				{
					...
					if(val && val !== 'http://')
					{
						$('<img src="'+val+'"/>').load(function()
						{
    						var style = '';
    						if(this.width > reimg_maxWidth) 
    						{
    							var style = ' style="background-color: white; border: 2px solid rgb(0, 102, 153); height: auto; width: '+reimg_maxWidth+'px; padding: 2px;"';
    						}
    						else if(img.height > reimg_maxHeight) 
    						{
    							var style = ' style="background-color: white; border: 2px solid rgb(0, 102, 153); height: '+reimg_maxHeight+'px; width: auto; padding: 2px;"';
    						}
    						editor.wysiwygEditorInsertHtml('<img'+style+' src="' + val + '" />');
						});
					}
					editor.closeDropDown(true);
					e.preventDefault();
	...
	$.sceditor.plugins.bbcode.bbcodes = {
		...
		img: {
			...
			format: function($element, content) {
				...
				return '[img]' + $element.attr('src') + '[/img]';
			},
			html: function(token, attrs, content) {
				var	undef, w, h, parts,
					attribs = '';
				...
				return '<img class="reimg" onload="reimg(this);" onerror="reimg(this);" src="' + escapeUriScheme(content) + '" />';
			}
		},
		// END_COMMAND

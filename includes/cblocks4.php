...
$cbinfo = $nuke_db->sql_fetchrow($nuke_db->sql_query("select * from `".$prefix."_nsncb_config` where `cgid`='4'"));
if($cbinfo['enabled'] == '1') {
    ...
    $result3 = $nuke_db->sql_query("SELECT * FROM `".$prefix."_nsncb_blocks` WHERE `cgid`='4' ORDER BY `cbid`");

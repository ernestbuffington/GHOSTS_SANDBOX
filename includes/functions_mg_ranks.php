...
function generate_ranks($nuke_user_row, $ranks_sql)
{
	$nuke_user_fields_array = array(
		'user_rank',
		'user_rank2',
		'user_rank3',
		'user_rank4',
	...
	$nuke_user_ranks = array();
	
	$is_banned = false;
	$is_guest = false;
	$rank_sw = false;

	for($j = 0; $j < count($nuke_user_ranks_array); $j++):
		$nuke_user_ranks[$nuke_user_ranks_array[$j]] = '';
	endfor;

	if($nuke_user_row['user_id'] == '-1')
	$is_guest = true;

	if($is_guest == false):
		for($j = 0; $j < count($ranks_sql['bannedrow']); $j++):
			if($ranks_sql['bannedrow'][$j]['ban_userid'] == $nuke_user_row['user_id'] )
		    $is_banned = true;
		endfor;
	endif;
	...
	for($j = 0; $j < count($ranks_sql['ranksrow']); $j++):
		...
		if($is_guest == true):
			if ($ranks_sql['ranksrow'][$j]['rank_special'] == '2'):
				$nuke_user_ranks['rank_01'] = $rank_tmp;
				$nuke_user_ranks['rank_01_img'] = $rank_img_tmp;
			endif;
		elseif($is_banned == true):
			...
			$day_diff = intval( (time() - (int) $nuke_user_row['user_regdate']) / 86400 );

			for($k = 0; $k < count($nuke_user_fields_array); $k++):
			
				switch($ranks_sql['ranksrow'][$j]['rank_special']):
					case '1':
						if($nuke_user_row[$nuke_user_fields_array[$k]] == $ranks_sql['ranksrow'][$j]['rank_id'])
						$rank_sw = true;
						break;
					case '0':
						if(($nuke_user_row[$nuke_user_fields_array[$k]] == '0') && ($nuke_user_row['user_posts'] >= $ranks_sql['ranksrow'][$j]['rank_min']))
						$rank_sw = true;
						break;
					case '-1':
						if(($nuke_user_row[$nuke_user_fields_array[$k]] == '-1') && ($day_diff >= $ranks_sql['ranksrow'][$j]['rank_min']))
						$rank_sw = true;
						break;
					default:
						break;
				...
				if ($rank_sw == true):
					$nuke_user_ranks[$nuke_user_ranks_array[(($k + 1) * 2) - 2]] = $rank_tmp;
					$nuke_user_ranks[$nuke_user_ranks_array[(($k + 1) * 2) - 1]] = $rank_img_tmp;
					$rank_sw = false;
				endif;
			endfor;

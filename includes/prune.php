...
function prune($forum_id, $prune_date, $prune_all = false)
{
        global $nuke_db, $lang;
     	...
     	{
     		message_die(NUKE_GENERAL_ERROR, 'Could not obtain lists of topics to sync', '', __LINE__, __FILE__, $sql);
     	}
        ...
        $prune_all = ($prune_all) ? '' : 'AND t.topic_vote = 0 AND t.topic_type <> ' . NUKE_POST_ANNOUNCE;
        //
        ...
        if ( !($result = $nuke_db->sql_query($sql)) )
        {
                message_die(NUKE_GENERAL_ERROR, 'Could not obtain lists of topics to prune', '', __LINE__, __FILE__, $sql);
        }
        ...
        if( $sql_topics != '' )
        {
                ...
                if ( !($result = $nuke_db->sql_query($sql)) )
                {
                        message_die(NUKE_GENERAL_ERROR, 'Could not obtain list of posts to prune', '', __LINE__, __FILE__, $sql);
                }
                ...
                if ( $sql_post != '' )
                {
                        ...
                        {
                                message_die(NUKE_GENERAL_ERROR, 'Could not delete watched topics during prune', '', __LINE__, __FILE__, $sql);
                        }
                        ...
                        if ( !$nuke_db->sql_query($sql) )
                        {
                                message_die(NUKE_GENERAL_ERROR, 'Could not delete topics during prune', '', __LINE__, __FILE__, $sql);
                        }

                        $pruned_topics = $nuke_db->sql_affectedrows();

                        $sql = "DELETE FROM " . NUKE_POSTS_TABLE . "
                                WHERE post_id IN ($sql_post)";
                        if ( !$nuke_db->sql_query($sql) )
                        {
                                message_die(NUKE_GENERAL_ERROR, 'Could not delete post_text during prune', '', __LINE__, __FILE__, $sql);
                        }

                        $pruned_posts = $nuke_db->sql_affectedrows();

                        $sql = "DELETE FROM " . NUKE_POSTS_TEXT_TABLE . "
                                WHERE post_id IN ($sql_post)";
                        if ( !$nuke_db->sql_query($sql) )
                        {
                                message_die(NUKE_GENERAL_ERROR, 'Could not delete post during prune', '', __LINE__, __FILE__, $sql);
                        }
...
function auto_prune($forum_id = 0)
{
        ...
        if ( !($result = $nuke_db->sql_query($sql)) )
        {
                message_die(NUKE_GENERAL_ERROR, 'Could not read auto_prune table', '', __LINE__, __FILE__, $sql);
        }
        ...
        if ( $row = $nuke_db->sql_fetchrow($result) )
        {
                if ( $row['prune_freq'] && $row['prune_days'] )
                {
                        ...
                        if ( !$nuke_db->sql_query($sql) )
                        {
                                message_die(NUKE_GENERAL_ERROR, 'Could not update forum table', '', __LINE__, __FILE__, $sql);
                        }
                }
        }

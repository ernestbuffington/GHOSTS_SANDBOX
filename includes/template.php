...
class Template {
	...
	var $nuke_cache_search = array();
	var $nuke_cache_replace = array();
	...
	// Cache settings
	var $use_nuke_cache = 1;
	var $nuke_cache_writable = 1;
 ...
 ******************************************************/
	...
	function set_filename($handle, $filename, $xs_include = false, $quiet = false)
	{
		...
		{
			$nuke_cache_time = @filemtime($this->files_cache[$handle]);
			if(@filemtime($this->files[$handle]) > $nuke_cache_time || $board_config['xs_template_time'] > $nuke_cache_time)
			{	
	...
	*/
	 ...
	 */
	function compile2($code, $handle, $nuke_cache_file)
	{
		$code = $this->compile_code('', $code, XS_USE_ISSET);
		if($nuke_cache_file && !empty($this->use_cache) && !empty($this->auto_compile))
		{
			$res = $this->write_cache($nuke_cache_file, $code);
			if($handle && $res)
			{
				$this->files_cache[$handle] = $nuke_cache_file;
			}
		}
		$code = '?'.'>'.$code.'<'."?php\n";
		return $code;

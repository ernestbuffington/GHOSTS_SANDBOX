...
class Swift_Mime_SimpleMimeEntity implements Swift_Mime_MimeEntity
{
  ...
  /**
   * Create a new SimpleMimeEntity with $headers, $encoder and $nuke_cache.
   * @param Swift_Mime_HeaderSet $headers
   * @param Swift_Mime_ContentEncoder $encoder
   * @param Swift_KeyCache $nuke_cache
   */
  public function __construct(Swift_Mime_HeaderSet $headers,
    Swift_Mime_ContentEncoder $encoder, Swift_KeyCache $nuke_cache)
  {
    $this->_cacheKey = uniqid();
    $this->_cache = $nuke_cache;
  ...
  public function toByteStream(Swift_InputByteStream $is)
  {
    ...
    if (empty($this->_immediateChildren))
    {
      if (isset($this->_body))
      {
        ...
        {
          $nuke_cacheIs = $this->_cache->getInputByteStream($this->_cacheKey, 'body');
          if ($nuke_cacheIs)
          {
            $is->bind($nuke_cacheIs);
          }

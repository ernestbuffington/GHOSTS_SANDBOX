...
if (!empty($cat)) 
{
    $catid = $nuke_db->sql_ufetchrow("SELECT cid FROM `".$prefix."_file_repository_categories` WHERE `cname` LIKE '%$cat%' LIMIT 1");
    if (empty($catid))
    {
        $result = $nuke_db->sql_query("SELECT `did`, `title`, `description`, `date`, `sname` FROM `".$prefix."_file_repository_items` WHERE `isactive` = 1 ORDER BY `did` DESC ".$num);
    } 
    else 
    {
        $catid = intval($catid);
        $result = $nuke_db->sql_query("SELECT `did`, `title`, `description`, `date`, `sname` FROM `".$prefix."_file_repository_items` WHERE `cid`='$catid' && `isactive` = 1 ORDER BY `did` DESC ".$num);
    }
} 
else 
{
...
while(list($did, $title, $description, $date, $submitter) = $nuke_db->sql_fetchrow($result)):

...
$cat = intval($cat);
if (!empty($cat)) {
    $catid = $nuke_db->sql_fetchrow($nuke_db->sql_query("SELECT catid FROM ".$prefix."_links_categories WHERE title LIKE '%$cat%' LIMIT 1"));
    if (empty($catid)) {
        $result = $nuke_db->sql_query("SELECT lid, title, description, date, submitter FROM ".$prefix."_links_links ORDER BY lid DESC ".$num);
    } else {
        $catid = intval($catid);
        $result = $nuke_db->sql_query("SELECT lid, title, description, date, submitter FROM ".$prefix."_links_links WHERE catid='$catid' ORDER BY lid DESC ".$num);
    }
} else {
    $result = $nuke_db->sql_query("SELECT lid, title, description, date, submitter FROM ".$prefix."_links_links ORDER BY lid DESC ".$num);
}
...
while(list($lid, $title, $description, $date) = $nuke_db->sql_fetchrow($result)) {

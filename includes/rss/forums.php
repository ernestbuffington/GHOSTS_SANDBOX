...
$result = $nuke_db->sql_query("SELECT t.topic_id, t.topic_title, t.topic_last_post_id FROM ".$prefix."_bbtopics t, ".$prefix."_bbforums f WHERE t.forum_id=f.forum_id AND f.auth_view=0 ORDER BY t.topic_last_post_id DESC ".$num);
...
while(list($topic_id, $topic_title, $topic_last_post_id) = $nuke_db->sql_fetchrow($result)) {
    $topic_title = entity_to_hex_value($topic_title);
    $result2 = $nuke_db->sql_query("SELECT post_text FROM ".$prefix."_bbposts_text WHERE post_id='$topic_last_post_id'");
    list($desc) = $nuke_db->sql_fetchrow($result2);
    ...
    $result3 = $nuke_db->sql_query("SELECT post_time AS post_time FROM ".$prefix."_bbposts WHERE post_id='$topic_last_post_id'");
    list($post_time) = $nuke_db->sql_fetchrow($result3);

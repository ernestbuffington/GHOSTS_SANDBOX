...
function nuke_phpBB_whoisonline($force=FALSE) # I see no call? TheGhost
{
    ...
    $result = $nuke_db->sql_query("SELECT * FROM `"._SESSION_TABLE."` ORDER BY `time` ASC");
    $count_sess     = 1;
    $count_hidden   = 0;
    $count_reg_user = 0;
    $count_guests   = 0;
    ...
    if ($nuke_db->sql_numrows($result) > 0) {
        while($sessions_all = $nuke_db->sql_fetchrow($result)) {
...
function select_nuke_session_url($session_page, $url_qs, $url_ps, $specific, $level, $id, $forum_data, $topic_data, $nuke_user_data, $cat_data)
{
    ...
    unset($location);
        ...
        $url_qs     = NUKE_POST_FORUM_URL .'='. intval($session_page);
        $url_ps        = '/modules.php?name=Forums&file=viewforum';
            }

        if ( (@strstr($url_qs, NUKE_POST_FORUM_URL .'=')) && (@strstr($url_ps, 'viewforum.')) )
            {
        ...
        if ( (@strstr($url_qs, NUKE_POST_TOPIC_URL .'=')) && (@strstr($url_ps, 'viewtopic.')) )
            {
        ...
        if (!$location)
            {
            if ( (@strstr($url_qs, NUKE_POST_POST_URL .'=')) && (@strstr($url_ps, 'viewtopic.')) )
        ...
        function set_session_url($id)
        {
          ...
          if (isset($HTTP_GET_VARS[NUKE_POST_USERS_URL]))
            $specific = intval($HTTP_GET_VARS[NUKE_POST_USERS_URL]);

          if (isset($HTTP_GET_VARS[NUKE_POST_FORUM_URL]))
            $specific = intval($HTTP_GET_VARS[NUKE_POST_FORUM_URL]);

          if (isset($HTTP_GET_VARS[NUKE_POST_GROUPS_URL]))
            $specific = intval($HTTP_GET_VARS[NUKE_POST_GROUPS_URL]);

          if (isset($HTTP_GET_VARS[NUKE_POST_CAT_URL]))
            $specific = intval($HTTP_GET_VARS[NUKE_POST_CAT_URL]);

          if (isset($HTTP_GET_VARS[NUKE_POST_TOPIC_URL]))
            $specific = intval($HTTP_GET_VARS[NUKE_POST_TOPIC_URL]);

          if (isset($HTTP_GET_VARS[NUKE_POST_POST_URL]))
            $specific = intval($HTTP_GET_VARS[NUKE_POST_POST_URL]);

          if (!isset($specific))
            $specific = 0;

    $q = "UPDATE ". NUKE_BB_SESSIONS_TABLE ." SET session_url_qs = '$query_string', session_url_ps = '$php_self', session_url_specific = '$specific' WHERE session_id = '$id'";
    $nuke_db->sql_query($q);
}
/*****[END]********************************************
 [ Mod:    Better Session Handling             v1.0.0 ]
...
function nuke_session_begin($nuke_user_id, $nuke_user_ip, $page_id, $auto_create = 0, $enable_autologin = 0, $admin = 0)
{
    ...
    if ( isset($HTTP_COOKIE_VARS[$cookiename . '_sid']) || isset($HTTP_COOKIE_VARS[$cookiename . '_data']) )
    {
        ...
        $sessionmethod = NUKE_SESSION_METHOD_COOKIE;
    }
    else
    {
        $sessiondata = array();
        $session_id = ( isset($HTTP_GET_VARS['sid']) ) ? $HTTP_GET_VARS['sid'] : '';
        $sessionmethod = NUKE_SESSION_METHOD_GET;
    }
    ...
    if ( !($result = $nuke_db->sql_query($sql, true)) )
    {
        message_die(NUKE_CRITICAL_ERROR, 'Could not obtain ban information', '', __LINE__, __FILE__, $sql);
    }

    if ( $ban_info = $nuke_db->sql_fetchrow($result) )
    {
        if ( $ban_info['ban_ip'] || $ban_info['ban_userid'] || $ban_info['ban_email'] )
        {
            message_die(NUKE_CRITICAL_MESSAGE, 'You_been_banned');
        }
    }
    ...
    if ( !$nuke_db->sql_query($sql) || !$nuke_db->sql_affectedrows() )
    {
        ...
        if ( !$nuke_db->sql_query($sql) )
        {
                ...
                {
                    $sql_error = $nuke_db->sql_error($result);
                    if ($sql_error["code"] == 1114)
                    {
                        $result = $nuke_db->sql_query('SHOW TABLE STATUS LIKE "'.NUKE_BB_SESSIONS_TABLE.'"');
                        ...
                        if ($row["Type"] == "HEAP")
                        {
                            if ($row["Rows"] > 2500)
                            {
                                ...
                                $nuke_db->sql_query("DELETE QUICK FROM ".NUKE_BB_SESSIONS_TABLE."$delete_order LIMIT 50");
                            }
                            else
                            {
                                $nuke_db->sql_query("ALTER TABLE ".NUKE_BB_SESSIONS_TABLE." MAX_ROWS=".($row["Rows"]+50));
                            }
                            if ($nuke_db->sql_query($sql))
                            {
                                $error = FALSE;
                ...
                if ($error)
                {
                    message_die(NUKE_CRITICAL_ERROR, "Error creating new session", "", __LINE__, __FILE__, $sql);
                }
                }
    }
    ...
    if ( $nuke_user_id != NUKE_ANONYMOUS )
    {
        ...
        if ($enable_autologin)
        {
            ...
            if ( !$nuke_db->sql_query($sql) )
            {
                message_die(NUKE_CRITICAL_ERROR, 'Error updating session key', '', __LINE__, __FILE__, $sql);
            }
 ...
 ******************************************************/
    ...
    session_start();
    global $nuke_db, $lang, $board_config, $session_id, $HTTP_COOKIE_VARS, $HTTP_GET_VARS, $SID, $pc_dateTime, $HTTP_SESSION_VARS, $cookie;
/*****[END]********************************************
 [ Mod:    Advanced Time Management            v2.2.0 ]
 ******************************************************/
 ...
 ******************************************************/
    ...
    if ( !empty($session_id) )
    {
        ...
        //
        $sql = "SELECT u.*, s.*
            FROM (" . NUKE_BB_SESSIONS_TABLE . " s, " . NUKE_USERS_TABLE . " u)
 ...
 ******************************************************/
        ...
        if ( isset($nuke_userdata['user_id']) )
        {
            ...
            {
                $SID = ($sessionmethod == NUKE_SESSION_METHOD_GET || defined('IN_ADMIN')) ? 'sid=' . $session_id : '';
...
function session_nuke_end($session_id, $nuke_user_id)
{
    ...
    if ( isset($nuke_userdata['session_key']) && $nuke_userdata['session_key'] != '' )
    {
        ...
        if ( !$nuke_db->sql_query($sql) )
        {
            message_die(NUKE_CRITICAL_ERROR, 'Error removing auto-login key', '', __LINE__, __FILE__, $sql);
        }
    }
...
function session_nuke_clean($session_id)
{
    ...
    {
        message_die(NUKE_CRITICAL_ERROR, 'Error clearing sessions table', '', __LINE__, __FILE__, $sql);
    }
    $nuke_db->sql_freeresult($result);
...
function session_nuke_reset_keys($nuke_user_id, $nuke_user_ip)
{
	...
	if ( !$nuke_db->sql_query($sql) )
	{
		message_die(NUKE_CRITICAL_ERROR, 'Error removing user session(s)', '', __LINE__, __FILE__, $sql);
	}
...
function append_nuke_sid($url, $non_html_amp = false)
{
    ...
    else if (preg_match("/nuke_redirect/", $url))
    {
            $url = str_replace("login.php", "modules.php?name=Your_Account", $url);         //  Strip the .php from all the files,
            $url = str_replace(".php", "", $url);         //  Strip the .php from all the files,
            $url = ($non_html_amp) ? str_replace("?nuke_redirect", "&nuke_redirect", $url) : str_replace("?nuke_redirect", "&amp;nuke_redirect", $url);         //  Strip the .php from all the files,
            $url = str_replace("modules", "modules.php", $url); //  and put it back for the modules.php
    }

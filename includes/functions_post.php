 ...
 ******************************************************/
        ...
        if ($mode != 'editpost')
        {
                $post_id = $nuke_db->sql_nextid();
        }

        $sql = ($mode != 'editpost') ? "INSERT INTO " . NUKE_POSTS_TEXT_TABLE . " (post_id, post_subject, bbcode_uid, post_text) VALUES ('$post_id', '$post_subject', '$bbcode_uid', '$post_message')" : "UPDATE " . NUKE_POSTS_TEXT_TABLE . " SET post_text = '$post_message',  bbcode_uid = '$bbcode_uid', post_subject = '$post_subject' WHERE post_id = '$post_id'";
        if (!$nuke_db->sql_query($sql))
        {
                message_die(NUKE_GENERAL_ERROR, 'Error in posting', '', __LINE__, __FILE__, $sql);
        }
...
/*****[BEGIN]******************************************
 [ Mod:    Must first vote to see results      v1.0.0 ]
 ******************************************************/
                $sql = (!$post_data['has_poll']) ? "INSERT INTO " . NUKE_VOTE_DESC_TABLE . " (topic_id, vote_text, vote_start, vote_length, poll_view_toggle) VALUES ('$topic_id', '$poll_title', '$current_time', " . ($poll_length * 86400) . ", '$poll_view_toggle')" : "UPDATE " . NUKE_VOTE_DESC_TABLE . " SET vote_text = '$poll_title', vote_length = " . ($poll_length * 86400) . ", poll_view_toggle = '$poll_view_toggle' WHERE topic_id = '$topic_id'";
/*****[END]********************************************
 [ Mod:    Must first vote to see results      v1.0.0 ]
 ******************************************************/
                if (!$nuke_db->sql_query($sql))
                ...
                if ($mode == 'editpost' && $post_data['has_poll'])
                {
                        ...
                        if (!($result = $nuke_db->sql_query($sql)))
                        {
                                message_die(NUKE_GENERAL_ERROR, 'Could not obtain vote data results for this topic', '', __LINE__, __FILE__, $sql);
                        }
                ...
                while (list($option_id, $option_text) = each($poll_options))
                {
                        if (!empty($option_text))
                        {
                                ...
                                $sql = ($mode != "editpost" || !isset($old_poll_result[$option_id])) ? "INSERT INTO " . NUKE_VOTE_RESULTS_TABLE . " (vote_id, vote_option_id, vote_option_text, vote_result) VALUES ('$poll_id', '$poll_option_id', '$option_text', '$poll_result')" : "UPDATE " . NUKE_VOTE_RESULTS_TABLE . " SET vote_option_text = '$option_text', vote_result = '$poll_result' WHERE vote_option_id = '$option_id' AND vote_id = '$poll_id'";
                                if (!$nuke_db->sql_query($sql))
                                {
                                        message_die(NUKE_GENERAL_ERROR, 'Error in posting', '', __LINE__, __FILE__, $sql);
                                }
                ...
                if ($delete_option_sql != '')
                {
                        ...
                        if (!$nuke_db->sql_query($sql))
                        {
                                message_die(NUKE_GENERAL_ERROR, 'Error deleting pruned poll options', '', __LINE__, __FILE__, $sql);
                        }
                }
        }
...
function update_post_stats(&$mode, &$post_data, &$forum_id, &$topic_id, &$post_id, &$user_id)
{
    	...
    	if ($mode != 'poll_delete')
    	{
    		$sql = "UPDATE " . NUKE_FORUMS_TABLE . " SET
    			$forum_update_sql
    			WHERE forum_id = $forum_id";
    		if (!$nuke_db->sql_query($sql))
    		{
        ...
        if ($topic_update_sql != '')
        {
                $sql = "UPDATE " . NUKE_BB_TOPICS_TABLE . " SET
                        $topic_update_sql
                        WHERE topic_id = '$topic_id'";
                if (!$nuke_db->sql_query($sql))
                {
 ...
 ******************************************************/
    if ($userdata['user_level'] == NUKE_USER) {
    	...
    	if ( !($result = $nuke_db->sql_query($sql)) )
    	{
    		message_die(NUKE_GENERAL_ERROR, 'Error geting users post stat', '', __LINE__, __FILE__, $sql);
    	}
 ...
 ******************************************************/
        ...
        if ($mode == 'poll_delete' || ($mode == 'delete' && $post_data['first_post'] && $post_data['last_post']) && $post_data['has_poll'] && $post_data['edit_poll'])
        {
                ...
                if (!$nuke_db->sql_query($sql))
                {
                        message_die(NUKE_GENERAL_ERROR, 'Error in deleting poll', '', __LINE__, __FILE__, $sql);
                }
 ...
 ******************************************************/
                        ...
                        if (!($result_topic = $nuke_db->sql_query($sql_topic)))
                        {
                                message_die(NUKE_GENERAL_ERROR, 'Could not get user name', '', __LINE__, __FILE__, $sql_topic);
                        }
                        $row_topic = $nuke_db->sql_fetchrow($result_topic);
                        if(!empty($row_topic["post_attachment"]))
                        {
                                ...
                                if (!($result_attach = $nuke_db->sql_query($sql_attach)))
                                {
                                       message_die(NUKE_GENERAL_ERROR, 'Could not get attachment', '', __LINE__, __FILE__, $sql_attach);
                                }
                                $row_attach = $nuke_db->sql_fetchrow($result_attach);
                        }
/*****[END]********************************************
 [ Mod:     Topic Text Reply Email             v1.0.0 ]
 ******************************************************/
                        if (!($result = $nuke_db->sql_query($sql)))
                        {
                                message_die(NUKE_GENERAL_ERROR, 'Could not obtain list of topic watchers', '', __LINE__, __FILE__, $sql);
                        }
                        ...
                        if ($row = $nuke_db->sql_fetchrow($result))
                        {
                                ...
                                if (count($bcc_list_ary))
                                {
                                    ...
                                    $notify_body_replace    = array(
                                        ...
                                        $server_protocol . $server_name . $server_port . $script_name . '&' . NUKE_POST_POST_URL . "=$post_id#$post_id",
                                        $poster_name,
                                        $text,
                                        $attachment,
                                        $server_protocol . $server_name . $server_port . $script_name . '&' . NUKE_POST_TOPIC_URL . "=$topic_id&unwatch=topic",
                                    ...
                                    $email_data = array(
                                        ...
                                        'topic_link'    => $server_protocol . $server_name . $server_port . $script_name . '&' . NUKE_POST_POST_URL . "=$post_id#$post_id",
                                        'reply_by'      => $poster_name,
                                        'contents'      => $text,
                                        'attachment'    => $attachment,
                                        'stop_watching' => $server_protocol . $server_name . $server_port . $script_name . '&' . NUKE_POST_TOPIC_URL . "=$topic_id&unwatch=topic",
                                        'signature'     => (!empty($board_config['board_email_sig'])) ? str_replace('<br />', "\n", "-- \n" . $board_config['board_email_sig']) : ''
                                    );
                        ...
                        if ($update_watched_sql != '')
                        {
                                $sql = "UPDATE " . NUKE_TOPICS_WATCH_TABLE . "
                                        SET notify_status = " . NUKE_TOPIC_WATCH_NOTIFIED . "
                                        WHERE topic_id = '$topic_id'

...
/*****[END]********************************************
 [ Base:    Caching System                     v1.0.0 ]
 ******************************************************/
            $result = $nuke_db->sql_query("SELECT * FROM `".$prefix."_nsncb_blocks`");
...
function CBSample($set) {
    ...
    $cbinfo = $nuke_db->sql_fetchrow($nuke_db->sql_query("SELECT * FROM `".$prefix."_nsncb_config` WHERE `cgid`='$set'"));
    ...
    $result3 = $nuke_db->sql_query("SELECT * FROM `".$prefix."_nsncb_blocks` WHERE `cgid`='$set' ORDER BY `cbid`");
    while($cbidinfo = $nuke_db->sql_fetchrow($result3)) {

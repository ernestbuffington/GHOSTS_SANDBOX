...
function is_client($client) {
    ...
    if (!empty($cid) AND !empty($pwd)) {
        list($pass) = $nuke_db->sql_ufetchrow("SELECT passwd FROM ".$prefix."_banner_clients WHERE cid='$cid'");
...
function theindex() {
    global $prefix, $nuke_db, $sitename;
...
function plans() {
    ...
    OpenTable();
    $result = $nuke_db->sql_query("SELECT * FROM ".$prefix."_banner_plans WHERE active='1'");
...
function client_home() {
    ...
    if (!is_client($client)) {
        ...
        $client = explode(":", $client);
        $cid = $client[0];
        $row = $nuke_db->sql_ufetchrow("SELECT * FROM ".$prefix."_banner_clients WHERE cid='$cid'");
...
function view_banner($cid, $bid) {
    ...
    if (!is_client($client)) {
        ...
        if ($cid != $client_id) {
            ...
            title($sitename.' '._ADSYSTEM);
            OpenTable();
            $row = $nuke_db->sql_ufetchrow("SELECT * FROM ".$prefix."_banner WHERE bid='$bid'");
            $cid = intval($row['cid']);
...
function client_report($cid, $bid) {
    ...
    if (!is_client($client)) {
        ...
        if ($cid != $client_id) {
            ...
            $bid = intval($bid);
            $cid = intval($cid);
            list($name, $email) = $nuke_db->sql_ufetchrow("SELECT name, email FROM ".$prefix."_banner_clients WHERE cid='$cid'");
            ...
            if (empty($email)) {
                ...
                list($bid, $imptotal, $impmade, $clicks, $imageurl, $clickurl, $date, $ad_class) = $nuke_db->sql_ufetchrow("SELECT bid, name, imptotal, impmade, clicks, imageurl, clickurl, date, ad_class FROM ".$prefix."_banner WHERE bid='$bid' AND cid='$cid'");
                $bid = intval($bid);
                $imptotal = intval($imptotal);
                $impmade = intval($impmade);
                $clicks = intval($clicks);

...
elseif(isset($_POST['unsub']) || isset($_POST['unsubpending']) && $group_id) 
{
    ...
    if($confirm): 
        ...
        $sql = "UPDATE ".NUKE_USERS_TABLE." SET user_color_gc = '', user_color_gi  = '', user_rank = 0 WHERE user_id = ".$userdata['user_id'];
        ...
        $sql = "DELETE FROM ".NUKE_USER_GROUP_TABLE." WHERE user_id = ".$userdata['user_id']." AND group_id = '$group_id'";
        ...
        if($userdata['user_level'] != NUKE_ADMIN && $userdata['user_level'] == NUKE_MOD): 
            ...
            if(!($row = $nuke_db->sql_fetchrow($result)) || $row['is_auth_mod'] == 0): 
                $sql = "UPDATE ".NUKE_USERS_TABLE." SET user_level = ".NUKE_USER." WHERE user_id = ".$userdata['user_id'];
                if (!($result = $nuke_db->sql_query($sql))) 
...
elseif($group_id) 
{
	...
	elseif($is_group_member || $is_group_pending_member): 
        ...
        $s_hidden_fields = '<input type="hidden" name="' . NUKE_POST_GROUPS_URL . '" value="' . $group_id . '" />';
	elseif($userdata['user_id'] == NUKE_ANONYMOUS): 

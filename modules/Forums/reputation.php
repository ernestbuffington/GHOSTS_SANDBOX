...
{
  message_die(NUKE_GENERAL_MESSAGE, $lang['No_action_specified']);
}
...
switch( $action  )
{
  case 'post':
    ...
    {
      message_die(NUKE_GENERAL_MESSAGE, $lang['No_user']);
    }
    if ( empty($HTTP_POST_VARS['post_id_to_give']) || !isset($HTTP_POST_VARS['post_id_to_give']) )
    {
      message_die(NUKE_GENERAL_MESSAGE, $lang['No_post']);
    }
    ...
    if ($repsum == 0)
    {
      message_die(NUKE_GENERAL_ERROR, $lang['Cant_give_zero']);
    } else if ($repsum < 0) {
      message_die(NUKE_GENERAL_ERROR, $lang['Cant_give_subzero']);
    }
    if (($repsum > $rep_config['repsum_limit']) && ($rep_config['repsum_limit'] != 0))
    {
      message_die(NUKE_GENERAL_ERROR, sprintf($lang['No_more_than_limit'], $rep_config['repsum_limit']));
    }

    // Check "ccode" of the post
    $sql = "SELECT p.bbcode_uid
        FROM " . NUKE_POSTS_TEXT_TABLE . " AS p
        WHERE p.post_id = " . $postid;
    if ( !($result = $nuke_db->sql_query($sql)) )
    {
      message_die(NUKE_GENERAL_ERROR, "Could not obtain information from posts", '', __LINE__, __FILE__, $sql);
    }
    ...
    if ( $nuke_userdata['user_level'] != NUKE_ADMIN && $nuke_userdata['user_level'] != NUKE_MOD )
    {
      ...
      {
        message_die(NUKE_GENERAL_MESSAGE, sprintf($lang['Too_little_time'], $rep_config['flood_control_time']));
      }
    }
  ...
  case 'add':
    ...
    {
      message_die(NUKE_GENERAL_MESSAGE, $lang['No_post_id_specified']);
    }
 ...
 ******************************************************/
  ...
  case 'globalstats':
    ...
    $sql = "SELECT COUNT(user_id) AS total_count, SUM(rep_sum) AS total_sum
        FROM " . NUKE_REPUTATION_TABLE;
    if ( !($result = $nuke_db->sql_query($sql)) )
    {
      message_die(NUKE_GENERAL_ERROR, "Could not obtain reputation stats", '', __LINE__, __FILE__, $sql);
    }
    $row = $nuke_db->sql_fetchrow($result);
    ...
    $sql = "SELECT u.username, r.user_id_2, r.rep_sum
        FROM " . NUKE_REPUTATION_TABLE . " AS r
        LEFT JOIN " . NUKE_USERS_TABLE . " AS u ON
 ...
 ******************************************************/
    ...
    $sql = "SELECT r.*, u.username, u.user_id, t.topic_title, f.forum_id, p.post_id
        ...
        LEFT JOIN " . NUKE_USERS_TABLE . " AS u ON
          u.user_id = r.user_id
        LEFT JOIN " . NUKE_POSTS_TABLE . " AS p ON
          p.post_id = r.post_id
        LEFT JOIN " . NUKE_BB_TOPICS_TABLE . " AS t ON
    ...
    while ($row = $nuke_db->sql_fetchrow($result))
    {
      ...
      {
        message_die(NUKE_GENERAL_ERROR, "Could not obtain data for this user", '', __LINE__, __FILE__, $sql);
      }
 ...
 ******************************************************/
    ...
    if ( !($result = $nuke_db->sql_query($sql)) )
    {
      message_die(NUKE_GENERAL_ERROR, "Could not obtain data for this post", '', __LINE__, __FILE__, $sql);
    }

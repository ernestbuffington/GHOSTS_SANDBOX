...
global $network_prefix, $network_db, $admin_file;
...
if(is_mod_admin($nuke_module_name)) {
    ...
    list($c_num) = $network_db->sql_ufetchrow("SELECT COUNT(*) FROM ".$network_prefix."_banner_clients", SQL_NUM);
    if ($c_num == 0) {
    ...
    function NetworkBannersAdmin() {
        global $network_prefix, $network_db, $bgcolor2, $banners, $admin_file, $ad_admin_menu_main, $bgcolor1;
        ...
        $result = $network_db->sql_query("SELECT bid, cid, name, imptotal, impmade, clicks, imageurl, date, position, active, ad_class, type FROM " . $network_prefix . "_banner WHERE active='1' ORDER BY position,bid", true);
        while (list($bid, $cid, $name, $imptotal, $impmade, $clicks, $imageurl, $date, $type, $active, $ad_class) = $network_db->sql_fetchrow($result)) {
            $bid = intval($bid);
            $cid = intval($cid);
            ...
            list($cid, $client_name) = $network_db->sql_ufetchrow("SELECT cid, name FROM " . $network_prefix . "_banner_clients WHERE cid='$cid'");
            $cid = intval($cid);
            $name = trim($name);
            if (empty($name)) {
                $name = _NONE;
            ...
            $row2 = $network_db->sql_ufetchrow("SELECT apid, position_name FROM ".$network_prefix."_banner_positions WHERE position_number='$type'");
        ...
        ."<td align=\"center\"><strong>" . _CLASS . "</strong></td>"
        ."<td align=\"center\"><strong>" . _FUNCTIONS . "</strong></td><tr>";
        $result = $network_db->sql_query("SELECT bid, cid, name, imptotal, impmade, clicks, imageurl, date, position, active, ad_class FROM " . $network_prefix . "_banner WHERE active='0' ORDER BY position,bid");
        while ($row = $network_db->sql_fetchrow($result)) {
            ...
            $row2 = $network_db->sql_ufetchrow("SELECT cid, name FROM " . $network_prefix . "_banner_clients WHERE cid='$cid'");
            $cid = intval($row2['cid']);
            $name = trim($row2['name']);
        ...
        ."<td align=\"center\"><strong>" . _FUNCTIONS . "</strong></td><tr>";
        $result3 = $network_db->sql_query("SELECT cid, name, contact, email FROM " . $network_prefix . "_banner_clients ORDER BY cid");
        while ($row3 = $network_db->sql_fetchrow($result3)) {
            $cid = intval($row3['cid']);
            $name = $row3['name'];
            $contact = $row3['contact'];
            $email = $row3['email'];
            $result4 = $network_db->sql_query("SELECT cid FROM " . $network_prefix . "_banner WHERE cid='$cid' AND active='1'");
            $numrows = $network_db->sql_numrows($result4);
            $row4 = $network_db->sql_fetchrow($result4);
            $network_db->sql_freeresult($result4);
            $rcid = intval($row4['cid']);
            list($numrows2) = $network_db->sql_ufetchrow("SELECT COUNT(*) FROM " . $network_prefix . "_banner WHERE cid='$cid' AND active='0'");
            echo "<td bgcolor=\"$bgcolor1\" align=\"center\">$name</td>"
    ...
    function add_network_banner() {
        global $network_prefix, $network_db, $banners, $admin_file, $ad_admin_menu;
        ...
        echo "<br />";
        OpenTable();
        $result = $network_db->sql_query("select * FROM ".$network_prefix."_banner_clients");
        if($network_db->sql_numrows($result) > 0) {
            ...
            $result = $network_db->sql_query("SELECT position_number, position_name FROM ".$network_prefix."_banner_positions ORDER BY position_number");
            while ($row = $network_db->sql_fetchrow($result)) {
    ...
    function NetworkBannerStatus($bid, $status) {
        ...
        $bid = intval($bid);
        $network_db->sql_query("UPDATE " . $network_prefix . "_banner SET active='$active' WHERE bid='$bid'");
        nuke_redirect($admin_file.'.php?op=NetworkBannersAdmin');
    }
    ...
    function NetworkBannersAdd($name, $cid, $adname, $imptotal, $imageurl, $clickurl, $alttext, $position, $active, $ad_class, $ad_code, $ad_width, $ad_height) {
        global $network_prefix, $network_db, $admin_file, $ad_admin_menu;
        ...
        $network_db->sql_query("insert into " . $network_prefix . "_banner VALUES (NULL, '$cid', '$adname', '$imptotal', '1', '0', '$imageurl', '$clickurl', '$alttext', now(), '0000-00-00 00:00:00', '$position', '$active', '$ad_class', '$ad_code', '$ad_width', '$ad_height', '')");
        nuke_redirect($admin_file.'.php?op=NetworkBannersAdmin');
    }

    function NetworkBannerAddClient($name, $contact, $email, $login, $passwd, $extrainfo) {
        global $network_prefix, $network_db, $admin_file;
        $network_db->sql_query("insert into " . $network_prefix . "_banner_clients VALUES (NULL, '$name', '$contact', '$email', '$login', '$passwd', '$extrainfo')");
        nuke_redirect($admin_file.'.php?op=NetworkBannersAdmin');
    }

    function NetworkBannerDelete($bid, $ok=0) {
        global $network_prefix, $network_db, $admin_file, $bgcolor1, $bgcolor2, $ad_admin_menu;
        $bid = intval($bid);
        if ($ok == 1) {
            $network_db->sql_query("DELETE FROM " . $network_prefix . "_banner WHERE bid='$bid'");
            nuke_redirect($admin_file.".php?op=NetworkBannersAdmin");
        } else {
            include_once(NUKE_BASE_DIR.'header.php');
            OpenTable();
	    ...
	    echo "<br />";
            ...
            CloseTable();
            echo "<br />";
            $row = $network_db->sql_fetchrow($network_db->sql_query("SELECT cid, name, imptotal, impmade, clicks, imageurl, clickurl, ad_class, ad_code, ad_width, ad_height FROM " . $network_prefix . "_banner WHERE bid='$bid'"));
            $cid = intval($row['cid']);
            $imptotal = intval($row['imptotal']);
            $impmade = intval($row['impmade']);
            $clicks = intval($row['clicks']);
    ...
    function NetworkBannerEdit($bid) {
        global $network_prefix, $network_db, $admin_file, $ad_admin_menu, $admlang;
        include_once(NUKE_BASE_DIR.'header.php');
        OpenTable();
	    echo "<div align=\"center\">\n<a href=\"$admin_file.php?op=NetworkBannersAdmin\">" . _BANNERS_ADMIN_HEADER . "</a></div>\n";
        echo "<br /><br />";
        ...
        echo "<br />";
        $bid = intval($bid);
        $row = $network_db->sql_ufetchrow("SELECT cid, name, imptotal, impmade, clicks, imageurl, clickurl, alttext, date, position, active, ad_class, ad_code, ad_width, ad_height FROM " . $network_prefix . "_banner WHERE bid='$bid'");
        $cid = intval($row['cid']);
        $imptotal = intval($row['imptotal']);
        $impmade = intval($row['impmade']);
        $clicks = intval($row['clicks']);
        ...
        $result3 = $network_db->sql_query("SELECT cid, name FROM " . $network_prefix . "_banner_clients");
        while ($row3 = $network_db->sql_fetchrow($result3)) {
        ...
        $result4 = $network_db->sql_query("SELECT position_number, position_name FROM ".$network_prefix."_banner_positions ORDER BY position_number");
        while ($row4 = $network_db->sql_fetchrow($result4)) {
    ...
    function NetworkBannerChange($bid, $cid, $adname, $imptotal, $impadded, $imageurl, $clickurl, $alttext, $position, $active, $ad_code, $ad_width, $ad_height, $impmade) {
        ...
        $active = intval($active);
        $bid = intval($bid);
        $network_db->sql_query("UPDATE " . $network_prefix . "_banner SET cid='$cid', name='$adname', imptotal='$imp', imageurl='$imageurl', clickurl='$clickurl', alttext='$alttext', position='$position', active='$active', ad_code='$ad_code', ad_width='$ad_width', ad_height='$ad_height' WHERE bid='$bid'");
        nuke_redirect($admin_file.".php?op=NetworkBannersAdmin");
    }
    ...
    function NetworkBannerClientDelete($cid, $ok=0) {
        ...
        $cid = intval($cid);
        if ($ok==1) {
            $network_db->sql_query("DELETE FROM " . $network_prefix . "_banner WHERE cid='$cid'");
            $network_db->sql_query("DELETE FROM " . $network_prefix . "_banner_clients WHERE cid='$cid'");
            nuke_redirect($admin_file.".php?op=NetworkBannersAdmin");
        } else {
            include_once(NUKE_BASE_DIR.'header.php');
            OpenTable();
	    ...
	    echo "<br />";
            ...
            CloseTable();
            echo "<br />";
            $row = $network_db->sql_fetchrow($network_db->sql_query("SELECT cid, name FROM " . $network_prefix . "_banner_clients WHERE cid='$cid'"));
            $cid = intval($row['cid']);
            $name = $row['name'];
            OpenTable();
            echo "<center><strong>" . _DELETECLIENT . ": $name</strong><br /><br />
                " . _SURETODELCLIENT . "<br /><br />";
            $result2 = $network_db->sql_query("SELECT imageurl, clickurl FROM " . $network_prefix . "_banner WHERE cid='$cid'");
            $numrows = $network_db->sql_numrows($result2);
            ...
            while ($row2 = $network_db->sql_fetchrow($result2)) {
    ...
    function NetworkBannerClientEdit($cid) {
        ...
        echo "<br />";
        $cid = intval($cid);
        $row = $network_db->sql_fetchrow($network_db->sql_query("SELECT name, contact, email, login, passwd, extrainfo FROM " . $network_prefix . "_banner_clients WHERE cid='$cid'"));
        $name = $row['name'];
        $contact = $row['contact'];
        $email = $row['email'];
        $login = $row['login'];
    ...
    function NetworkBannerClientChange($cid, $name, $contact, $email, $extrainfo, $login, $passwd) {
        ...
        $cid = intval($cid);
        $network_db->sql_query("UPDATE ".$network_prefix."_banner_clients SET name='$name', contact='$contact', email='$email', login='$login', passwd='$passwd', extrainfo='$extrainfo' WHERE cid='$cid'");
        nuke_redirect($admin_file.".php?op=NetworkBannersAdmin#top");
    }

    function ad_network_positions() {
        global $network_prefix, $network_db, $banners, $admin_file, $ad_admin_menu, $bgcolor1, $bgcolor2;
        ...
        $result = $network_db->sql_query("SELECT * FROM ".$network_prefix."_banner_positions ORDER BY apid");
        while ($row = $network_db->sql_fetchrow($result)) {
            $ban_num = $network_db->sql_numrows($network_db->sql_query("SELECT * FROM ".$network_prefix."_banner WHERE position='".$row['position_number']."'"));
        ...
        echo "<br />";
        OpenTable();
        $numrows = $network_db->sql_numrows($network_db->sql_query("SELECT * FROM ".$network_prefix."_banner_positions"));
        if ($numrows == 0) {
            $pos_num = 0;
        } else {
            $row = $network_db->sql_fetchrow($network_db->sql_query("SELECT position_number FROM ".$network_prefix."_banner_positions ORDER BY position_number DESC LIMIT 0,1"));
    ...
    function position_network_save($apid=0, $ad_position_number, $ad_position_name, $position_new=0) {
        ...
        if ($position_new == 1) {
            $network_db->sql_query("INSERT INTO ".$network_prefix."_banner_positions VALUES (NULL, '$ad_position_number', '$ad_position_name')");
        } else {
            $apid = intval($apid);
            $network_db->sql_query("UPDATE ".$network_prefix."_banner_positions SET position_name='$ad_position_name' WHERE apid='$apid'");
        }
        nuke_redirect($admin_file.'.php?op=ad_network_positions');
    }
    ...
    function position_network_edit($apid) {
        ...
        echo "<br />";
        OpenTable();
        $row = $network_db->sql_fetchrow($network_db->sql_query("SELECT * FROM ".$network_prefix."_banner_positions WHERE apid='$apid'"));
    ...
    function position_delete($apid, $ok=0, $active=0, $new_pos=x) {
        ...
        if ($ok == 1) {
            if ($new_pos == "x" || empty($new_post)) {
                $network_db->sql_query("DELETE FROM ".$network_prefix."_banner_positions WHERE apid='$apid'");
            } else {
                if ($active == "same") {
                    $row = $network_db->sql_fetchrow($network_db->sql_query("SELECT * FROM ".$network_prefix."_banner_positions WHERE apid='$apid'"));
                    $result = $network_db->sql_query("SELECT * FROM ".$network_prefix."_banner WHERE position='".$row['position_number']."'");
                    while($row2 = $network_db->sql_fetchrow($result)) {
                        $network_db->sql_query("UPDATE ".$network_prefix."_banner SET position='$new_pos' WHERE bid='".$row2['bid']."'");
                    }
                    $network_db->sql_freeresult($result);
                    $network_db->sql_query("DELETE FROM ".$network_prefix."_banner_positions WHERE apid='$apid'");
                    ...
                    $result = $network_db->sql_query("SELECT * FROM ".$network_prefix."_banner WHERE position='".$row['position_number']."'");
                    while($row2 = $network_db->sql_fetchrow($result)) {
                        $network_db->sql_query("UPDATE ".$network_prefix."_banner SET position='$new_pos', active='1' WHERE bid='".$row2['bid']."'");
                    }
                    $network_db->sql_freeresult($result);
                    $network_db->sql_query("DELETE FROM ".$network_prefix."_banner_positions WHERE apid='$apid'");
                    ...
                    $result = $network_db->sql_query("SELECT * FROM ".$network_prefix."_banner WHERE position='".$row['position_number']."'");
                    while($row2 = $network_db->sql_fetchrow($result)) {
                        $network_db->sql_query("UPDATE ".$network_prefix."_banner SET position='$new_pos', active='0' WHERE bid='".$row2['bid']."'");
                    }
                    $network_db->sql_freeresult($result);
                    $network_db->sql_query("DELETE FROM ".$network_prefix."_banner_positions WHERE apid='$apid'");
                } elseif ($active == "delete_all") {
                    $row = $network_db->sql_fetchrow($network_db->sql_query("SELECT * FROM ".$network_prefix."_banner_positions WHERE apid='$apid'"));
                    $network_db->sql_query("DELETE FROM ".$network_prefix."_banner WHERE position='".$row['position_number']."'");
                    $network_db->sql_query("DELETE FROM ".$network_prefix."_banner_positions WHERE apid='$apid'");
                }
            }
            nuke_redirect($admin_file.'.php?op=ad_network_positions');
	    ...
	    echo "<br />";
            ...
            $numrows = $network_db->sql_numrows($network_db->sql_query("SELECT * FROM ".$network_prefix."_banner WHERE position='".$row['position_number']."'"));
            if($numrows != 0) {
                echo ""._POSITIONHASADS."<br /><br />";
                echo "<form action=\"".$admin_file.".php\" method=\"POST\">";
                echo ""._MOVEADS.": <select name=\"new_pos\">";
                $result = $network_db->sql_query("SELECT * FROM ".$network_prefix."_banner_positions WHERE apid!='$apid'");
                while($row = $network_db->sql_fetchrow($result)) {
    ...
    function ad_network_terms($save=0, $terms_body=0, $country=0) {
        global $network_prefix, $network_db, $banners, $admin_file, $ad_admin_menu, $admlang;
        if ($save != 0) {
            $network_db->sql_query("UPDATE ".$network_prefix."_banner_terms SET terms_body='".Fix_Quotes($terms_body)."', country='$country'");
            nuke_redirect($admin_file.".php?op=ad_network_terms");
            exit;
        }
        include_once(NUKE_BASE_DIR.'header.php');
        OpenTable();
        ...
        echo "<br />";
        OpenTable();
        $row = $network_db->sql_fetchrow($network_db->sql_query("SELECT * FROM ".$network_prefix."_banner_terms"));
        ...
        $result = $network_db->sql_query("SELECT `flag_name` FROM `".$network_prefix."_bbflags` ORDER BY `flag_name`");
        while ($row2 = $network_db->sql_fetchrow($result)) {
    ...
    function ad_network_plans() {
        global $network_prefix, $network_db, $admin_file, $ad_admin_menu, $bgcolor1, $bgcolor2;
        include_once(NUKE_BASE_DIR.'header.php');
        OpenTable();
	    echo "<div align=\"center\">\n<a href=\"$admin_file.php?op=NetworkBannersAdmin\">" . _BANNERS_ADMIN_HEADER . "</a></div>\n";
        echo "<br /><br />";
        ...
        CloseTable();
        echo "<br />";
        $numrows = $network_db->sql_numrows($network_db->sql_query("SELECT * FROM ".$network_prefix."_banner_plans"));
        if ($numrows != 0) {
            OpenTable();
            $result = $network_db->sql_query("SELECT * FROM ".$network_prefix."_banner_plans");
    ...
    function ad_network_plans_add($name, $description, $delivery, $type, $price, $buy_links, $status) {
        ...
        if (!empty($name) AND !empty($description) AND !empty($delivery) AND (isset($type) AND is_numeric($type)) AND !empty($price) AND !empty($buy_links) AND !empty($status)) {
            $network_db->sql_query("INSERT INTO ".$network_prefix."_banner_plans VALUES (NULL, '$status', '$name', '$description', '$delivery', '$type', '$price', '$buy_links')");
            nuke_redirect($admin_file.'.php?op=ad_network_plans');
            exit;
        } else {
            include_once(NUKE_BASE_DIR.'header.php');
            OpenTable();
    ...
    function ad_network_plans_edit($pid) {
        ...
        echo "<br />";
        OpenTable();
        $row = $network_db->sql_fetchrow($network_db->sql_query("SELECT * FROM ".$network_prefix."_banner_plans WHERE pid='$pid'"));
    ...
    function ad_network_plans_save($pid, $name, $description, $delivery, $type, $price, $buy_links, $status) {
        ...
        if (!empty($name) AND !empty($description) AND !empty($delivery) AND (isset($type) AND is_numeric($type)) AND !empty($price) AND !empty($buy_links) AND !empty($status)) {
            $network_db->sql_query("UPDATE ".$network_prefix."_banner_plans SET active='$status', name='$name', description='$description', delivery='$delivery', delivery_type='$type', buy_links='$buy_links', price='$price' WHERE pid='$pid'");
            nuke_redirect($admin_file.'.php?op=ad_network_plans');
            exit;
        } else {
            include_once(NUKE_BASE_DIR.'header.php');
    ...
    function ad_network_plans_delete($pid, $ok=0) {
        ...
        if ($ok == 1) {
            $network_db->sql_query("DELETE FROM ".$network_prefix."_banner_plans WHERE pid='$pid'");
            nuke_redirect($admin_file.'.php?op=ad_network_plans');
            exit;
        } else {
            include_once(NUKE_BASE_DIR.'header.php');
    ...
    function ad_network_plans_status($pid, $status) {
        ...
        $pid = intval($pid);
        $network_db->sql_query("UPDATE ".$network_prefix."_banner_plans SET active='$active' WHERE pid='$pid'");
        nuke_redirect($admin_file.'.php?op=ad_network_plans');
    }

...
function is_ad_client($network_ad_client) {
    ...
    if (!empty($cid) AND !empty($pwd)) {
        list($pass) = $network_db->sql_ufetchrow("SELECT passwd FROM ".$network_prefix."_banner_clients WHERE cid='$cid'");
...
function theindex() {
    global $network_prefix, $network_db, $sitename;
...
function ad_plans() {
    ...
    OpenTable();
    $result = $network_db->sql_query("SELECT * FROM ".$network_prefix."_banner_plans WHERE active='1'");
...
function ad_client_valid($login, $pass) {
    ...
    if ($network_db->sql_numrows($result) != 1) {
        ...
        $row = $network_db->sql_fetchrow($result);
        $cid = $row['cid'];
...
function network_client_home() {
    ...
    if (!is_ad_client($network_ad_client)) {
        ...
        $network_ad_client = explode(":", $network_ad_client);
        $cid = $network_ad_client[0];
        $row = $network_db->sql_ufetchrow("SELECT * FROM ".$network_prefix."_banner_clients WHERE cid='$cid'");
        ...
        $result = $network_db->sql_query($sql, true);
        while ($row = $network_db->sql_fetchrow($result)) {
            $bid = $row['bid'];
            $bid = intval($bid);
...
function view_banner($cid, $bid) {
    ...
    if (!is_ad_client($network_ad_client)) {
        ...
        if ($cid != $ad_client_id) {
            ...
            title($sitename.' '._ADSYSTEM);
            OpenTable();
            $row = $network_db->sql_ufetchrow("SELECT * FROM ".$network_prefix."_banner WHERE bid='$bid'");
            $cid = intval($row['cid']);
...
function ad_client_report($cid, $bid) {
    ...
    if (!is_ad_client($network_ad_client)) {
        ...
        if ($cid != $ad_client_id) {
            ...
            $bid = intval($bid);
            $cid = intval($cid);
            list($name, $email) = $network_db->sql_ufetchrow("SELECT name, email FROM ".$network_prefix."_banner_clients WHERE cid='$cid'");
            ...
            if (empty($email)) {
                ...
                list($bid, $imptotal, $impmade, $clicks, $imageurl, $clickurl, $date, $ad_class) = $network_db->sql_ufetchrow("SELECT bid, name, imptotal, impmade, clicks, imageurl, clickurl, date, ad_class FROM ".$network_prefix."_banner WHERE bid='$bid' AND cid='$cid'");
                $bid = intval($bid);
                $imptotal = intval($imptotal);
                $impmade = intval($impmade);
                $clicks = intval($clicks);

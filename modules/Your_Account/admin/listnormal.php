...
if(is_mod_admin($nuke_module_name)) {
    ...
    $totalselected = $nuke_db->sql_numrows($nuke_db->sql_query("SELECT * FROM ".$nuke_user_prefix."_users $where"));
    ...
    $result = $nuke_db->sql_query("SELECT * FROM ".$nuke_user_prefix."_users $where ORDER BY username LIMIT $min,".$ya_config['perpage']."");
    while($chnginfo = $nuke_db->sql_fetchrow($result)) {
        echo "<tr bgcolor='$bgcolor1'><form method='post' action='modules.php?name=$nuke_module_name&amp;file=admin'>\n";

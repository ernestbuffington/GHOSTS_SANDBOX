...
if(is_mod_admin($nuke_module_name)) {
  ...
  amain();
  echo "<br />\n";
    $result = $nuke_db->sql_query("SELECT * FROM ".$nuke_user_prefix."_users_temp WHERE user_id='$chng_uid'");
    if($nuke_db->sql_numrows($result) > 0) {
    $chnginfo = $nuke_db->sql_fetchrow($result);
    OpenTable();
    echo "<center><table border='0'><form action='modules.php?name=$nuke_module_name&amp;file=admin' method='post'>\n";
    ...
    while ($sqlvalue = $nuke_db->sql_fetchrow($result)) {
        $t = $sqlvalue[fid];
        $result1 = $nuke_db->sql_query("SELECT * FROM ".$nuke_user_prefix."_cnbya_value_temp WHERE uid='$chng_uid' AND fid='$t'");
        while ($tmpsqlvalue = $nuke_db->sql_fetchrow($result1)) {
    ...
    echo "<tr><td colspan=\"2\" align=\"left\"><br />\n";
        ...
        echo "<td width=\"3\"></td><td>\n";
            if ($ya_config['useactivate'] == 0) {
                echo "<form action='modules.php?name=$nuke_module_name&amp;file=admin' method='post'><td>\n";
                if (isset($min)) { echo "<input type='hidden' name='min' value='$min'>\n"; }
                if (isset($xop)) { echo "<input type='hidden' name='xop' value='$xop'>\n"; }

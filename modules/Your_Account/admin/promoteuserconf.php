...
if (is_mod_admin('super')) {
    ...
    $num = $nuke_db->sql_numrows($nuke_db->sql_query("SELECT * FROM ".$prefix."_authors WHERE aid='$add_aid'"));
    if ($num > 0) {
 ...
 ******************************************************/
        ...
        $result = $nuke_db->sql_query("INSERT INTO " . $prefix . "_authors VALUES ('$add_aid', '$add_name', '$add_url', '$add_email', '$add_password', '0', '$add_radminsuper', '$add_admlanguage')");
    }
    ...
    $add_radminsuper = intval($add_radminsuper);
    $add_admlanguage = check_html($add_admlanguage, 'nohtml');
    $result = $nuke_db->sql_query("INSERT INTO " . $prefix . "_authors VALUES ('$add_aid', '$add_name', '$add_url', '$add_email', '$add_password', '0', '$add_radminarticle','$add_radmintopic','$add_radminuser','$add_radminsurvey','$add_radminlink','$add_radminfaq','$add_radmindownload','$add_radminreviews','$add_radminnewsletter','$add_radminforum','$add_radmincontent','$add_radminency','$add_radminsuper','$add_admlanguage')");
    }
    else {
    ...
    $add_radminsuper = intval($add_radminsuper);
    $add_admlanguage = check_html($add_admlanguage, 'nohtml');
    $result = $nuke_db->sql_query("INSERT INTO " . $prefix . "_authors VALUES ('$add_aid', '$add_name', '$add_url', '$add_email', '$add_password', '0', '$add_radminarticle','$add_radmintopic','$add_radminuser','$add_radminsurvey','$add_radminsection','$add_radminlink','$add_radminephem','$add_radminfaq','$add_radmindownload','$add_radminreviews','$add_radminnewsletter','$add_radminforum','$add_radmincontent','$add_radminency','$add_radminsuper','$add_admlanguage')");
        }
        /////////////////////END/////////////////////////////
        if (!$result) {
            $pagetitle = ": "._USERADMIN." - "._PROMOTEUSER;
        ...
        if ($add_radminforum == "1") { $nuke_db->sql_query("UPDATE ".$nuke_user_prefix."_users SET user_level='2' WHERE user_id='$chng_uid'"); }
    }

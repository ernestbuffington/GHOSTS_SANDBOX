    ...
    while (false !== ($entry = $d->read())) {
        if( preg_match('/(\.gif$|\.png$|\.jpg|\.jpeg)$/is', $entry) ) {
            if( $entry != '.' && $entry != '..' && !is_file($d . '/' . $entry) && !is_link($d . '/' . $entry) ) {
                ...
                //avatarfix by menelaos dot hetnet dot nl
                echo "<a href=\"modules.php?name=$nuke_module_name&amp;op=avatarsave&amp;category=$avatarcategory&amp;avatar=$entry\"><img src=\"$direktori/$avatarcategory/$entry\" border=\"0\" alt=\"$entryname\" title=\"$entryname\" hspace=\"10\" vspace=\"10\"></a>";
            }
            if ($temcount == 10) {
                echo "<br />";
                $temcount -= 10;

...
if (!defined('CNBYA')) {
    ...
    // Last 10 Comments
    if ($articlecomm == 1) {
        $result6 = $nuke_db->sql_query("SELECT tid, sid, subject FROM ".$prefix."_comments WHERE name='$usrinfo[username]' ORDER BY tid DESC LIMIT 0,10");
        if (($nuke_db->sql_numrows($result6) > 0)) {
            echo "<br />";
            OpenTable();
/*****[BEGIN]******************************************
 ...
 ******************************************************/
    ...
    // Last 10 Submissions
    $result7 = $nuke_db->sql_query("SELECT sid, title FROM ".$prefix."_stories WHERE informant='$usrinfo[username]' ORDER BY sid DESC LIMIT 0,10");

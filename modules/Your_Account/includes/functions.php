...
function Show_CNBYA_menu(){
    global $stop, $nuke_module_name, $nuke_redirect, $mode, $t, $f, $ya_config;
    OpenTable();
    if ($stop) {
    ...
    echo "[ <a href=\"modules.php?name=$nuke_module_name\">"._LOGIN."</a> \n";
    echo "| <a href=\"modules.php?name=$nuke_module_name&amp;op=new_user\">"._REGNEWUSER."</a> ]\n";
    echo "</td><td align=\"right\"><span class=\"content\">\n";
    echo "[ <span class=\"content\"><a href=\"modules.php?name=$nuke_module_name&amp;op=pass_lost\">"._PASSWORDLOST."</a> \n";
    echo "| <a href=\"modules.php?name=$nuke_module_name&amp;op=ShowCookiesRedirect\">"._YA_COOKIEDELALL."</a> ]</span>\n";
    echo "</td></tr></table>\n";
    CloseTable();
}
...
function yacookie($setuid, $setusername, $setpass, $setstorynum, $setumode, $setuorder, $setthold, $setnoscore, $setublockon, $settheme, $setcommentmax) {
    global $ya_config, $nuke_db, $prefix, $identify;
    $ip = $identify->get_ip();
    $result = $nuke_db->sql_query("SELECT time FROM ".$prefix."_session WHERE uname='$setusername'");
    $ctime = time();
    ...
    if (!empty($setusername)) {
        ...
        if ($row = $nuke_db->sql_fetchrow($result)) {
            $nuke_db->sql_query("UPDATE ".$prefix."_session SET uname='$setusername', time='$ctime', host_addr='$ip', guest='$guest' WHERE uname='$uname'");
        } else {
            $nuke_db->sql_query("INSERT INTO ".$prefix."_session (uname, time, host_addr, guest) VALUES ('$uname', '$ctime', '$ip', '$guest')");
        }
    }
...
function amain() {
    global $ya_config, $nuke_module_name, $nuke_db, $nuke_user_prefix, $bgcolor2, $bgcolor1, $textcolor1, $find, $what, $match, $query, $admin_file;
    $cnbyaversion = $ya_config['version'];

    OpenTable();
    $act = $nuke_db->sql_numrows($nuke_db->sql_query("SELECT * FROM ".$nuke_user_prefix."_users WHERE user_level>'0' AND user_id>'1'"));
    $sus = $nuke_db->sql_numrows($nuke_db->sql_query("SELECT * FROM ".$nuke_user_prefix."_users WHERE user_level='0' AND user_id>'1'"));
    $del = $nuke_db->sql_numrows($nuke_db->sql_query("SELECT * FROM ".$nuke_user_prefix."_users WHERE user_level='-1' AND user_id>'1'"));
    $nor = $nuke_db->sql_numrows($nuke_db->sql_query("SELECT * FROM ".$nuke_user_prefix."_users WHERE user_id>'1'"));
    $pen = $nuke_db->sql_numrows($nuke_db->sql_query("SELECT * FROM ".$nuke_user_prefix."_users_temp"));
    ...
    echo "</tr>\n";
    echo "<tr>\n";
    echo "<td align='center' width='33%'><a href='modules.php?name=$nuke_module_name&amp;file=admin&amp;op=addUser'>"._ADDUSER."</a></td>\n";
    echo "<td align='center' width='33%'><a href='modules.php?name=$nuke_module_name&amp;file=admin&amp;op=UsersConfig'>"._USERSCONFIG."</a></td>\n";
    echo "<td align='right'><a href='modules.php?name=$nuke_module_name&amp;file=admin&amp;op=listnormal&amp;query=1'>"._ACTIVEUSERS.":</a>";
    echo "<td align='left'>($act)</td>\n";
    echo "</tr>\n";
    echo "<tr>";
    echo "<td align='center' width='33%'><a href='modules.php?name=$nuke_module_name&amp;file=admin&amp;op=addField'>"._YA_ADDFIELD."</a></td>\n";
    echo "<td align='center' width='33%'><a href='modules.php?name=$nuke_module_name&amp;file=admin&amp;op=CookieConfig'>"._COOKIECONFIG."</a></td>\n";
    echo "<td align='right'><a href='modules.php?name=$nuke_module_name&amp;file=admin&amp;op=listnormal&amp;query=a'>"._NORMALUSERS.":</a>";
    echo "<td align='left'>($nor)</td>\n";
    echo "</tr>\n";
    echo "<tr>\n";
    echo "<td align='center' width='33%'><a href='modules.php?name=$nuke_module_name&amp;file=admin&amp;op=searchUser'>"._SEARCHUSERS."</a></td>\n";
    echo "<td align='center' width='33%'><a href='modules.php?name=$nuke_module_name&amp;file=admin&amp;op=editTOS'>"._EDITTOS."</a></td>\n";
    echo "<td align='right'><a href='modules.php?name=$nuke_module_name&amp;file=admin&amp;op=listnormal&amp;query=-1'>"._DELETEUSERS.":</a>";
    echo "<td align='left'>($del)</td>\n";
    echo "</tr>\n";
    echo "<tr>\n";
    echo "<td align='center' width='33%'>&nbsp;</td>\n";
    echo "<td align='center' width='33%'>&nbsp;</td>\n";
    echo "<td align='right'><a href='modules.php?name=$nuke_module_name&amp;file=admin&amp;op=listnormal&amp;query=0'>"._SUSPENDUSERS.":</a>";
    echo "<td align='left'>($sus)</td>\n";
    echo "</tr>\n";
    echo "<tr>\n";
//  echo "<td align='center' width='33%'><a href='modules.php?name=$nuke_module_name&amp;file=admin&amp;op=autoSuspend'>"._YA_CHKAUTOSUS."</a></td>";
    echo "<td align='center' width='33%'><a href='modules.php?name=$nuke_module_name&amp;file=admin&amp;op=credits'>"._CREDITS."</a></td>\n";
    echo "<td align='center' width='33%'>[CNB Your Account $cnbyaversion]</td>\n";
    echo "<td align='right'><a href='modules.php?name=$nuke_module_name&amp;file=admin&amp;op=listpending'>"._WAITINGUSERS.":</a>";
...
function asearch() {
    global $nuke_module_name, $bgcolor2, $bgcolor1, $textcolor1, $find, $what, $match, $query, $nuke_db, $nuke_user_prefix;
    OpenTable();
    echo "<table style='margin:auto' cellpadding='2' cellspacing='2' border='0'>\n";
    echo "<form method='post' action='modules.php?name=$nuke_module_name&amp;file=admin'>\n";
    ...
    $result = $nuke_db->sql_query("DESCRIBE " . $nuke_user_prefix . "_users");
...
function mmain($nuke_user) {
    global $stop, $nuke_module_name, $nuke_redirect, $mode, $t, $f, $ya_config, $nuke_user, $p;
    if(!is_user()) {
        ...
        global $cookie;
        nuke_redirect("modules.php?name=$nuke_module_name&op=userinfo&username=$cookie[1]");
    }
}
...
function yapagenums($op, $totalselected, $perpage, $max, $find, $what, $match, $query) {
    ...
    if ($pages != 1 && $pages != 0) {
        ...
        echo "<tr><form action='modules.php?name=$nuke_module_name&amp;file=admin' method='post'>\n";

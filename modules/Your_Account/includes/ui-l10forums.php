...
if (!defined('CNBYA')) {
    ...
    $result8 = $nuke_db->sql_query("SELECT t.topic_id, t.topic_title, f.forum_name, t.forum_id FROM ".$prefix."_bbtopics t, ".$prefix."_bbforums f WHERE t.forum_id=f.forum_id AND t.topic_poster='$usrinfo[user_id]' AND auth_view<'2' AND auth_read<'2' AND auth_post<'2' order by t.topic_time DESC LIMIT 0,10");
...
/*****[END]********************************************
 [ Mod:    Advanced Username Color             v1.0.5 ]
 ******************************************************/
        while(list($topic_id, $topic_title, $forum_name, $forum_id) = $nuke_db->sql_fetchrow($result8)) {
...
/*****[END]********************************************
 [ Mod:    Advanced Username Color             v1.0.5 ]
 ******************************************************/
        while(list($post_id, $post_subject, $forum_name, $forum_id) = $nuke_db->sql_fetchrow($result12)) {

...
#################################################[ SandBox Example Source Code ]###########################################################################################
                                                                            ...
                                                                            ######################################################################################################
                                                                            # $nuke_module_name is defined in the module index.php file and can be found in modules/SandBox/index.php #
# set the current file name!                                                ########################################################## ###########################################
$current_file_name = basename(__FILE__, '.php');                                                                                     # #
                                                                                                                                     # #
$nuke_page_header  = '<div align="center"><h2>Loading JQuery and Bootstrap into the current module for editing and testing...</h2></div>';# #
$nuke_page_header .= '<div align="center"><h1>If you are logged in via FTP the current file location is: /public_html/modules/'.$nuke_module_name.'/'.$current_file_name.'.php<h1></div>';
$nuke_page_header .= '<div align="center">It is always a great idea to leave yourself notes like this, it will help you find the file you need!</div>';

$nuke_page_header .= "<a class=\"sourceview\" href=\"#\" onClick = \"tag=this.nextElementSibling; tag.style.display=tag.style.display == 'none' ? 'block':'none'\"><strong>Click here to show | hide source code</strong></a>";
$nuke_page_header .= "<div id=\"php_source\" style=\"display:none\">";

print_r($nuke_page_header);

OpenTableCode();
highlight_file("modules/Titanium_SandBox/x-bootstrap_4_modal.php");
CloseTableCode();

$nuke_page_header2 = "</div><hr />";
#################################################[ SandBox Example Source Code ]###########################################################################################
# TEST CODE GOES HERE - START
###########################################################################################################################################################################

# Load the header tags so the browser stays out of quirk mode OPEN TAG
$nuke_page_header2 .= "<head>";

# load the style sheet for this template
$nuke_page_header2 .= "<link rel=\"StyleSheet\" href=\"modules/Titanium_SandBox/css/x-bootstrap_4_modal.css\" type=\"text/css\">\n\n\n";
...
$nuke_page_header2 .= "<script type=\"text/javascript\" language=\"javascript\" src=\"".HTTPS."dev_includes/jquery/jquery.js\"></script>\n";
...
$nuke_page_header2 .= "<script type=\"text/javascript\" language=\"javascript\" src=\"".HTTPS."dev_includes/bootstrap/js/bootstrap.min.js\"></script>\n";

# Load the header tags so the browser stays out of quirk mode CLOSE TAG
$nuke_page_header2 .= "</head>";

# Check to see if Jquery is working START
$nuke_page_header2 .= "<script type=\"text/javascript\" language=\"javascript\">$(function() { alert('JQuery is loaded and working fine!') })</script>\n";
# Check to see if Jquery is working END

print_r($nuke_page_header2);

...
OpenTableCode();
highlight_file("modules/Titanium_SandBox/x-host_information.php");
CloseTableCode();
print "</div><hr />";
#################################################[ SandBox Example Source Code ]###########################################################################################
...
echo"<span class=\"html\">"
  . "1. All elements of the <b>\$_SERVER</b> array whose keys begin with <b>'HTTP_'</b> come from HTTP request headers and are not to be trusted.<br><br>"
  . "2. All HTTP headers sent to the script are made available through the <b>\$_SERVER</b> array, with names prefixed by <b>'HTTP_'</b>.<br><br>"
  . "3. <b>\$_SERVER['PHP_SELF']</b> is dangerous if misused. If login.php/nearly_arbitrary_string is requested, <b>\$_SERVER['PHP_SELF']</b> will contain<br>" 
  . "not just login.php, but the entire login.php/nearly_arbitrary_string. If you've printed <b>\$_SERVER['PHP_SELF']</b> as the value of the action attribute<br>" 
  . "of your form tag without performing HTML encoding, an attacker can perform <b>XSS</b> attacks by offering users a link to your site such as this:<br>"
  . "<br><br>&lt;a href='<a href=\"http://www.example.com/login.php/\" rel=\"nofollow\" target=\"_blank\">http://www.example.com/login.php/</a><br>"
  . "	 \"&gt;&lt;script type=\"text/javascript\"&gt;...&lt;/script&gt;&lt;span a=\"'&gt;Example.com&lt;/a&gt;<br><br>The javascript block would define an event handler function and <br>"
  . "	 bind it to the form's submit event. This event handler would load via an &lt;img> tag an external file, with the submitted username and password as parameters.<br>"

 ."<br><br>";
 
 echo "Use <b>\$_SERVER['SCRIPT_NAME']</b> instead of <b>\$_SERVER['PHP_SELF']</b>. HTML encode every string sent to the browser that should not be interpreted <br />";
 echo "as HTML, unless you are absolutely certain that it cannot contain anything that the browser can interpret as HTML.";
...
echo "Caveat: This is before URL rewrites (i.e. it's as per the original call URL). *<br>";
echo "*: I've seen at least one situation where this is not true (there was another <b>\$_SERVER</b> variable to use instead supplied by the URL rewriter),<br />"; 
echo "but the author of the URL rewriter later fixed it so probably fair to dismiss this particular note.<br />";
echo "Caveat: Not set on all PHP environments, may need setting via <b>\$_SERVER['REDIRECT_URL']</b> . '?' . http_build_query(\$_GET) [if <b>\$_SERVER['REDIRECT_URL']</b> <br />";
echo "is set, and imperfect as we don't know what GET parameters were originally passed vs which were injected in the URL rewrite] <br />";
echo "--otherwise-- <b>\$_SERVER['PHP_SELF']</b> . '?' . http_build_query($_GET).<br>";
echo "Works on web mode: Yes<br>";
echo "Works on CLI mode: No<br>";

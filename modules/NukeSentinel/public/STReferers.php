...
$totalselected = $nuke_db->sql_numrows($nuke_db->sql_query("SELECT `rid` FROM `".$prefix."_nsnst_referers`"));
if($totalselected > 0) {
  ...
  echo '</tr>'."\n";
  $result = $nuke_db->sql_query("SELECT * FROM `".$prefix."_nsnst_referers` ORDER BY `referer` $direction LIMIT $min,$perpage");

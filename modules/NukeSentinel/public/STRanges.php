...
$totalselected = $nuke_db->sql_numrows($nuke_db->sql_query("SELECT `reason` FROM `".$prefix."_nsnst_blocked_ranges`"));
if ($totalselected > 0) {
  // Page Sorting
  echo '<table summary="" align="center" border="0" cellpadding="2" cellspacing="2" width="100%">'."\n";
  echo '<tr>'."\n";
  echo '<td align="right" colspan="3">'."\n";
  echo '<form method="post" action="modules.php?name='.$nuke_module_name.'" style="padding: 0px; margin: 0px;">'."\n";
  ...
  echo '</tr>'."\n";
  $result = $nuke_db->sql_query("SELECT * FROM `".$prefix."_nsnst_blocked_ranges` ORDER BY $column $direction LIMIT $min,$perpage");

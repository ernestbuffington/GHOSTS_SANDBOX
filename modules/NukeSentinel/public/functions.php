...
function stmain_menu($subtitle = "") {
  global $nuke_db, $prefix, $nuke_module_name;
  ...
  echo '<tr><td><a href="modules.php?name='.$nuke_module_name.'&amp;op=STIPS">'._AB_BLOCKEDIPS.'</a></td></tr>'."\n";
  echo '<tr><td><a href="modules.php?name='.$nuke_module_name.'&amp;op=STRanges">'._AB_BLOCKEDRANGES.'</a></td></tr>'."\n";
  echo '<tr><td><a href="modules.php?name='.$nuke_module_name.'&amp;op=STReferers">'._AB_BLOCKEDREFERERS.'</a></td></tr>'."\n";
  if($checkrow > 0) { echo '<tr><td><a href="modules.php?name='.$nuke_module_name.'&amp;op=STIP2C">'._AB_IP2COUNTRY.'</a></td></tr>'."\n"; }
  echo '</table>'."\n";
  CloseTable();
}
...
function stpagenumspub($op, $totalselected, $perpage, $max, $column, $direction) {
  ...
  if($pages != 1 && $pages != 0) {
    ...
    echo '<tr>'."\n";
    echo '<td width="33%">'."\n";
    echo '<form action="modules.php?name='.$nuke_module_name.'&amp;op='.$op.'" method="post" style="padding: 0px; margin: 0px;">'."\n";

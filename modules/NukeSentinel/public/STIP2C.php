...
if ($totalselected > 0) {
  ...
  echo '<form action="modules.php?name='.$nuke_module_name.'" method="post" style="padding: 0px; margin: 0px;">'."\n";
  ...
  echo '</tr>'."\n";
  $result = $nuke_db->sql_query("SELECT * FROM `".$prefix."_nsnst_ip2country` ORDER BY $column $direction LIMIT $min,$perpage");
  while ($getIPs = $nuke_db->sql_fetchrow($result)) {
    $getIPs['ip_lo_ip'] = long2ip($getIPs['ip_lo']);
    $getIPs['ip_hi_ip'] = long2ip($getIPs['ip_hi']);
    list($getIPs['country']) = $nuke_db->sql_fetchrow($nuke_db->sql_query("SELECT `country` FROM `".$prefix."_nsnst_countries` WHERE `c2c`='".$getIPs['c2c']."' LIMIT 0,1"));

...
$totalselected = $nuke_db->sql_numrows($nuke_db->sql_query("SELECT `reason` FROM `".$prefix."_nsnst_blocked_ips`"));
if ($totalselected > 0) {
  // Page Sorting
  ...
  while ($getIPs = $nuke_db->sql_fetchrow($result)) {
    ...
    if((is_admin() AND $ab_config['display_reason']==1) OR ((is_user() OR is_admin()) AND $ab_config['display_reason']==2) OR $ab_config['display_reason']==3) {
      $result2 = $nuke_db->sql_query("SELECT `reason` FROM `".$prefix."_nsnst_blockers` WHERE `blocker`='".$getIPs['reason']."' LIMIT 0,1");
      list($reason) = $nuke_db->sql_fetchrow($result2);

...
function mcp_post_details($id, $mode, $action)
{
	...
	$url = append_sid("modules.php?name=phpBB3&amp;file=mcp?" . phpbb_extra_url());
		...
		case 'whois':
			...
			if ($auth->acl_get('m_info', $post_info['forum_id']))
			{
				...
				$template->assign_vars(array(
					'RETURN_POST'	=> sprintf($user->lang['RETURN_POST'], '<a href="' . append_sid("modules.php?name=phpBB3&amp;file=mcp", "i=$id&amp;mode=$mode&amp;p=$post_id") . '">', '</a>'),
					'U_RETURN_POST'	=> append_sid("modules.php?name=phpBB3&amp;file=mcp", "i=$id&amp;mode=$mode&amp;p=$post_id"),
	...
	$mcp_post_template_data = array(
		...
		'U_APPROVE_ACTION'		=> append_sid("modules.php?name=phpBB3&amp;file=mcp", "i=queue&amp;p=$post_id"),
		...
		'U_EDIT'				=> ($auth->acl_get('m_edit', $post_info['forum_id'])) ? append_sid("modules.php?name=phpBB3&amp;file=posting", "mode=edit&amp;p={$post_info['post_id']}") : '',
		'U_FIND_USERNAME'		=> append_sid("modules.php?name=phpBB3&amp;file=memberlist", 'mode=searchuser&amp;form=mcp_chgposter&amp;field=username&amp;select_single=true'),
		'U_MCP_APPROVE'			=> append_sid("modules.php?name=phpBB3&amp;file=mcp", 'i=queue&amp;mode=approve_details&amp;p=' . $post_id),
		'U_MCP_REPORT'			=> append_sid("modules.php?name=phpBB3&amp;file=mcp", 'i=reports&amp;mode=report_details&amp;p=' . $post_id),
		'U_MCP_USER_NOTES'		=> append_sid("modules.php?name=phpBB3&amp;file=mcp", 'i=notes&amp;mode=user_notes&amp;u=' . $post_info['user_id']),
		'U_MCP_WARN_USER'		=> ($auth->acl_get('m_warn')) ? append_sid("modules.php?name=phpBB3&amp;file=mcp", 'i=warn&amp;mode=warn_user&amp;u=' . $post_info['user_id']) : '',
		'U_VIEW_POST'			=> append_sid("modules.php?name=phpBB3&amp;file=viewtopic", 'p=' . $post_info['post_id'] . '#p' . $post_info['post_id']),
		'U_VIEW_TOPIC'			=> append_sid("modules.php?name=phpBB3&amp;file=viewtopic", 't=' . $post_info['topic_id']),

		'MINI_POST_IMG'			=> ($post_unread) ? $user->img('icon_post_target_unread', 'UNREAD_POST') : $user->img('icon_post_target', 'POST'),

		'RETURN_TOPIC'			=> sprintf($user->lang['RETURN_TOPIC'], '<a href="' . append_sid("modules.php?name=phpBB3&amp;file=viewtopic", "p=$post_id") . "#p$post_id\">", '</a>'),
		'RETURN_FORUM'			=> sprintf($user->lang['RETURN_FORUM'], '<a href="' . append_sid("modules.php?name=phpBB3&amp;file=viewforum", "f={$post_info['forum_id']}&amp;start={$start}") . '">', '</a>'),
		...
		'U_WHOIS'				=> ($auth->acl_get('m_info', $post_info['forum_id'])) ? append_sid("modules.php?name=phpBB3&amp;file=mcp", "i=$id&amp;mode=$mode&amp;action=whois&amp;p=$post_id&amp;ip={$post_info['poster_ip']}") : '',
	);
	...
	if ($auth->acl_get('m_info', $post_info['forum_id']))
	{
		...
		if (count($users_ary))
		{
			...
			foreach ($users_ary as $user_id => $user_row)
			{
				$template->assign_block_vars('userrow', array(
					...
					'U_SEARCHPOSTS' => append_sid("modules.php?name=phpBB3&amp;file=search", 'author_id=' . $user_id . '&amp;sr=topics'))
				);
			}
		}
		...
		while ($row = $db->sql_fetchrow($result))
		{
			...
			$template->assign_block_vars('iprow', array(
				...
				'U_WHOIS'		=> append_sid("modules.php?name=phpBB3&amp;file=mcp", "i=$id&amp;mode=$mode&amp;action=whois&amp;p=$post_id&amp;ip={$row['poster_ip']}"))
			);
		}
		$db->sql_freeresult($result);

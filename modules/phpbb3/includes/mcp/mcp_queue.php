	...
	public function main($id, $mode)
	{
		...
		switch ($mode)
		{
			case 'approve_details':
				...
				$post_url = append_sid("modules.php?name=phpBB3&amp;file=viewtopic", 'p=' . $post_info['post_id'] . '#p' . $post_info['post_id']);
				$topic_url = append_sid("modules.php?name=phpBB3&amp;file=viewtopic", 't=' . $post_info['topic_id']);
				...
				$post_data = array(
					...
					'RETURN_QUEUE'			=> sprintf($user->lang['RETURN_QUEUE'], '<a href="' . append_sid("modules.php?name=phpBB3&amp;file=mcp", 'i=queue' . (($topic_id) ? '&amp;mode=unapproved_topics' : '&amp;mode=unapproved_posts')) . '&amp;start=' . $start . '">', '</a>'),
					...
					'U_LOOKUP_IP'			=> ($auth->acl_get('m_info', $post_info['forum_id'])) ? append_sid("modules.php?name=phpBB3&amp;file=mcp", 'i=queue&amp;mode=approve_details&amp;p=' . $post_id . '&amp;lookup=' . $post_info['poster_ip']) . '#ip' : '',
				);
			...
			case 'deleted_posts':
				...
				foreach ($rowset as $row)
				{
					...
					$post_row = array(
						'U_TOPIC'			=> append_sid("modules.php?name=phpBB3&amp;file=viewtopic", 't=' . $row['topic_id']),
						'U_VIEWFORUM'		=> append_sid("modules.php?name=phpBB3&amp;file=viewforum", 'f=' . $row['forum_id']),
						'U_VIEWPOST'		=> append_sid("modules.php?name=phpBB3&amp;file=viewtopic", 'p=' . $row['post_id']) . (($mode == 'unapproved_posts') ? '#p' . $row['post_id'] : ''),
						'U_VIEW_DETAILS'	=> append_sid("modules.php?name=phpBB3&amp;file=mcp", "i=queue&amp;start=$start&amp;mode=approve_details&amp;p={$row['post_id']}" . (($mode == 'unapproved_topics') ? "&amp;t={$row['topic_id']}" : '')),
	...
	static public function approve_posts($action, $post_id_list, $id, $mode)
	{
		...
		if (confirm_box(true))
		{
			...
			foreach ($post_info as $post_id => $post_data)
			{
				...
				$post_url = append_sid("modules.php?name=phpBB3&amp;file=viewtopic", "p={$post_data['post_id']}") . '#p' . $post_data['post_id'];

...
function mcp_forum_view($id, $mode, $action, $forum_info)
{
	...
	$url = append_sid("modules.php?name=phpBB3&amp;file=mcp?$url_extra");
	...
	*/
	...
	$template->assign_vars(array(
		...
		'U_VIEW_FORUM'			=> append_sid("modules.php?name=phpBB3&amp;file=viewforum", 'f=' . $forum_id),
		'U_VIEW_FORUM_LOGS'		=> ($auth->acl_gets('a_', 'm_', $forum_id) && $module->loaded('logs')) ? append_sid("modules.php?name=phpBB3&amp;file=mcp", 'i=logs&amp;mode=forum_logs&amp;f=' . $forum_id) : '',
	...
	foreach ($topic_list as $topic_id)
	{
		...
		if ($row_ary['topic_status'] == ITEM_MOVED)
		{
			$topic_row = array_merge($topic_row, array(
				'U_VIEW_TOPIC'		=> append_sid("modules.php?name=phpBB3&amp;file=viewtopic", "t={$row_ary['topic_moved_id']}"),
				'U_DELETE_TOPIC'	=> ($auth->acl_get('m_delete', $forum_id)) ? append_sid("modules.php?name=phpBB3&amp;file=mcp", "i=$id&amp;f=$forum_id&amp;topic_id_list[]={$row_ary['topic_id']}&amp;mode=forum_view&amp;action=delete_topic") : '',
				'S_MOVED_TOPIC'		=> true,
		...
		else
		{
			...
			$topic_row = array_merge($topic_row, array(
				'U_VIEW_TOPIC'		=> append_sid("modules.php?name=phpBB3&amp;file=mcp", "i=$id&amp;f=$forum_id&amp;t={$row_ary['topic_id']}&amp;mode=topic_view"),

				'S_SELECT_TOPIC'	=> ($merge_select && !in_array($row_ary['topic_id'], $source_topic_ids)) ? true : false,
				'U_SELECT_TOPIC'	=> $u_select_topic,
				'U_MCP_QUEUE'		=> $u_mcp_queue,
				'U_MCP_REPORT'		=> ($auth->acl_get('m_report', $forum_id)) ? append_sid("modules.php?name=phpBB3&amp;file=mcp", 'i=main&amp;mode=topic_view&amp;t=' . $row_ary['topic_id'] . '&amp;action=reports') : '',
...
function merge_topics($forum_id, $topic_ids, $to_topic_id)
{
	...
	$redirect = $request->variable('redirect', "modules.php?name=phpBB3&amp;file=mcp?f=$forum_id&amp;i=main&amp;mode=forum_view");
	...
	if (confirm_box(true))
	{
		...
		$return_link .= (($return_link) ? '<br /><br />' : '') . sprintf($user->lang['RETURN_NEW_TOPIC'], '<a href="' . append_sid("modules.php?name=phpBB3&amp;file=viewtopic", 't=' . $to_topic_id) . '">', '</a>');
		$redirect = $request->variable('redirect', "modules.php?name=phpBB3&amp;file=viewtopic?t=$to_topic_id");

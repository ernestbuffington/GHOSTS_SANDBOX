...
global $prefix, $nuke_db, $textcolor1;
...
echo '<br />';
$result = $nuke_db->sql_query("SELECT sid, title, counter FROM ".$prefix."_stories $queryalang ORDER BY counter DESC LIMIT 0,$top");
...
##############################################################################################################################################################################
$result2 = $nuke_db->sql_query("SELECT sid, title, ratings FROM ".$prefix."_stories $querya1lang score!='0' ORDER BY ratings DESC LIMIT 0,$top");
if ($nuke_db->sql_numrows($result2) > 0) 
{
...
##############################################################################################################################################################################
$result3 = $nuke_db->sql_query("SELECT sid, title, score, ratings FROM ".$prefix."_stories $querya1lang score!='0' ORDER BY ratings+score DESC LIMIT 0,$top");
if ($nuke_db->sql_numrows($result3) > 0) 
{
...
if ($articlecomm == 1) 
{
    $result4 = $nuke_db->sql_query("SELECT sid, title, comments FROM ".$prefix."_stories $queryalang ORDER BY comments DESC LIMIT 0,$top");

    if ($nuke_db->sql_numrows($result4) > 0) 
	{
...
# Top 10 categories 
##############################################################################################################################################################################
$result5 = $nuke_db->sql_query("SELECT catid, title, counter FROM ".$prefix."_stories_cat ORDER BY counter DESC LIMIT 0,$top");

if ($nuke_db->sql_numrows($result5) > 0) 
{
...
##############################################################################################################################################################################
$result7 = $nuke_db->sql_query("SELECT username, counter FROM ".$nuke_user_prefix."_users WHERE counter > '0' ORDER BY counter DESC LIMIT 0,$top");
if ($nuke_db->sql_numrows($result7) > 0) 
{
    echo '<fieldset style="border-color: '.$fieldset_color.'; border-width: '.$fieldset_border_width.'; border-style: solid;">';
...
# Top 10 Polls 
##############################################################################################################################################################################
//$result8 = $nuke_db->sql_query("select * from ".$prefix."_poll_desc $queryplang");
//if ($nuke_db->sql_numrows($result8)>0) 
//{
//
//    echo '<fieldset style="border-color: '.$fieldset_color.'; border-width: '.$fieldset_border_width.'; border-style: solid;">';
...
//	$result9 = $nuke_db->sql_query("SELECT pollID, pollTitle, timeStamp, voters FROM ".$prefix."_poll_desc $queryplang order by voters DESC limit 0,$top");
//    $counter = 0;
...
//		for($i = 0; $i < 12; $i++) 
//		{
//            $result10 = $nuke_db->sql_query("SELECT optionCount FROM ".$prefix."_poll_data WHERE (pollID='$id') AND (voteID='$i')");
//            $row10 = $nuke_db->sql_fetchrow($result10);
...
# Top 10 authors 
##############################################################################################################################################################################
$result11 = $nuke_db->sql_query("SELECT aid, counter FROM ".$prefix."_authors ORDER BY counter DESC LIMIT 0,$top");

if ($nuke_db->sql_numrows($result11) > 0) 
{
    echo '<fieldset style="border-color: '.$fieldset_color.'; border-width: '.$fieldset_border_width.'; border-style: solid;">';
    echo '<legend align="center" id="Legend5" runat="server" visible="true" style="width:auto; margin-bottom: 0px; font-weight: bold;"><font 
	color="'.$textcolor1.'">'.$top.' '._MOST_ACTIVE_BLOG_AUTHORS.'</font></strong></legend>';
...
/* Top 10 reviews */
//$result12 = $nuke_db->sql_query("SELECT id, title, hits FROM ".$prefix."_reviews $queryrlang ORDER BY hits DESC LIMIT 0,$top");
//if ($nuke_db->sql_numrows($result12) > 0) {
...
/* Top 10 downloads */
//$result13 = $nuke_db->sql_query("SELECT lid, cid, title, hits FROM ".$prefix."_nsngd_downloads ORDER BY hits DESC LIMIT 0,$top");
//if ($nuke_db->sql_numrows($result13) > 0) {
//    echo "<div style=\"padding: 10px;\"><span class=\"option\"><strong>$top "._DOWNLOADEDFILES."</strong></span><ol>\n";
//    while ($row13 = $nuke_db->sql_fetchrow($result13)) {
//        $lid = intval($row13['lid']);
//        $cid = intval($row13['cid']);
//        $title = stripslashes(check_html($row13['title'], "nohtml"));
//        $hits = intval($row13['hits']);
//        if($hits>0) {
//            $row_res = $nuke_db->sql_fetchrow($nuke_db->sql_query("SELECT title FROM ".$prefix."_nsngd_categories WHERE cid='$cid'"));
...
//    echo "</ol></div>\n\n";
//}
//$nuke_db->sql_freeresult($result13);

/* Top 10 Pages in Content */
//$result14 = $nuke_db->sql_query("SELECT pid, title, counter FROM ".$prefix."_pages WHERE active='1' ORDER BY counter DESC LIMIT 0,$top");
//if ($nuke_db->sql_numrows($result14) > 0) {
...
//    echo "</ol></div>\n\n";
//}
//$nuke_db->sql_freeresult($result14);

...
function edit_event($eid) {
    global $prefix, $nuke_db, $admin_file;
		...
		$result = $nuke_db->sql_query("SELECT eid, month, day, year, reoccur, time, ampm, title FROM ".$prefix."_ecalendar WHERE eid = '$eid'");
		list($eid, $month, $day, $year, $reoccur, $time, $ampm, $title) = $nuke_db->sql_fetchrow($result);
		addCSSToHead('./modules/ECalendar/css/astyle.css','file');
		$hour = '';
		$min = '';
		if (!empty($time)){ 
...
function events_display() {
	global $prefix, $nuke_db, $admin_file;
	$count_events = $nuke_db->sql_numrows($nuke_db->sql_query("SELECT * FROM ".$prefix."_ecalendar"));
	...
	$limit2 = $pagination->getRange2();
        ...
        $result = $nuke_db->sql_query("SELECT eid, month, day, year, reoccur, time, ampm, title FROM ".$prefix."_ecalendar ORDER by `eid` ASC  LIMIT ".$limit1.", ".$limit2);
        ...
        if ($nuke_db->sql_numrows($result) > 0) {
            while (list($eid, $month, $day, $year, $reoccur, $time, $ampm, $title) = $nuke_db->sql_fetchrow($result)) {
				$x++;
				...
				if (($today > $date) && ($reoccur == 0)){
					$result = $nuke_db->sql_query("DELETE FROM `".$prefix."_ecalendar` WHERE eid = '$eid'");
				}
            }
			echo '</table>' , PHP_EOL;
			echo '<br />' , PHP_EOL;
...
function remove_event($eid){
	...
	$eid = intval($eid);
    $result = $nuke_db->sql_query("DELETE FROM `".$prefix."_ecalendar` WHERE eid = '$eid'");
	nuke_redirect($admin_file.".php?op=ecalendar");
}
...
function add_event() {
	...
	$result = $nuke_db->sql_query("INSERT INTO `".$prefix."_ecalendar`(`month`, `day`, `year`, `reoccur`, `time`, `ampm`, `title`) VALUES ('".$month."', '".$day."', '".$year."', '".$reoccur."', '".$time."', '".$ampm."', '".$title."')");
	include_once(NUKE_BASE_DIR.'header.php');
		if(!$result) {
...
function update_event($eid) {
	...
	$ampm = $_POST['ampm'];
	$title = htmlentities($_POST['title'], ENT_QUOTES);
 $nuke_db->sql_query("UPDATE `".$prefix."_ecalendar` SET `day` = '".$day."', `month` = '".$month."', `year` = '".$year."', `reoccur` = '".$reoccur."', `time` = '".$time."', `ampm` = '".$ampm."', `title` = '".$title."' WHERE eid = '$eid'");
	nuke_redirect($admin_file.".php?op=ecalendar");
}
/*===================================================

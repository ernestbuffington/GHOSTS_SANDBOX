    ...
    while ( $current_day <= $num_days ) {
 	...
 	//Main SQL call for the information for the day and also for the reoccuring event.
		$sql = 'WHERE `day`= '.$current_day.' AND `month` = '.$month.' AND `year` = '.$year.' AND  `reoccur` = \'0\' OR (`day` = '.$current_day.' AND `reoccur` = \'1\') OR (`day` = '.$current_day.' AND `month` = '.$month.' AND `reoccur` = \'2\')';
	  	$result = $nuke_db->sql_query("SELECT eid FROM ".$prefix."_ecalendar ".$sql);
	    if ($nuke_db->sql_numrows($result) > 0) {
			...
			$result = $nuke_db->sql_query("SELECT `eid`, `month`, `day`, `year`, `reoccur`, `time`, `ampm`, `title` FROM ".$prefix."_ecalendar ".$sql." ORDER by `eid` DESC");
	        while (list($eid, $emonth, $eday, $eyear, $reoccur, $time, $ampm, $title) = $nuke_db->sql_fetchrow($result)) {

...
if ($currentlang) {
    include_once(NUKE_MODULES_DIR.$nuke_module_name.'/lang-admin/lang-'.$currentlang.'.php');
} else {
    include_once(NUKE_MODULES_DIR.$nuke_module_name.'/lang-admin/lang-english.php');
}
...
if ((($rowColor = $nuke_cache->load('theme_'.$ThemeSel, 'shoutbox')) == false) || empty($rowColor)) {
    $sql = "SELECT * FROM `".$prefix."_shoutbox_themes` WHERE themeName='$ThemeSel'";
    $result = $nuke_db->sql_query($sql);
    $rowColor = $nuke_db->sql_fetchrow($result);
    $nuke_cache->save('theme_'.$ThemeSel, 'shoutbox', $rowColor);
    $nuke_db->sql_freeresult($result);
}

global $shout_conf;

if ((($shout_conf = $nuke_cache->load('conf', 'shoutbox')) == false) || empty($shout_conf)) {
    $sql = "SELECT * FROM `".$prefix."_shoutbox_conf`";
    $result = $nuke_db->sql_query($sql);
    $shout_conf = $nuke_db->sql_fetchrow($result);
    $nuke_cache->save('conf', 'shoutbox', $shout_conf);
}
...
function manageShouts($page, $pruned) {
    global $prefix, $nuke_db, $admin, $admin_file, $sbURL, $nuke_module_name, $shout_conf, $rowColor, $nuke_userinfo, $board_config;
    ...
    $resultA = $nuke_db->sql_query($sql);
    $aCount = $nuke_db->sql_fetchrow($resultA);
    ...
    $x = 1;
    while ($shout = $nuke_db->sql_fetchrow($nameresult)) {
        ...
        $nameresult2 = $nuke_db->sql_query($sql);
        $row = $nuke_db->sql_fetchrow($nameresult2);
 ...
 ******************************************************/
        ...
        if ($shout['ip'] != 'noip') {
            ...
            $nameIPresult = $nuke_db->sql_query($sql);
            $nameIProw = $nuke_db->sql_fetchrow($nameIPresult);
            $nuke_db->sql_freeresult($nameIPresult);
            if ($nameIProw) {
    ...
    $resultTS = $nuke_db->sql_query($sql);
    $TSrows = $nuke_db->sql_numrows($resultTS);
    $nuke_db->sql_freeresult($resultTS);
    if ($TSrows > 0) {
...
function pruneSubmit($pruneDays, $page) {
    ...
    if ($pruneDays > 0 AND is_numeric($pruneDays)) {
        ...
        if ($pruneDays > 0) {
            ...
            $sql = "SELECT `id` FROM `".$prefix."_shoutbox_shouts`";
            $result = $nuke_db->sql_query($sql);
            $shoutsBefore = $nuke_db->sql_numrows($result);
            $nuke_db->sql_freeresult($result);
            ...
            $sql = "SELECT `id` FROM `".$prefix."_shoutbox_shouts`";
            $result = $nuke_db->sql_query($sql);
            $shoutsAfter = $nuke_db->sql_numrows($result);
...
function stickySubmit($stickyShout, $stickyUsername, $stickySlot, $page) {
    global $prefix, $nuke_db, $admin_file, $shout_conf;
    if ($stickyShout) {
        $conf = $shout_conf;
        ...
        $stickyResult = $nuke_db->sql_query($sql);
        $stickyRow = $nuke_db->sql_fetchrow($stickyResult);
        $nuke_db->sql_freeresult($stickyResult);
        if ($stickyRow) {
...
function ShoutSave($shoutID, $ShoutComment, $page) {
    ...
    if($conf['censor'] == "yes") {
        ...
        $nuke_db->sql_freeresult($cresult);
        $ShoutComment = implode(" ",$ShoutArrayReplace);
...
function adminDropCount($aCount, $page) {
    global $prefix, $nuke_db, $admin_file, $admin;
...
function ShoutBoxLayout() {
    global $prefix, $nuke_db, $admin_file, $sbURL, $shout_conf, $rowColor;
    include_once(NUKE_BASE_DIR.'header.php');
    LinkAdmin();
    $conf = $shout_conf;
...
function ShoutBoxLayoutSet($daten, $numbern, $heightn, $textboxwidth, $smiliesperrow, $reverseshouts, $pointspershout, $shoutsperpagehistory, $serverTimezone) {
    ...
    $nuke_db->sql_query($sql);
    $nuke_cache->delete('conf', 'shoutbox');
    $nuke_cache->resync();
...
function ShoutBoxAdminMonitor() {
    global $prefix, $nuke_db, $admin_file, $nuke_module_name, $nuke_cache;
    OpenTable();
    ...
    $filename = "modules/$nuke_module_name/language/index.html";
    if (file_exists($filename) != TRUE) { $SBhealthCount = 1; }
    $filename = "modules/$nuke_module_name/images/down/Black.gif";
    if (file_exists($filename) != TRUE) { $SBhealthCount = 1; }
    $filename = "modules/$nuke_module_name/images/up/Black.gif";
    if (file_exists($filename) != TRUE) { $SBhealthCount = 1; }
    $filename = "modules/$nuke_module_name/images/pause/Black.gif";
    if (file_exists($filename) != TRUE) { $SBhealthCount = 1; }
    ...
    $filename = "modules/$nuke_module_name";
    if (is_dir($filename) != TRUE) { $SBhealthCount = 2; }
    $filename = "modules/$nuke_module_name/copyright.php";
    if (file_exists($filename) != TRUE) { $SBhealthCount = 2; }
    $filename = "includes/shoutbox.js";
    if (file_exists($filename) != TRUE) { $SBhealthCount = 2; }
    $filename = "modules/$nuke_module_name/index.php";
    if (file_exists($filename) != TRUE) { $SBhealthCount = 2; }
    $filename = "modules/$nuke_module_name/language/lang-english.php";
    if (file_exists($filename) != TRUE) { $SBhealthCount = 2; }
    $filename = "blocks/block-$nuke_module_name.php";
    if (file_exists($filename) != TRUE) { $SBhealthCount = 2; }
    $filename = "modules/$nuke_module_name/images/paypal.gif";
    if (file_exists($filename) != TRUE) { $SBhealthCount = 2; }
    ...
    if (file_exists($filename) == TRUE) { $SBhealthCount = 2; }
    $sqlCheck = "SELECT * FROM ".$prefix."_blocks WHERE blockfile='block-$nuke_module_name.php'";
    $resultCheck = $nuke_db->sql_query($sqlCheck);
    $numrowsCheck = $nuke_db->sql_numrows($resultCheck);
    if ($numrowsCheck != 1) { $SBhealthCount = 2; }
    $rowCheck = $nuke_db->sql_fetchrow($resultCheck);
    if ($rowCheck['active'] != 1) { $SBhealthCount = 2; }
    if (($rowCheck['view'] != 0) AND ($rowCheck['view'] != 1)) { $SBhealthCount = 2; }
    $sqlCheck = "SELECT * FROM ".$prefix."_modules WHERE title='$nuke_module_name'";
    $resultCheck = $nuke_db->sql_query($sqlCheck);
    $numrowsCheck = $nuke_db->sql_numrows($resultCheck);
    if ($numrowsCheck != 1) { $SBhealthCount = 2; }
    $rowCheck = $nuke_db->sql_fetchrow($resultCheck);
...
function shoutHealth($SBhealthCount) {
    global $prefix, $nuke_db, $admin_file, $nuke_module_name;
    include_once(NUKE_BASE_DIR.'header.php');
    LinkAdmin();
    $ShoutMenuOptionActive = 2;
    ShoutBoxAdminMenu($ShoutMenuOptionActive);
    ...
    $numrowsCheck = $nuke_db->sql_numrows($resultCheck);
    if ($numrowsCheck != 1) {
        echo "<p class=\"content\"><strong>Shout Box block not added:</strong> block-$nuke_module_name.php&nbsp;&nbsp;<strong>Recommendation:</strong> You have not added the Shout Box block into PHP-Nuke. Click the Blocks icon in the admin area, scroll down the 'Add a new block' area. In the filename drop down, choose block-$nuke_module_name.php. Set 'Who can view this' as 'All Visitors', and click the 'Create Block' button.</p>";
    }
    $rowCheck = $nuke_db->sql_fetchrow($resultCheck);
    if ($rowCheck['active'] != '1') {
        echo "<p class=\"content\"><strong>Shout Box block not active:</strong> block-$nuke_module_name.php&nbsp;&nbsp;<strong>Recommendation:</strong> Click the Blocks icon in the admin area, scroll down to the 'Blocks Administration' area. In the Functions column and Shout Box row, choose 'Activate'. In the following page, choose 'Yes' to activate the block.</p>";
    }
    if (($rowCheck['view'] != 0) && ($rowCheck['view'] != 1)) {
        echo "<p class=\"content\"><strong>Shout Box block not publicly viewable:</strong> block-$nuke_module_name.php&nbsp;&nbsp;<strong>Recommendation:</strong> Click the Blocks icon in the admin area, scroll down to the 'Blocks Administration' area. In the Functions column and Shout Box row, choose 'Edit'. In the following page, set 'Who can view this' as 'All Visitors', and click the 'Save Block' button.</p>";
    }
    $sqlCheck = "SELECT * FROM ".$prefix."_modules WHERE title='$nuke_module_name'";
    $resultCheck = $nuke_db->sql_query($sqlCheck);
    $numrowsCheck = $nuke_db->sql_numrows($resultCheck);
    if ($numrowsCheck != 1) {
        echo "<p class=\"content\"><strong>Shout Box module not added:</strong> $nuke_module_name.php&nbsp;&nbsp;<strong>Recommendation:</strong> You have not added the Shout Box module into PHP-Nuke. Upload the files into the modules directory. You can obtain the files from the Shout Box installation zip file. Once they are uploaded, Click the Modules icon in PHP-Nuke administration. In the Functions column and Shout Box row, choose 'Activate'. In the Functions column and Shout Box row, choose 'Edit'. In the following page, set 'Who can view this' as 'All Visitors', and click the 'Save Changes' button.</p>";
    }
    $rowCheck = $nuke_db->sql_fetchrow($resultCheck);
    if ($rowCheck['active'] != 1) {
        echo "<p class=\"content\"><strong>Shout Box module not active:</strong> $nuke_module_name&nbsp;&nbsp;<strong>Recommendation:</strong> Click the Modules icon in PHP-Nuke administration. In the Functions column and Shout Box row, choose 'Activate'.</p>";
    }
    if (($rowCheck['view'] != 0) AND ($rowCheck['view'] != 1)) {
        echo "<p class=\"content\"><strong>Shout Box module not publicly viewable:</strong> block-$nuke_module_name.php&nbsp;&nbsp;<strong>Recommendation:</strong> Click the Modules icon in PHP-Nuke administration. In the Functions column and Shout Box row, choose 'Edit'. In the following page, set 'Who can view this' as 'All Visitors', and click the 'Save Changes' button.</p>";
    }
...
function ShoutBoxThemeing() {
    global $prefix, $nuke_db, $admin_file, $sbURL, $nuke_module_name, $rowColor;
    include_once(NUKE_BASE_DIR.'header.php');
    LinkAdmin();
    ...
    $imagesDir = dir(NUKE_MODULES_DIR.$nuke_module_name."/images/up");
    $imageList = '';
    while ($image=$imagesDir->read()) {
        if (!empty($image) && $image != '.' && $image != '..' && $image != 'index.html' && file_exists(NUKE_MODULES_DIR.$nuke_module_name."/images/down/$image") && file_exists(NUKE_MODULES_DIR.$nuke_module_name."/images/pause/$image")) {
            $imageList .= "$image ";
        }
    }
    ...
    $backgroundDir = dir(NUKE_MODULES_DIR.$nuke_module_name."/images/background");
    $backImageList = '';
...
function themeSubmit($themeColorValues, $totalThemes) {
    global $prefix, $nuke_db, $admin_file, $nuke_cache, $ThemeSel;
    for ($x = 0; $x <= $totalThemes; $x++) {
        ...
        $nuke_db->sql_query($sql);
        $nuke_cache->delete('theme_'.$ThemeSel, 'shoutbox');
        $nuke_cache->resync();
    }
...
function ShoutBoxPermissions() {
    global $prefix, $nuke_db, $admin_file, $shout_conf, $rowColor;
    include_once(NUKE_BASE_DIR.'header.php');
    LinkAdmin();
    $conf = $shout_conf;
...
function managecensor() {
    ...
    if ($numrows > 0) {
        ...
        for ($shx = 1; $censor = $nuke_db->sql_fetchrow($nameresult); $shx++) {
...
function addcensor($addcensor, $addcensorr) {
    ...
    $nuke_db->sql_query($sql);
    $nuke_cache->delete('censor', 'shoutbox');
    $nuke_cache->resync();
    header("Location: ".$admin_file.".php?op=shout&Submit=managecensor");
    exit;
}
...
function ShoutBoxBans() {
    ...
    $numrows = $nuke_db->sql_numrows($ipresult);
    if ($numrows > 0) {
        ...
        for ($shx = 1; $badips = $nuke_db->sql_fetchrow($ipresult); $shx++) {
    ...
    if ($numrows > 0) {
        ...
        for ($shx = 1; $badnames = $nuke_db->sql_fetchrow($nameresult); $shx++) {
...
function addname($addname) {
    ...
    $nuke_db->sql_query($sql);
    $nuke_cache->delete('nameblock', 'shoutbox');
    $nuke_cache->resync();
    header("Location: ".$admin_file.".php?op=shout&Submit=ShoutBoxBans");
    exit;
}
...
switch($Submit) {
    ...
    $idresult = $nuke_db->sql_query($sql);
    $banip = $nuke_db->sql_fetchrow($idresult);
    $addip = $banip['ip'];
    addip($addip, $page);
    break;

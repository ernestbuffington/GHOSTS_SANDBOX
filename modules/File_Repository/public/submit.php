...
{
	global $nuke_db, $admin_file, $lang_new, $nuke_module_name, $settings, $themes, $nuke_userinfo, $admin, $nuke_user;
	OpenTable();
	...
	echo '<br />';
	echo '<form action="modules.php?name='.$nuke_module_name.'&amp;action=submitdownload_save" method="post" enctype="multipart/form-data">'."\n";
	echo '<table style="width: 100%;" border="0" cellpadding="4" cellspacing="1" class="forumline">'."\n";
	echo '  <tr'._bgColor(2).'>'."\n";
	echo '    <td'._tdcss(false,'center',_sh(),2).'>'._suh($lang_new[$nuke_module_name]['SUBMITDOWNLOAD_HEADER']).'</td>'."\n";
	echo '  </tr>'."\n";
	if ($numrows > 0 && $settings['users_can_upload'] == true && _check_users_permissions($settings['group_allowed_to_upload']) == true ):
		...
		echo '  </tr>'."\n";
		echo '  <tr'._bgColor(1).'>'."\n";
		echo '    <td'._tdcss('50%',false,_sc()).'>'._sut($lang_new[$nuke_module_name]['CATEGORY']).'</td>'."\n";
		echo '    <td'._tdcss('50%',false,_sc()).'>'._category_parents_and_children('cid',0,false,true).'</td>'."\n";
		echo '  </tr>'."\n";
		echo '  <tr'._bgColor(2).'>'."\n";
		echo '    <td'._tdcss(false,false,_sh(),2).'>'._sut($lang_new[$nuke_module_name]['FILES']).'</td>'."\n";
		echo '  </tr>'."\n";
		echo '  <tr'._bgColor(1).'>'."\n";
		...
		echo '          <td'._tdcss('75%').'>'._sut($lang_new[$nuke_module_name]['FILE']).'</td>'."\n";
		echo '        </tr>';
		...
		echo '  </tr>'."\n";
		echo '  <tr'._bgColor(2).'>'."\n";
		echo '    <td'._tdcss(false,false,_sh(),2).'>'._sut($lang_new[$nuke_module_name]['SCREENSHOTS']).'</td>'."\n";
		echo '  </tr>'."\n";
		echo '  <tr'._bgColor(1).'>'."\n";
		echo '    <td'._tdcss(false,false,_sc(),2).'>'."\n";
		echo '      <table style="width: 100%; white-space: nowrap;" cellpadding="0" cellspacing="0" border="0">'."\n";
		...
		echo '  <tr'._bgColor(2).'>'."\n";
		echo '	  <td'._tdcss(FALSE,FALSE,_sh(),2).'>'._suh($lang_new[$nuke_module_name]['SUBMITTER_DETAILS']).'</td>'."\n";
		echo '  </tr>'."\n";
		echo '  <tr'._bgColor(1).'>'."\n";
		echo '    <td'._tdcss('50%',FALSE,_sc()).'>'._sut($lang_new[$nuke_module_name]['SUBMITTER_USERNAME']).'</td>'."\n";
		echo '    <td'._tdcss('50%',FALSE,_sc()).'>'.$nuke_userinfo['username'].'</td>'."\n";
		echo '  </tr>'."\n";
		echo '  <tr'._bgColor(1).'>'."\n";
		echo '    <td'._tdcss('50%',FALSE,_sc()).'>'._sut($lang_new[$nuke_module_name]['SUBMITTER_EMAIL']).'</td>'."\n";
		...
		if($settings['users_can_upload'] == false):
			...
			// echo $lang_new[$nuke_module_name]['USER_UPLOAD_DISABLED'];
			echo '  <tr'._bgColor(1).'>'."\n";
			echo '    <td'._tdcss(false,'center',_sc(),2).'>'.$lang_new[$nuke_module_name]['USER_UPLOAD_DISABLED'].'</td>'."\n";
			echo '  </tr>'."\n";
...
function _file_repository_save_submitdownload()
{
	...
	if ($numrows > 0 && $settings['users_can_upload'] == true && _check_users_permissions($settings['group_allowed_to_upload']) == true ):
		...
		if (count( $error_messages ) > 0):
			...
			_nuke_redirect('modules.php?name='.$nuke_module_name.'&action=submitdownload_success');
		endif;
...
{
	global $lang_new, $nuke_module_name, $settings, $nuke_userinfo;
	OpenTable();
...
function _file_repository_success_submitdownload()
{
	...
	OpenTable();
	echo '<div style="text-align: center;">';
	echo '<p>'.sprintf($lang_new[$nuke_module_name]['SUBMITDOWNLOAD_SUCCESS'], $nuke_userinfo['username']).'</p>';
	echo '</div>';
	CloseTable();
}
...
endswitch;
		...
		// 	echo '          <td'._tdcss('10%','center').'><span id="add-screen-field" class="dm-sprite attachment" alt="'.$lang_new[$nuke_module_name]['SCREENSHOT_ANOTHER'].'" title="'.$lang_new[$nuke_module_name]['SCREENSHOT_ANOTHER'].'"></span></td>'."\n";
		// 	echo '        <tr>'."\n";

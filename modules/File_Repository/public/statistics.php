...
function _general_statistics()
{
	...
	echo '  <tr'._bgColor(2).'>'."\n";
	echo '    <td'._tdcss(false,'center',_sh(),2).'>'._suh($lang_new[$nuke_module_name]['STATISTICS_HEADER']).'</td>'."\n";
	echo '  </tr>'."\n";
	echo '  <tr'._bgColor(1).'>'."\n";
	echo '    <td'._tdcss(false,false,_sc()).'>';
	echo '      <table width="100%" border="0" cellpadding="4" cellspacing="1" class="forumline">'."\n";
	echo '        <tr'._bgColor(1).'>';
	echo '          <td'._tdcss('70%',false,_sc()).'>'._sut($lang_new[$nuke_module_name]['TOTAL_CATEGORIES']).'</td>';
	echo '          <td'._tdcss('30%','center',_sc()).'>'.$totalcategories.'</td>';
	echo '        </tr>';
	echo '        <tr'._bgColor(1).'>';
	echo '          <td'._tdcss('70%',false,_sc()).'>'._sut($lang_new[$nuke_module_name]['TOTAL_FILES']).'</td>';
	...
	echo '      <table width="100%" border="0" cellpadding="4" cellspacing="1" class="forumline">'."\n";
	echo '        <tr'._bgColor(1).'>';
	echo '          <td'._tdcss('70%',false,_sc()).'>'._sut($lang_new[$nuke_module_name]['TOTAL_DOWNLOADS']).'</td>';
	echo '          <td'._tdcss('30%','center',_sc()).'>'.(($statistic['totalhits']) ? $statistic['totalhits'] : '0').'</td>';
	echo '        </tr>';
	echo '        <tr'._bgColor(1).'>';
	echo '          <td'._tdcss('70%',false,_sc()).'>'._sut($lang_new[$nuke_module_name]['TOTAL_DATABASE_SIZE']).'</td>';
	...
	# TOP 5 POPULAR FILES
	echo '  <tr'._bgColor(2).'>'."\n";
	echo '	  <td'._tdcss('50%',false,_sh()).'>'._suh($lang_new[$nuke_module_name]['TOPFIVE']).'</td>'."\n";
	echo '	  <td'._tdcss('50%',false,_sh()).'>'._suh($lang_new[$nuke_module_name]['LATEST_FILES']).'</td>'."\n";
	echo '  </tr>'."\n";
	echo '  <tr'._bgColor(1).'>'."\n";
	...
	{
		echo '        <tr'._bgColor(2).'>'."\n";
		echo '          <td'._tdcss('70%',false,_sh()).'>'._suh($lang_new[$nuke_module_name]['TITLE']).'</td>'."\n";
		echo '          <td'._tdcss('30%','center',_sh()).'>'._suh($lang_new[$nuke_module_name]['HITS']).'</td>'."\n";
		echo '        </tr>'."\n";
		while($popular = $nuke_db->sql_fetchrow($presult))
		{
			...
			echo '        <tr'._bgColor(1).'>'."\n";
			echo '	        <td'._tdcss('70%',false,_sc()).'><a'._ls().' href="modules.php?name='.$nuke_module_name.'&amp;action=view&amp;did='.$popular['did'].'">'._colorization($popular['title'],$popular['color']).'</a>'._item_is_new($iteminfo['isnew'],$popular['isupdated'])._mostpopular($popular['hits']).'</td>';
		...
		echo '        <tr'._bgColor(1).'>'."\n";
		echo '	        <td'._tdcss(false,'center',_sc(),2).'>'.$lang_new[$nuke_module_name]['NOINFO'].'</td>';
		echo '        </tr>';
	}
	echo '      </table>'."\n";
	echo '    </td>'."\n";
	...
	if($totaldownloads > 0)
	{
		echo '        <tr'._bgColor(2).'>'."\n";
		echo '          <td'._tdcss('70%',false,_sh()).'>'._suh($lang_new[$nuke_module_name]['TITLE']).'</td>'."\n";
		echo '          <td'._tdcss('30%','center',_sh()).'>'._suh($lang_new[$nuke_module_name]['DATE_ADDED']).'</td>'."\n";
		echo '        </tr>'."\n";
		while($latest = $nuke_db->sql_fetchrow($lresult))
		{
			...
			echo '        <tr'._bgColor(1).'>'."\n";
			echo '	        <td'._tdcss('70%',false,_sc()).'><a'._ls().' href="modules.php?name='.$nuke_module_name.'&amp;action=view&amp;did='.$latest['did'].'">'._colorization($latest['title'],$latest['color']).'</a>'._item_is_new($iteminfo['isnew'],$latest['isupdated'])._mostpopular($latest['hits']).'</td>';

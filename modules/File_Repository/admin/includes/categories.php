...
function _file_repository_categories()
{
	...
	echo '  <tr'._bgColor(2).'>'."\n";
	echo '    <td'._tdcss(FALSE,'center',_sh(),5).'>'._suh($lang_new[$nuke_module_name]['CATEGORY_LIST']).'</td>'."\n";
	echo '  </tr>'."\n";
	echo '  <tr'._bgColor(2).'>'."\n";
	echo '    <td'._tdcss(FALSE,FALSE,_sh(),2).'>'._suh($lang_new[$nuke_module_name]['CATEGORY_NAME']).'</td>'."\n";	
	echo '    <td'._tdcss('10%','center',_sh()).'>'._suh($lang_new[$nuke_module_name]['FILES']).'</td>'."\n";							
	echo '    <td'._tdcss('10%','center',_sh()).'>'._suh($lang_new[$nuke_module_name]['CATEGORY_UPLOADS']).'</td>'."\n";
	echo '    <td'._tdcss('20%','center',_sh()).'>'._suh($lang_new[$nuke_module_name]['UPLOAD_PERMISSIONS']).'</td>'."\n";
	echo '  </tr>'."\n";
	echo '  <tr'._bgColor(1).(($nuke_db->sql_numrows($result) == 0 ) ? '' : ' style="display:none;"').'>';
	echo '    <td'._tdcss(FALSE,'center',_sc(),5).'>'._sut($lang_new[$nuke_module_name]['CATEGORY_NONE']).'</td>';
	echo '  </tr>';
...
//---------------------------------------------------------------------
	echo '<table style="width: 100%;" border="0" cellpadding="4" cellspacing="1" class="forumline">'."\n";
	echo '  <tr'._bgColor(2).'>'."\n";
	echo '    <td'._tdcss(FALSE,'center',_sh(),2).'>'._suh($lang_new[$nuke_module_name]['CATEGORY_ADD']).'</td>'."\n";
	echo '  </tr>'."\n";
	echo '  <tr'._bgColor(1).(($row['parentid'] == 0 && $_GET['action'] <> 'newcat') ? ' style="display:none;"' : '').'>'."\n";
	echo '    <td'._tdcss('50%',FALSE,_sc()).'>'._sut($lang_new[$nuke_module_name]['CATEGORY_PARENT']).'</td>'."\n";
	echo '    <td'._tdcss('50%',FALSE,_sc()).'>'._category_parents('parent',$row['parentid']).'</td>'."\n";
	echo '  </tr>'."\n";
	echo '  <tr'._bgColor(1).'>'."\n";
	echo '    <td'._tdcss('50%',FALSE,_sc()).'>'._sut($lang_new[$nuke_module_name]['CATEGORY_NAME']).'</td>'."\n";
	echo '    <td'._tdcss('50%',FALSE,_sc()).'>'._input('text','title','250px',$row['cname']).'</td>'."\n";
	echo '  </tr>'."\n";
	echo '  <tr'._bgColor(1).'>'."\n";
	echo '    <td'._tdcss('50%',FALSE,_sc()).'>'._sut($lang_new[$nuke_module_name]['CATEGORY_COLOR']).'</td>'."\n";
	...
	echo '  </tr>'."\n";
	echo '  <tr'._bgColor(1).'>'."\n";
	echo '    <td'._tdcss('50%',FALSE,_sc()).'>'._sut($lang_new[$nuke_module_name]['CATEGORY_ALLOWED']).'</td>'."\n";
	echo '    <td'._tdcss('50%',FALSE,_sc()).'>'._selectbox('isallowed',array(0 => $lang_new[$nuke_module_name]['N'], 1 => $lang_new[$nuke_module_name]['Y']),$row['isallowed']).'</td>'."\n";
	echo '  </tr>'."\n";
	echo '  <tr'._bgColor(1).'>'."\n";
	echo '    <td'._tdcss('50%',FALSE,_sc()).'>'._sut($lang_new[$nuke_module_name]['CATEGORY_WHOADD']).'</td>'."\n";

...
//-------------------------------------------------------------------------------------
	...
	echo '  <tr'._bgColor(2).'>'."\n";
	echo '    <td'._tdcss(FALSE,'center',_sh(),5).'>'._suh($lang_new[$nuke_module_name]['FILE_LIST']).'</td>'."\n";
	echo '  </tr>'."\n";
	echo '  <tr'._bgColor(2).'>'."\n";
	echo '    <td'._tdcss('5%','center',_sh()).'>'._suh($lang_new[$nuke_module_name]['ACTIVE']).'</td>'."\n";
	echo '    <td'._tdcss('55%',FALSE,_sh()).'>'._suh($lang_new[$nuke_module_name]['ITEM']).'</td>'."\n";
	echo '    <td'._tdcss('10%','center',_sh()).'>'._suh($lang_new[$nuke_module_name]['RATINGS']).'</td>'."\n";
	echo '    <td'._tdcss('10%','center',_sh()).'>'._suh($lang_new[$nuke_module_name]['DOWNLOADS']).'</td>'."\n";
	echo '    <td'._tdcss('20%','center',_sh()).'>'._suh($lang_new[$nuke_module_name]['DOWNLOADS_PERMISSIONS']).'</td>'."\n";
	echo '  </tr>'."\n";
	...
	echo '    <td'._tdcss(FALSE,'center',_sc(),5).'>'._sut($lang_new[$nuke_module_name]['FILE_NONE']).'</td>';
	echo '  </tr>';
	while($download = $nuke_db->sql_fetchrow($result))
	{
		if($download['title'])
		{
			...
			echo '  <tr'._bgColor(1).'>'."\n";
			echo '    <td'._tdcss('5%','center',_sc()).'>'._sut((($download['isactive'] == 1) ? $lang_new[$nuke_module_name]['Y'] : $lang_new[$nuke_module_name]['N'])).'</td>'."\n";
			echo '    <td'._tdcss('55%',FALSE,_sc()).'>'."\n";
			echo '      <table width="100%" border="0" cellpadding="0" cellspacing="0">'."\n";
			echo '        <tr>'."\n";
			echo '          <td'._tdcss('80%',FALSE).'><a'._ls().' href="modules.php?name='.$nuke_module_name.'&amp;action=view&amp;did='.$download['did'].'" target="_blank">'._colorization($download['title'],$download['color']).'</a></td>'."\n";
	...
	if($count_downloads > $settings['most_popular'])
	{
		if ($pagination->getCurrent() == 1)
			$first = ' | '.$lang_new[$nuke_module_name]['FIRST'].' | ';
		else
			$first = ' | <a href="'.$admin_file.'.php?op='._MODNAME.'&amp;action=files&amp;page='.$pagination->getFirst().'">'.$lang_new[$nuke_module_name]['FIRST'].'</a> |';
			
		if ($pagination->getPrevious())
			$prev = '<a href="'.$admin_file.'.php?op='._MODNAME.'&amp;action=files&amp;page='.$pagination->getPrevious().'">'.$lang_new[$nuke_module_name]['PREV'].'</a> | ';
		else
			$prev = $lang_new[$nuke_module_name]['PREV'].' | ';
			
		if ($pagination->getNext())
			$next = '<a href="'.$admin_file.'.php?op='._MODNAME.'&amp;action=files&amp;page='.$pagination->getNext().'">'.$lang_new[$nuke_module_name]['NEXT'].'</a> | ';
		else
			$next = $lang_new[$nuke_module_name]['NEXT'].' | ';
			
		if ($pagination->getLast())
			$last = '<a href="'.$admin_file.'.php?op='._MODNAME.'&amp;action=files&amp;page='.$pagination->getLast().'">'.$lang_new[$nuke_module_name]['LAST'].'</a>';
		else
...
function _file_repository_broken_files()
{
	...
	echo '<table style="width: 100%;" border="0" cellpadding="4" cellspacing="1" class="forumline">'."\n";
	echo '  <tr'._bgColor(2).'>'."\n";
	echo '    <td'._tdcss(FALSE,'center',_sh(),3).'>'._suh($lang_new[$nuke_module_name]['BROKEN_ITEMS']).'</td>'."\n";
	echo '  </tr>'."\n";
	...
	if($count_broken_downloads['isbroken'] > 0):
		...
		echo '    <td'._tdcss('70%',FALSE,_sh()).'>'.$lang_new[$nuke_module_name]['FILE_TITLE'].'</td>'."\n";
		echo '    <td'._tdcss('20%','center',_sh()).'>'.$lang_new[$nuke_module_name]['FILE_OPTIONS'].'</td>'."\n";
		echo '  </tr>'."\n";
		while($broken = $nuke_db->sql_fetchrow($result)):
			...
			echo '    <td'._tdcss('20%','center',_sc()).'><a href="'.$admin_file.'.php?op='._MODNAME.'&amp;action=editfile&amp;did='.$broken['did'].'">'.$lang_new[$nuke_module_name]['REPAIR'].'</a>&nbsp;|&nbsp;<a href="'.$admin_file.'.php?op='._MODNAME.'&amp;action=deleteitem&amp;did='.$broken['did'].'">'.$lang_new[$nuke_module_name]['DELETE'].'</a></td>'."\n";
			echo '  </tr>'."\n";
			$isbroken++;
		...
		echo '  <tr'._bgColor(2).'>'."\n";
		echo '    <td'._tdcss(FALSE,'center',_sc(),3).'>'.$lang_new[$nuke_module_name]['BROKEN_ITEMS_NONE'].'</td>'."\n";
		echo '  </tr>'."\n";
...
function _file_repository_new_file()
{
	...
	echo '<table style="width: 100%;" border="0" cellpadding="4" cellspacing="1" class="forumline">'."\n";
	echo '  <tr'._bgColor(2).'>'."\n";
	echo '    <td'._tdcss(FALSE,'center',_sh(),2).'>'._suh($lang_new[$nuke_module_name]['FILE_ADD']).'</td>'."\n";
	echo '  </tr>'."\n";
	echo '  <tr'._bgColor(1).'>'."\n";
	echo '    <td'._tdcss('50%',FALSE,_sc()).'>'._sut($lang_new[$nuke_module_name]['FILE_TITLE']).'</td>'."\n";
	echo '    <td'._tdcss('50%',FALSE,_sc()).'>'._input('text','title','250px',$row['title']).'</td>'."\n";
	echo '  </tr>'."\n";
	echo '  <tr'._bgColor(1).'>'."\n";
	echo '    <td'._tdcss('50%',FALSE,_sc()).'>'._sut($lang_new[$nuke_module_name]['PREVIEW']).'</td>'."\n";
	echo '    <td'._tdcss('50%',FALSE,_sc()).'>'._input('text','preview','250px',$row['preview']).'</td>'."\n";
	echo '  </tr>'."\n";
	echo '  <tr'._bgColor(1).'>'."\n";
	echo '    <td'._tdcss('50%',FALSE,_sc()).'>'._sut($lang_new[$nuke_module_name]['FILE_COLOR']).'</td>'."\n";
	...
	echo '  </tr>'."\n";
	echo '  <tr'._bgColor(1).'>'."\n";
	echo '    <td'._tdcss('50%',FALSE,_sc()).'>'._sut($lang_new[$nuke_module_name]['FILE_VERSION']).'</td>'."\n";
	echo '    <td'._tdcss('50%',FALSE,_sc()).'>'._input('text','version','100px',$row['version']).'</td>'."\n";
	echo '  </tr>'."\n";
	echo '  <tr'._bgColor(1).'>'."\n";
	echo '    <td'._tdcss('50%',FALSE,_sc()).'>'._sut($lang_new[$nuke_module_name]['FILE_CATEGORY']).'</td>'."\n";
	echo '    <td'._tdcss('50%',FALSE,_sc()).'>'._category_parents_and_children('cid',$row['cid']).'</td>'."\n";
	echo '  </tr>'."\n";
	echo '  <tr'._bgColor(1).'>'."\n";
	echo '    <td'._tdcss('50%',FALSE,_sc()).'>'._sut($lang_new[$nuke_module_name]['ATTACH_LOCAL_FILE']).'</td>'."\n";
	echo '    <td'._tdcss('50%',FALSE,_sc()).'><a'._ls().' href="javascript:void(0);" class="mainforum_btn" id="isuploaded"><span style="text-transform: '.(($settings['utext'] == 1) ? 'uppercase' : 'none').';">'.$lang_new[$nuke_module_name]['SHOW_LOCAL'].'</span></a></td>'."\n";
	echo '  </tr>'."\n";
	echo '  <tr'._bgColor(2).' class="attachment_options" style="display: none;">'."\n";
	echo '	  <td'._tdcss(FALSE,FALSE,_sh(),2).'>'._suh($lang_new[$nuke_module_name]['LOCAL_FILES']).'</td>'."\n";
	echo '  </tr>'."\n";
	...
	echo '        <tr>';
	echo '          <td'._tdcss('25%').'>'._sut($lang_new[$nuke_module_name]['FILE_TITLE']).'</td>'."\n";
	echo '          <td'._tdcss('45%').'>'._sut($lang_new[$nuke_module_name]['FILE']).'</td>'."\n";
	echo '          <td'._tdcss('20%','center').'>'._sut($lang_new[$nuke_module_name]['FILE_SIZE']).'</td>'."\n";
	echo '          <td'._tdcss('10%','center').'>'._sut($lang_new[$nuke_module_name]['FILE_OPTIONS']).'</td>'."\n";
	echo '        <tr>';
	...
	echo '  </tr>';
	echo '  <tr>'."\n";
	echo '	  <td'._tdcss(FALSE,FALSE,_sh(),2).'>'._suh($lang_new[$nuke_module_name]['FILES']).'</td>'."\n";
	echo '  </tr>'."\n";
	echo '  <tr'._bgColor(1).'>'."\n";
	...
	if($nuke_db->sql_numrows($result) > 0)
	{
		while($file = $nuke_db->sql_fetchrow($result))
		{
			...
			echo '          <td'._tdcss('45%',FALSE,FALSE,FALSE,TRUE).'>'.$filename.((!file_exists(_FILE_REPOSITORY_DIR.$file['filename'])) ? '<span style="color: darkred;">&nbsp;'.$lang_new[$nuke_module_name]['FILE_MISSING'].'</span>' : '').'</td>'."\n";
	...
	echo '          <td'._tdcss('10%','center').'>'._submit($lang_new[$nuke_module_name]['UPLOAD'],'fileupload').'</td>'."\n";
	echo '        </tr>'."\n";
	echo '      </table>'."\n";
	echo '</td>'."\n";
	echo '  </tr>'."\n";
	echo '  <tr'._bgColor(2).'>'."\n";
	echo '	  <td'._tdcss(FALSE,FALSE,_sh(),2).'>'._suh($lang_new[$nuke_module_name]['SCREENSHOTS']).'</td>'."\n";
	echo '  </tr>'."\n";
	echo '  <tr'._bgColor(1).'>'."\n";
	echo '	  <td'._tdcss(FALSE,FALSE,_sc(),2).'>'."\n";
	...
	if($nuke_db->sql_numrows($result) > 0)
	{
		while($screen = $nuke_db->sql_fetchrow($result))
		{
			...
			echo '          <td'._tdcss('45%',FALSE,FALSE,FALSE,TRUE).'><a'._ls().get_image_viewer('screens').' href="'._FILE_REPOSITORY_SCREENS.$screen['filename'].'" title="'.$screen['title'].'">'.$screen['filename'].'</a>'.((!file_exists(_FILE_REPOSITORY_SCREENS.$screen['filename'])) ? '<span style="color: darkred;">&nbsp;'.$lang_new[$nuke_module_name]['FILE_MISSING'].'</span>' : '').'</td>'."\n";
	...
	echo '          <td'._tdcss('10%','center').'>'._submit($lang_new[$nuke_module_name]['UPLOAD'],'imageupload').'</td>'."\n";
	echo '        </tr>'."\n";
	echo '      </table>'."\n";
	echo '</td>'."\n";
	echo '  </tr>'."\n";
	echo '  <tr'._bgColor(2).'>'."\n";
	echo '	  <td'._tdcss(FALSE,FALSE,_sh(),2).'>'._suh($lang_new[$nuke_module_name]['DESCRIPTION']).'</td>'."\n";
	echo '  </tr>'."\n";	
	echo '  <tr'._bgColor(1).'>'."\n";
	...
	echo '	  <td'._tdcss(FALSE,FALSE,_sh(),2).'>'._suh($lang_new[$nuke_module_name]['FIXES']).'</td>'."\n";
	echo '  </tr>'."\n";	
	...
	echo '  </tr>'."\n";
	echo '  <tr'._bgColor(2).'>'."\n";
	echo '	  <td'._tdcss(FALSE,FALSE,_sh(),2).'>'._suh($lang_new[$nuke_module_name]['AUTHOR_DETAILS']).'</td>'."\n";
	echo '  </tr>'."\n";
	echo '  <tr'._bgColor(1).'>'."\n";
	echo '    <td'._tdcss('50%',FALSE,_sc()).'>'._sut($lang_new[$nuke_module_name]['AUTHOR']).'</td>'."\n";
	echo '    <td'._tdcss('50%',FALSE,_sc()).'>'._input('text','author','250px',$row['author']).'</td>'."\n";
	echo '  </tr>'."\n";
	echo '  <tr'._bgColor(1).'>'."\n";
	echo '    <td'._tdcss('50%',FALSE,_sc()).'>'._sut($lang_new[$nuke_module_name]['EMAIL']).'</td>'."\n";
	echo '    <td'._tdcss('50%',FALSE,_sc()).'>'._input('email','author_email','250px',$row['author_email']).'</td>'."\n";
	echo '  </tr>'."\n";
	echo '  <tr'._bgColor(1).'>'."\n";
	echo '    <td'._tdcss('50%',FALSE,_sc()).'>'._sut($lang_new[$nuke_module_name]['WEBSITE']).'</td>'."\n";
...
//---------------------------------------------------------------------
	echo '  <tr>'."\n";
	echo '	  <td'._tdcss(FALSE,FALSE,_sh(),2).'>'._suh($lang_new[$nuke_module_name]['DETAILS']).'</td>'."\n";
	echo '  </tr>'."\n";
	echo '  <tr'._bgColor(1).'>'."\n";
	echo '    <td'._tdcss('50%',FALSE,_sc()).'>'._sut($lang_new[$nuke_module_name]['ISALLOWED']).'</td>'."\n";
	echo '    <td'._tdcss('50%',FALSE,_sc()).'>'._list_available_permission_groups('isallowed',$row['groups']).'</td>'."\n";
	echo '  </tr>'."\n";
	echo '  <tr'._bgColor(1).'>'."\n";
	echo '    <td'._tdcss('50%',FALSE,_sc()).'>'._sut($lang_new[$nuke_module_name]['POSTS']).'</td>'."\n";
	echo '    <td'._tdcss('50%',FALSE,_sc()).'>'._input('text','posts','100px',$row['posts']).'</td>'."\n";
	echo '  </tr>'."\n";
	echo '  <tr'._bgColor(1).'>'."\n";
	echo '    <td'._tdcss('50%',FALSE,_sc()).'>'._sut($lang_new[$nuke_module_name]['HITS']).'</td>'."\n";
	...
	if ( $_GET['action'] != 'newfile' ):
		...
		echo '    <td'._tdcss('50%',FALSE,_sc()).'>'._sut($lang_new[$nuke_module_name]['ISUPDATED']).'</td>'."\n";
		echo '    <td'._tdcss('50%',FALSE,_sc()).'>'._selectbox('isupdated',array(0 => $lang_new[$nuke_module_name]['N'], 1 => $lang_new[$nuke_module_name]['Y']),0).'</td>'."\n";
		echo '  </tr>'."\n";
		# added 1.1.0
		echo '  <tr'._bgColor(1).'>'."\n";
		echo '    <td'._tdcss('50%',FALSE,_sc()).'>'._sut($lang_new[$nuke_module_name]['APPROVED']).'</td>'."\n";
		echo '    <td'._tdcss('50%',FALSE,_sc()).'>'._selectbox('isapproved',array(0 => $lang_new[$nuke_module_name]['N'], 1 => $lang_new[$nuke_module_name]['Y']),$row['isapproved']).'</td>'."\n";
		echo '  </tr>'."\n";
		# added in 1.1.0

	endif;		
	echo '  <tr'._bgColor(1).'>'."\n";
	echo '    <td'._tdcss('50%',FALSE,_sc()).'>'._sut($lang_new[$nuke_module_name]['ACTIVE']).'</td>'."\n";
	echo '    <td'._tdcss('50%',FALSE,_sc()).'>'._selectbox('isactive',array(0 => $lang_new[$nuke_module_name]['N'], 1 => $lang_new[$nuke_module_name]['Y']),$row['isactive']).'</td>'."\n";
	echo '  </tr>'."\n";

	// if(file_exists(_FILE_REPOSITORY_PLUGINS.'shop/functions.php'))
	// {
	// 	echo '  <tr>'."\n";
	// 	echo '	  <td'._tdcss(FALSE,FALSE,_sh(),2).'>'._suh($lang_new[$nuke_module_name]['PAYPAL']).'</td>'."\n";
	// 	echo '  </tr>'."\n";
	// 	echo '  <tr'._bgColor(1).'>'."\n";
	// 	echo '    <td'._tdcss('50%',FALSE,_sc()).'>'._sut($lang_new[$nuke_module_name]['ACCOUNT']).'</td>'."\n";
	// 	echo '    <td'._tdcss('50%',FALSE,_sc()).'>'._input('text','paypal','250px',$row['paypal']).'</td>'."\n";
	// 	echo '  </tr>'."\n";
	// 	echo '  <tr'._bgColor(1).'>'."\n";
	// 	echo '    <td'._tdcss('50%',FALSE,_sc()).'>'._sut($lang_new[$nuke_module_name]['PRICE']).'</td>'."\n";
	// 	echo '    <td'._tdcss('50%',FALSE,_sc()).'>'._input('text','price','100px',$row['price']).'&nbsp;'._selectbox('currency',array('AUD' => $lang_new[$nuke_module_name]['AUD'], 'CAD' => $lang_new[$nuke_module_name]['CAD'], 'EUR' => $lang_new[$nuke_module_name]['EUR'], 'GBP' => $lang_new[$nuke_module_name]['GBP'], 'USD' => $lang_new[$nuke_module_name]['USD']),$row['currency']).'</td>'."\n";
	// 	echo '  </tr>'."\n";
	// }
//---------------------------------------------------------------------
	echo '  <tr'._bgColor(2).'>'."\n";
	echo '    <td'._tdcss(FALSE,'center',_sf(),2).'>'._submit($lang_new[$nuke_module_name]['SAVE']).'</td>'."\n";
	echo '  </tr>'."\n";
	echo '</table>'."\n";
	echo '</form>'."\n";
}
...
{
	global $nuke_db, $admin_file, $nuke_module_name, $nuke_userinfo, $settings;
...
//-------------------------------------------------------------------------------
	if (!$_SERVER['HTTP_X_REQUESTED_WITH'])
		die(json_encode(array('error' => $lang_new[$nuke_module_name]['ERROR_NICE_TRY'])));
//-------------------------------------------------------------------------------	
//-------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------	
	...
	// 	die(json_encode(array('error' => $lang_new[$nuke_module_name]['ERROR_NOT_SUPPORTED'])));
//-------------------------------------------------------------------------------	
//-------------------------------------------------------------------------------
//	On upload give the file a random name to prevent doubling of files
//-------------------------------------------------------------------------------
...
function _file_repository_client_uploads()
{
	...
	echo '    <td'._tdcss('40%',FALSE,_sh()).'>'._suh($lang_new[$nuke_module_name]['ITEM']).'</td>'."\n";
	echo '    <td'._tdcss('30%','center',_sh()).'>'._suh($lang_new[$nuke_module_name]['SUBMITTER_DETAILS']).'</td>'."\n";
	echo '    <td'._tdcss('20%','center',_sh()).'>'._suh($lang_new[$nuke_module_name]['SUBMITTED']).'</td>'."\n";
	echo '    <td'._tdcss('5%','center',_sh()).'>'._suh($lang_new[$nuke_module_name]['OPTION']).'</td>'."\n";
	echo '  </tr>'."\n";

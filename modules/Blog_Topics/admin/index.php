...
{
    global $prefix, $nuke_db, $admin_file, $tipath;
    ...
    $count = 0;
    $result = $nuke_db->sql_query("SELECT topicid, topicname, topicimage, topictext from " . $prefix . "_topics order by topicname");
...
function topicedit($topicid) 
{
    ...
    $query = $nuke_db->sql_query("SELECT topicid, topicname, topicimage, topictext from ".$prefix . "_topics where topicid='$topicid'");
    list($topicid, $topicname, $topicimage, $topictext) = $nuke_db->sql_fetchrow($query);
    ...
    $res = $nuke_db->sql_query("SELECT rid, name, url from ".$prefix . "_related where tid='$topicid'");
    $num = $nuke_db->sql_numrows($res);
    
	if ($num == 0) 
    echo "<tr><td><span class=\"tiny\">"._NORELATED . "</span></td></tr>";
    
        while($row2 = $nuke_db->sql_fetchrow($res)):
            $rid = intval($row2['rid']);
            $name = $row2['name'];
...
function relatededit($tid, $rid) {
   ...
   // OpenTable();
    ...
    $rid = intval($rid);
    $tid = intval($tid);
    $row = $nuke_db->sql_fetchrow($nuke_db->sql_query("SELECT name, url from ".$prefix . "_related where rid='$rid'"));
        $name = $row['name'];
        $url = $row['url'];
    $row2 = $nuke_db->sql_fetchrow($nuke_db->sql_query("SELECT topictext, topicimage from ".$prefix . "_topics where topicid='$tid'"));
...
function relatedsave($tid, $rid, $name, $url) {
    global $prefix, $nuke_db, $admin_file;
    $rid = intval($rid);
    $nuke_db->sql_query("update ".$prefix . "_related set name='$name', url='$url' where rid='$rid'");
    nuke_redirect($admin_file.".php?op=topicedit&topicid=$tid");
}

function relateddelete($tid, $rid) {
    global $prefix, $nuke_db, $admin_file;
    $rid = intval($rid);
    $nuke_db->sql_query("delete from ".$prefix . "_related where rid='$rid'");
    nuke_redirect($admin_file.".php?op=topicedit&topicid=$tid");
}
...
function topicmake($topicname, $topicimage, $topictext) {
    ...
    $nuke_db->sql_query("INSERT INTO ".$prefix . "_topics VALUES (NULL,'$topicname','$topicimage','$topictext','0')");
    nuke_redirect($admin_file.".php?op=topicsmanager#Add");
}
...
function topicchange($topicid, $topicname, $topicimage, $topictext, $name, $url) {
    ...
    $url = Fix_Quotes($url);
    $topicid = intval($topicid);
    $nuke_db->sql_query("update ".$prefix . "_topics set topicname='$topicname', topicimage='$topicimage', topictext='$topictext' where topicid='$topicid'");
    if (!$name) {
    } else {
        $nuke_db->sql_query("insert into ".$prefix . "_related VALUES (NULL, '$topicid','$name','$url')");
    }
    nuke_redirect($admin_file.".php?op=topicedit&topicid=$topicid");
}

function topicdelete($topicid, $ok=0) {
    global $prefix, $nuke_db, $ne_config, $admin_file;
    $topicid = intval($topicid);
    if ($ok==1) {
    $row = $nuke_db->sql_fetchrow($nuke_db->sql_query("SELECT sid from " . $prefix . "_stories where topic='$topicid'"));
        $sid = intval($row['sid']);
        // Copyright (c) 2000-2005 by NukeScripts Network
        if($ne_config['hometopic'] == $topicid) { ne_save_config("hometopic", "0"); }
        // Copyright (c) 2000-2005 by NukeScripts Network
        $nuke_db->sql_query("delete from " . $prefix . "_stories where topic='$topicid'");
        $nuke_db->sql_query("delete from " . $prefix . "_topics where topicid='$topicid'");
        $nuke_db->sql_query("delete from " . $prefix . "_related where tid='$topicid'");
    $row2 = $nuke_db->sql_fetchrow($nuke_db->sql_query("SELECT sid from " . $prefix . "_comments where sid='$sid'"));
        $sid = intval($row2['sid']);
        $nuke_db->sql_query("delete from " . $prefix . "_comments where sid='$sid'");
        nuke_redirect($admin_file.".php?op=topicsmanager");
    } else {
        global $topicimage;
        include(NUKE_BASE_DIR."header.php");
        OpenTable();
    ...
    $row3 = $nuke_db->sql_fetchrow($nuke_db->sql_query("SELECT topicimage, topictext from " . $prefix . "_topics where topicid='$topicid'"));

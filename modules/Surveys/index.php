...
function pollLatest() {
    ...
    $result = $nuke_db->sql_query("SELECT pollID FROM ".$prefix."_poll_desc WHERE artid='0' $querylang ORDER BY pollID DESC LIMIT 1");
    $pollID = $nuke_db->sql_fetchrow($result);
    $nuke_db->sql_freeresult($result);
    return($pollID[0]);
}

function pollCollector($pollID, $voteID, $forwarder) {
    global $prefix, $nuke_db, $evoconfig, $identify;
    $ip = $identify->get_ip();
    $number_of_days = intval($evoconfig['poll_days']);
    $past = time()-86400*$number_of_days;
    $nuke_db->sql_query("DELETE FROM ".$prefix."_poll_check WHERE time < $past");
    $result = $nuke_db->sql_query("SELECT ip FROM ".$prefix."_poll_check WHERE ip='$ip' AND pollID='$pollID'");
    list($ips) = $nuke_db->sql_fetchrow($result);
    $nuke_db->sql_freeresult($result);
    if ($ip != $ips) {
        $ctime = time();
        $nuke_db->sql_query("INSERT INTO ".$prefix."_poll_check (ip, time, pollID) VALUES ('$ip', '$ctime', '$pollID')");
        $nuke_db->sql_query("UPDATE ".$prefix."_poll_data SET optionCount=optionCount+1 WHERE pollID='$pollID' AND voteID='$voteID'");
        if (!empty($voteID)) {
            $nuke_db->sql_query("UPDATE ".$prefix."_poll_desc SET voters=voters+1 WHERE pollID='$pollID'");
        }
    }
    nuke_redirect("$forwarder");
    exit;
}
...
function pollResults($pollID) {
    ...
    $holdtitle = $nuke_db->sql_fetchrow($result);
    $nuke_db->sql_freeresult($result);
    echo "<strong>$holdtitle[1]</strong><br /><br />";

    $result = $nuke_db->sql_query("SELECT SUM(optionCount) FROM ".$prefix."_poll_data WHERE pollID='$pollID'");
    list($sum) = $nuke_db->sql_fetchrow($result);
    $nuke_db->sql_freeresult($result);
    echo "<table border=\"0\">";

    /* cycle through all options */
    $result = $nuke_db->sql_query("SELECT optionText, optionCount FROM ".$prefix."_poll_data WHERE pollID='$pollID' AND optionText!='' ORDER BY voteID");
    while(list($optionText, $optionCount) = $nuke_db->sql_fetchrow($result)) {

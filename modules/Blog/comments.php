...
function modone() {
    ...
    if(((isset($admin)) && ($moderate == 1)) || ($moderate==2)) echo "<form action=\"modules.php?name=$nuke_module_name&file=comments\" method=\"post\">";
}
...
{
    global $nuke_user, $bgcolor1, $bgcolor2, $textcolor1, $textcolor2, $anonpost, $prefix, $nuke_db, $nuke_module_name;
    ...
    if(!$query) {
        ...
        $count = $nuke_db->sql_numrows($query);
    }
    $nuke_db->sql_freeresult($query);

    $sid = intval($sid);

    $query = $nuke_db->sql_query("SELECT title FROM ".$prefix."_stories WHERE sid='$sid'");

    list($un_title) = $nuke_db->sql_fetchrow($query);
...
function DisplayKids ($tid, $mode, $order=0, $thold=0, $level=0, $dummy=0, $tblwidth=99) {
	...
	global $datetime, $nuke_user, $cookie, $bgcolor1, $reasons, $anonymous, $anonpost, $commentlimit, $prefix, $textcolor2, $nuke_db, $nuke_module_name, $nuke_user_prefix, $nuke_userinfo;
                         ...
                         $result = $nuke_db->sql_query("SELECT `tid`, 
												...
												`datePublished`, 
												 `dateModified`, 
													...
													FROM ".$prefix."_comments WHERE pid='$tid' ORDER BY datePublished, tid");
    ...
    while ($row = $nuke_db->sql_fetchrow($result)) 
	{
        ...
        if((isset($nuke_userinfo['commentmax'])) && (strlen($r_comment) > $nuke_userinfo['commentmax'])) echo substr("$r_comment", 0, $nuke_userinfo['commentmax'])."<br /><br /><strong><a href=\"modules.php?name=$nuke_module_name&amp;file=comments&amp;sid=$r_sid&amp;tid=$r_tid&amp;mode=$mode&amp;order=$order&amp;thold=$thold\">"._READREST."</a></strong>";
        elseif(strlen($r_comment) > $commentlimit) echo substr("$r_comment", 0, $commentlimit)."<br /><br /><strong><a href=\"modules.php?name=$nuke_module_name&amp;file=comments&amp;sid=$r_sid&amp;tid=$r_tid&amp;mode=$mode&amp;order=$order&amp;thold=$thold\">"._READREST."</a></strong>";
        else echo $r_comment;
        echo "</td></tr></table><br /><br />";
        if ($anonpost==1 OR is_mod_admin($nuke_module_name) OR is_user()) {
            echo "<font class=\"content\" color=\"$textcolor2\"> [ <a href=\"modules.php?name=$nuke_module_name&amp;file=comments&amp;op=Reply&amp;pid=$r_tid&amp;sid=$r_sid&amp;mode=$mode&amp;order=$order&amp;thold=$thold\">"._REPLY."</a>";
        }
       ...
       while ($row = $nuke_db->sql_fetchrow($result)) {
        ...
        if($r_score >= $thold) {
			...
			echo "<li><font class=\"content\" color=\"$textcolor2\"><a href=\"modules.php?name=$nuke_module_name&amp;file=comments&amp;op=showreply&amp;tid=$r_tid&amp;sid=$r_sid&amp;pid=$r_pid&amp;mode=$mode&amp;order=$order&amp;thold=$thold#$r_tid\">$r_subject</a> "._BY." $r_name "._ON." $r_date</font><br />";
...
function DisplayBabies ($tid, $level=0, $dummy=0) {
    global $datetime, $anonymous, $prefix, $nuke_db, $nuke_module_name;
    $comments = 0;
                         $result = $nuke_db->sql_query("SELECT `tid`, 
	                                                      `pid`, 
														  `sid`, 
												`datePublished`, 
												 `dateModified`, 
												    ...
												    `host_name`, 
													  ...
													  `comment`, 
													    `score`, 
													   `reason` FROM ".$prefix."_comments WHERE pid='$tid' ORDER BY date, tid");
	...
	echo "<a href=\"modules.php?name=$nuke_module_name&amp;file=comments&amp;op=showreply&amp;tid=$r_tid&amp;mode=$mode&amp;order=$order&amp;thold=$thold\">$r_subject</a></font><font class=\"content\"> "._BY." $r_name "._ON." $r_date<br />";
...
function DisplayTopic ($sid, $pid=0, $tid=0, $mode="thread", $order=0, $thold=0, $level=0, $nokids=0) 
{
	...
	$acomm, $articlecomm, $nuke_db, $nuke_module_name, $nukeurl, $admin_file, $nuke_user_prefix, $nuke_userinfo, $cookie;
	...
	FROM ".$prefix."_comments WHERE sid='$sid' and pid='$pid'";
    ...
    if(is_mod_admin($nuke_module_name)) {
        $row3 = $nuke_db->sql_fetchrow($nuke_db->sql_query("SELECT host_name FROM ".$prefix."_comments WHERE tid='$tid'"));
        $host_name = $row3["host_name"];
        echo "<br /><strong>(IP: $host_name)</strong>";
    }
    echo "</font></td></tr><tr><td>";
    if((isset($nuke_userinfo['commentmax'])) && (strlen($r_comment) > $nuke_userinfo['commentmax'])) echo substr("$r_comment", 0, $nuke_userinfo['commentmax'])."<br /><br /><strong><a href=\"modules.php?name=$nuke_module_name&amp;file=comments&amp;sid=$r_sid&amp;tid=$r_tid&amp;mode=$mode&amp;order=$order&amp;thold=$thold\">"._READREST."</a></strong>";
    elseif(strlen($comment) > $commentlimit) echo substr("$comment", 0, $commentlimit)."<br /><br /><strong><a href=\"modules.php?name=$nuke_module_name&amp;file=comments&amp;sid=$sid&tid=$tid&mode=$mode&order=$order&thold=$thold\">"._READREST."</a></strong>";
    else echo $comment;
    echo "</td></tr></table><br /><br />";
    if ($anonpost==1 OR is_mod_admin($nuke_module_name) OR is_user()) {
        echo "<font class=\"content\"> [ <a href=\"modules.php?name=$nuke_module_name&amp;file=comments&amp;op=Reply&amp;pid=$tid&amp;sid=$sid&amp;mode=$mode&amp;order=$order&amp;thold=$thold\">"._REPLY."</a>";
    }
    if ($pid != 0) {
        $row4 = $nuke_db->sql_fetchrow($nuke_db->sql_query("SELECT pid FROM ".$prefix."_comments WHERE tid='$pid'"));
        $erin = intval($row4["pid"]);
        echo " | <a href=\"modules.php?name=$nuke_module_name&amp;file=comments&amp;sid=$sid&amp;pid=$erin&amp;mode=$mode&amp;order=$order&amp;thold=$thold\">"._PARENT."</a>";
    }
    ...
    $nuke_db->sql_freeresult($something);
...
{
    global $nuke_module_name, $nuke_user, $cookie, $datetime, $bgcolor1, $bgcolor2, $bgcolor3, $bgcolor4, $admin, $anonpost, $prefix, $textcolor2, $nuke_db;

    include_once(NUKE_BASE_DIR."header.php");

    $row = $nuke_db->sql_fetchrow($nuke_db->sql_query("SELECT datePublished, dateModified, name, email, subject, comment, score, reason FROM ".$prefix."_comments WHERE tid='$tid' AND sid='$sid'"));
    ...
    if ($anonpost==1 OR is_mod_admin($nuke_module_name) OR is_user()) {
    echo "<font class=content> [ <a href=\"modules.php?name=$nuke_module_name&amp;file=comments&amp;op=Reply&amp;pid=$tid&amp;sid=$sid&amp;mode=$mode&amp;order=$order&amp;thold=$thold\">"._REPLY."</a> | <a href=\"modules.php?name=$nuke_module_name&amp;file=article&amp;sid=$sid&mode=$mode&order=$order&thold=$thold\">"._ROOT."</a>";
    }
    modtwo($tid, $score, $reason);
    echo " ]";
    modthree($sid, $mode, $order, $thold);
...
function reply($pid, $sid, $mode, $order, $thold) 
{
    ...
    global $nuke_module_name, $nuke_user, $cookie, $bgcolor1, $bgcolor2, $bgcolor3, $nuke_db, $anonpost, $anonymous, $admin, $AllowableHTML;

    if ($anonpost == 0 AND !is_user() AND !is_mod_admin($nuke_module_name)) {
    ...
    if ($pid != 0) 
	{
        $row = $nuke_db->sql_fetchrow($nuke_db->sql_query("SELECT datePublished, dateModified, name, email, subject, comment, score FROM ".$prefix."_comments WHERE tid='$pid'"));
	...
	{
        $row2 = $nuke_db->sql_fetchrow($nuke_db->sql_query("SELECT datePublished, dateModified, title, hometext, bodytext, informant, notes FROM ".$prefix."_stories WHERE sid='$sid'"));
    ...
    if ($pid == 0) {
        $row3 = $nuke_db->sql_fetchrow($nuke_db->sql_query("SELECT title FROM ".$prefix."_stories WHERE sid='$sid'"));
        $subject = stripslashes(check_html($row3["title"], "nohtml"));
    } else {
        $row4 = $nuke_db->sql_fetchrow($nuke_db->sql_query("SELECT subject FROM ".$prefix."_comments WHERE tid='$pid'"));
...
function replyPreview ($pid, $sid, $subject, $comment, $xanonpost, $mode, $order, $thold, $posttype) 
{
    ...
    global $nuke_module_name, $nuke_user, $cookie, $AllowableHTML, $anonymous, $anonpost;
    ...
    //echo "<br />";
    OpenTable();
    echo "<form action=\"modules.php?name=$nuke_module_name&amp;file=comments\" method=\"post\"><font class=\"option\"><strong>"._YOURNAME.":</strong></font> ";
    if (is_user()) {
        echo "<a href=\"modules.php?name=Your_Account\">$cookie[1]</a> <font class=\"content\">[ <a href=\"modules.php?name=Your_Account&amp;op=logout\">"._LOGOUT."</a> ]</font><br /><br />";
    } else {
...
function CreateTopic ($xanonpost, $subject, $comment, $pid, $sid, $host_name, $mode, $order, $thold, $posttype) 
{
	...
	global $nuke_module_name, $nuke_user, $nuke_userinfo, $EditedMessage, $cookie, $AllowableHTML, $ultramode, $prefix, $anonpost, $articlecomm, $nuke_db;
    ...
    $ip = identify::get_ip();
    $fakeresult = $nuke_db->sql_query("SELECT acomm FROM ".$prefix."_stories WHERE sid='$sid'");
    $fake = $nuke_db->sql_numrows($fakeresult);
    $comment = trim($comment);
    
	if (($fake == 1) AND ($articlecomm == 1)) 
	{
      $fakerow = $nuke_db->sql_fetchrow($fakeresult);
	...
	if (((($anonpost == 0) AND (is_user())) OR ($anonpost == 1)) AND ($acomm == 0)) 
	{
	  ...
	  $nuke_db->sql_query("INSERT INTO `".$prefix."_comments` (`tid`, 
	                                                      `pid`, 
														  `sid`, 
												`datePublished`, 
												 `dateModified`, 
	  ...
	  $nuke_db->sql_query("UPDATE ".$prefix."_stories SET comments=comments+1 WHERE sid='$sid'");
   ...
   else 
   {
    ...
    $nuke_db->sql_freeresult($fakeresult);
    ...
    nuke_redirect("modules.php?name=$nuke_module_name&file=article&sid=$sid$options");
}
...
switch($op) {
    ...
    nuke_redirect("modules.php?name=$nuke_module_name&file=article&sid=$sid&mode=$mode&order=$order&thold=$thold");
    break;

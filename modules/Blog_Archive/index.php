...
function select_month() 
{
	...
	$result = $nuke_db->sql_query("SELECT datePublished FROM ".$prefix."_stories ORDER BY datePublished DESC");
    
	echo "<ul>";

    $thismonth = '';
    
	while(list($time) = $nuke_db->sql_fetchrow($result)) 
	{
...
function show_month($year, $month, $month_l) 
{
	...
	$result = $nuke_db->sql_query("SELECT sid, 
	                               catid, 
								   title, 
						   datePublished, 
						    dateModified, 
	...
	while ($row = $nuke_db->sql_fetchrow($result)) 
	{
		...
		elseif ($catid != 0) 
		{
            $row_res = $nuke_db->sql_fetchrow($nuke_db->sql_query("SELECT title FROM ".$prefix."_stories_cat WHERE catid='$catid'"));
	...
	$result2 = $nuke_db->sql_query("SELECT datePublished FROM ".$prefix."_stories ORDER BY datePublished DESC");
...
function show_all($min) 
{
	...
	$result = $nuke_db->sql_query("SELECT sid, catid, title, datePublished, dateModified, comments, counter, topic, alanguage, score, ratings FROM ".$prefix."_stories ORDER BY sid DESC LIMIT $min,$max");
    
	$numrows = $nuke_db->sql_numrows($nuke_db->sql_query("select * FROM ".$prefix."_stories"));

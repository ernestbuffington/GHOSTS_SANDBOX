...
{
    global $nuke_module_name, $nuke_userinfo, $customlang;
    include_once(NUKE_BASE_DIR.'header.php');
    if ( !get_query_var( 'recap', 'get', 'int' ) ):
    	title($customlang[$nuke_module_name]['recommend']);
    else:
    	/**
	 	 *	If recaptcha has failed, display the error header.
	 	 */
    	title($customlang[$nuke_module_name]['recaptcha_error']);
    endif;
    OpenTable();
	...
	if ( !is_user() ):
		...
		echo '<div class="center">'.sprintf($customlang[$nuke_module_name]['must_be_user'], '<a class="bbcode-href" href="modules.php?name=Your_Account">', '</a>').'</div>';
		CloseTable();
	...
	if ( get_query_var( 'recap', 'get', 'int' ) == 1 ):
		...
		echo '<div class="col-12 center"><h2 style="color:#FF0000 !important;">'.$customlang[$nuke_module_name]['recaptcha_failed'].'</h2><br />[ <a href="modules.php?name=Recommend_Us">'.$customlang[$nuke_module_name]['goback'].'</a> ]</div>';
		CloseTable();
		include_once(NUKE_BASE_DIR.'footer.php');
	...
	<div class="col-10" style="display: table; border-collapse: separate; border-spacing: 10px; margin: 0 auto">
	  ...
	  <div class="col-12" style="display: table-row;">	    	
        <div class="col-6" style="display: table-cell;"><?php echo $customlang[$nuke_module_name]['your_name'] ?></div>
        <div class="col-6" style="display: table-cell;"><input style="width: 98%" type="text" name="yname" value="<?php echo $your_name ?>" required></div>
	  </div>
	  <div class="col-12" style="display: table-row;">	    	
        <div class="col-6" style="display: table-cell;"><?php echo $customlang[$nuke_module_name]['your_mail'] ?></div>
	  ...
	  <div class="col-12" style="display: table-row;">	    	
        <div class="col-6" style="display: table-cell;"><?php echo $customlang[$nuke_module_name]['friend_name'] ?></div>
        <div class="col-6" style="display: table-cell;"><input style="width: 98%" type="text" name="fname" value="" required></div>
	  </div>
	  <div class="col-12" style="display: table-row;">	    	
        <div class="col-6" style="display: table-cell;"><?php echo $customlang[$nuke_module_name]['friend_mail'] ?></div>
        <div class="col-6" style="display: table-cell;"><input style="width: 98%" type="email" name="fmail" value="" required></div>
	  </div>

	  <div class="col-12" style="display: table-row;">	    	
        <div class="col-6" style="display: table-cell; vertical-align: top;"><?php echo $customlang[$nuke_module_name]['message'] ?><span class="evo-sprite help tooltip float-right" title="<?php echo $customlang[$nuke_module_name]['optional'] ?>"></span></div>
	...
	?>
	  ...
	  <div class="col-12 center" style="padding-top: 10px;"><input type="submit" value="<?php echo $customlang[$nuke_module_name]['send'] ?>"></div>
	</form>
...
function SendSite() 
{
    global $sitename, $slogan, $nukeurl, $nuke_module_name, $customlang;
    ...
    $subject  = $customlang[$nuke_module_name]['interesting']." $sitename ".$customlang[$nuke_module_name]['from']." $yname";
	$message  = $customlang[$nuke_module_name]['hello']." $fname<br /><br />";
	$message .= $customlang[$nuke_module_name]['your_friend'].": $yname ".$customlang[$nuke_module_name]['our_site']." $sitename ".$customlang[$nuke_module_name]['interest_sent']."<br /><br />";
	$message .= $customlang[$nuke_module_name]['sitename'].": $sitename<br />$slogan<br />";
	$message .= $customlang[$nuke_module_name]['siteurl'].": <a href=\"$nukeurl\">$nukeurl</a><br /><br />";
	if ( $short_message ):
		$message .= $customlang[$nuke_module_name]['why_i_recommend'].": <br /><br /> $short_message";
	endif;
	...
	if ( !security_code_check($_POST['g-recaptcha-response'], $gfxchk) ):
    	 ...
    	 */
		nuke_redirect("modules.php?name=$nuke_module_name&recap=1");
	elseif (empty($fname) || empty($fmail) || empty($yname) || empty($ymail)):
    	 ...
    	 */
		nuke_redirect("modules.php?name=$nuke_module_name&op=SiteSent&fname=$fname");
...
{	
    include_once(NUKE_BASE_DIR.'header.php');
    global $nuke_module_name, $customlang;
    $fname = get_query_var( 'fname', 'get' );
    OpenTable();
    echo '<div class="col-12 center">'.$customlang[$nuke_module_name]['reference'].' '.$fname.'...<br /><br />'.$customlang[$nuke_module_name]['thank_you'].'</div>';
    CloseTable();

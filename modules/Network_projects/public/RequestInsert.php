...
if($project['allowrequests'] > 0) {
  ...
  if(empty($stop)) {
    ...
    $network_db->sql_query("INSERT INTO `".$network_prefix."_requests` VALUES (NULL, '$project_id', '$type_id', '$status_id', '$request_name', '$request_description', '$submitter_name', '$submitter_email', '$submitter_ip', '$date', '0', '0')");
    list($request_id) = $network_db->sql_fetchrow($network_db->sql_query("SELECT `request_id` FROM `".$network_prefix."_requests` WHERE `date_submitted`='$date' AND `project_id`='$project_id' AND `type_id`='$type_id' AND `status_id`='$status_id' AND `request_name`='$request_name'"));

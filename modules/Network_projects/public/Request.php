...
if($project['allowrequests'] > 0) {
  ...
  echo "<td colspan='3' width='100%'><nobr><a href='modules.php?name=$nuke_module_name&amp;op=Project&amp;project_id=".$project['project_id']."'>".$project['project_name']."</a></nobr></td></tr>\n";
  ...
  $pjimage = pjimage("requester.png", $nuke_module_name);
  echo "<tr><td align='center'><img src='$pjimage'></td>\n";
  ...
  $memberresult = $network_db->sql_query("SELECT `member_id`, `position_id` FROM `".$network_prefix."_requests_members` WHERE `request_id`='$request_id' ORDER BY `member_id`");
  ...
  CloseTable();
  //echo "<br />\n";
  $commentresult = $network_db->sql_query("SELECT `comment_id` FROM `".$network_prefix."_requests_comments` WHERE `request_id`='$request_id' ORDER BY `date_commented` asc");
  $comment_total = $network_db->sql_numrows($commentresult);
  OpenTable();
  echo "<table border='1' cellpadding='2' cellspacing='0' width='100%'>\n";
  echo "<tr><td bgcolor='$bgcolor2' width='100%'><nobr><strong>"._NETWORK_COMMENTS."</strong> <strong>(</strong> <a href='modules.php?name=$nuke_module_name&amp;op=RequestCommentSubmit&amp;request_id=$request_id'>"._NETWORK_COMMENTADD."</a> <strong>)</strong></nobr></td><tr>\n";
  if($comment_total > 0){

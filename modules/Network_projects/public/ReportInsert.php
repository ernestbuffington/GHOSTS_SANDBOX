...
if($project['allowreports'] > 0) {
  ...
  if(empty($stop)) {
    ...
    $network_db->sql_query("INSERT INTO `".$network_prefix."_reports` VALUES (NULL, '$project_id', '$type_id', '$status_id', '$report_name', '$report_description', '$submitter_name', '$submitter_email', '$submitter_ip', '$date', '0', '0')");
    list($report_id) = $network_db->sql_fetchrow($network_db->sql_query("SELECT `report_id` FROM `".$network_prefix."_reports` WHERE `date_submitted`='$date' AND `project_id`='$project_id' AND `type_id`='$type_id' AND `status_id`='$status_id' AND `report_name`='$report_name'"));

...
if($project['allowrequests'] > 0) {
  ...
  if(empty($stop)) {
    ...
    $network_db->sql_query("INSERT INTO `".$network_prefix."_requests_comments` VALUES (NULL, '$request_id', '$commenter_name', '$commenter_email', '$commenter_ip', '$comment_description', '$date')");
    $network_db->sql_query("UPDATE `".$network_prefix."_requests` SET `date_commented`='$date' WHERE `request_id`='$request_id'");

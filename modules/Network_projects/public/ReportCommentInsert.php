...
if($project['allowreports'] > 0) {
  ...
  if(empty($stop)) {
    ...
    $network_db->sql_query("INSERT INTO `".$network_prefix."_reports_comments` VALUES (NULL, '$report_id', '$commenter_name', '$commenter_email', '$commenter_ip', '$comment_description', '$date')");
    $network_db->sql_query("UPDATE `".$network_prefix."_reports` SET `date_commented`='$date' WHERE `report_id`='$report_id'");

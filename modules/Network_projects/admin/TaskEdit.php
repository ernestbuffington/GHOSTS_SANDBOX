...
$pagetitle = "::: "._NETWORK_TITLE." ".$pj_config['version_number']."::: "._NETWORK_TASKS.": "._NETWORK_EDITTASK;
...
$prioritylist = $network_db->sql_query("SELECT `priority_id`, `priority_name` FROM `".$network_prefix."_tasks_priorities` ORDER BY `priority_weight`");
while(list($s_priority_id, $s_priority_name) = $network_db->sql_fetchrow($prioritylist)){
...
$statuslist = $network_db->sql_query("SELECT `status_id`, `status_name` FROM `".$network_prefix."_tasks_status` ORDER BY `status_weight`");
...
while(list($member_id, $member_name) = $network_db->sql_fetchrow($memberlistresult)) {
  $memberexresult = $network_db->sql_query("SELECT `member_id` FROM `".$network_prefix."_tasks_members` WHERE `member_id`='$member_id' AND `task_id`='$task_id'");
...
$membersresult = $network_db->sql_query("SELECT `member_id`, `position_id` FROM `".$network_prefix."_tasks_members` WHERE `task_id`='$task_id'");

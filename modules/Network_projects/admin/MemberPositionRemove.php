...
$positionlist = $network_db->sql_query("SELECT `position_id`, `position_name` FROM `".$network_prefix."_members_positions` WHERE `position_id` != '$position_id' AND `position_id` > 0 ORDER BY `position_weight`");
while(list($s_position_id, $s_position_name) = $network_db->sql_fetchrow($positionlist)){

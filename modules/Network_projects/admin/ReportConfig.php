...
$pagetitle = "::: "._NETWORK_TITLE." ".$pj_config['version_number']."::: "._NETWORK_REPORTS.": "._NETWORK_CONFIG;
...
$status = $network_db->sql_query("SELECT `status_id`, `status_name` FROM `".$network_prefix."_reports_status` ORDER BY `status_weight`");
while(list($status_id, $status_name) = $network_db->sql_fetchrow($status)) {
...
$type = $network_db->sql_query("SELECT `type_id`, `type_name` FROM `".$network_prefix."_reports_types` ORDER BY `type_weight`");
while(list($type_id, $type_name) = $network_db->sql_fetchrow($type)) {
...
$position = $network_db->sql_query("SELECT `position_id`, `position_name` FROM `".$network_prefix."_members_positions` ORDER BY `position_name`");
while(list($position_id, $position_name) = $network_db->sql_fetchrow($position)) {

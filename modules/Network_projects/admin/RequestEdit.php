...
$pagetitle = "::: "._NETWORK_TITLE." ".$pj_config['version_number']."::: "._NETWORK_REQUESTS.": "._NETWORK_REQUESTEDIT;
...
while(list($p_project_id, $p_project_name) = $network_db->sql_fetchrow($projectlist)){
...
$typelist = $network_db->sql_query("SELECT `type_id`, `type_name` FROM `".$network_prefix."_requests_types` ORDER BY `type_weight`");
while(list($t_type_id, $t_type_name) = $network_db->sql_fetchrow($typelist)){
...
$statuslist = $network_db->sql_query("SELECT `status_id`, `status_name` FROM `".$network_prefix."_requests_status` ORDER BY `status_weight`");
while(list($s_status_id, $s_status_name) = $network_db->sql_fetchrow($statuslist)){
...
$memberlistresult = $network_db->sql_query("SELECT `member_id`, `member_name` FROM `".$network_prefix."_members` ORDER BY `member_name`");
while(list($member_id, $member_name) = $network_db->sql_fetchrow($memberlistresult)) {
    $memberexresult = $network_db->sql_query("SELECT `member_id` FROM `".$network_prefix."_requests_members` WHERE `member_id`='$member_id' AND `request_id`='$request_id'");
    $numrows = $network_db->sql_numrows($memberexresult);
...
$membersresult = $network_db->sql_query("SELECT `member_id`, `position_id` FROM `".$network_prefix."_requests_members` WHERE `request_id`='$request_id'");
$numrows = $network_db->sql_numrows($membersresult);
if($numrows > 0){
    while(list($member_id, $position_id) = $network_db->sql_fetchrow($membersresult)){
        ...
        while(list($l_position_id, $l_position_name) = $network_db->sql_fetchrow($positionlistresult)) {

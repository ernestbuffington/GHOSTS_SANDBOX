...
$memberlist = $network_db->sql_query("SELECT `member_id`, `member_name` FROM `".$network_prefix."_members` WHERE `member_id` != '$member_id' ORDER BY `member_name`");
while(list($s_member_id, $s_member_name) = $network_db->sql_fetchrow($memberlist)){

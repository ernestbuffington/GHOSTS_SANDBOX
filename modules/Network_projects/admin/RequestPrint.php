...
$memberresult = $network_db->sql_query("SELECT `member_id` FROM `".$network_prefix."_requests_members` WHERE `request_id`='$request_id' ORDER BY `member_id`");
...
if($member_total != 0){
  while(list($member_id) = $network_db->sql_fetchrow($memberresult)) {

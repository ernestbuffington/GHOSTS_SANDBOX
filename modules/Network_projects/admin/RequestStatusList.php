...
//echo "<br />";
$statusresult = $network_db->sql_query("SELECT * FROM `".$network_prefix."_requests_status` WHERE `status_weight` > 0 ORDER BY `status_weight`");
...
if($status_total != 0){
  while($status_row = $network_db->sql_fetchrow($statusresult)) {
    ...
    list($pid1) = $network_db->sql_fetchrow($network_db->sql_query("SELECT `status_id` FROM `".$network_prefix."_requests_status` WHERE `status_weight`='$weight1'"));

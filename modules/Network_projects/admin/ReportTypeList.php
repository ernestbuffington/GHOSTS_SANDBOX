...
//echo "<br />\n";
$typeresult = $network_db->sql_query("SELECT * FROM `".$network_prefix."_reports_types` WHERE `type_weight` > 0 ORDER BY `type_weight`");
$type_total = $network_db->sql_numrows($typeresult);
OpenTable();
...
if($type_total != 0){
  while($type_row = $network_db->sql_fetchrow($typeresult)) {
    ...
    list($pid1) = $network_db->sql_fetchrow($network_db->sql_query("SELECT `type_id` FROM `".$network_prefix."_reports_types` WHERE `type_weight`='$weight1'"));
    list($pid2) = $network_db->sql_fetchrow($network_db->sql_query("SELECT `type_id` FROM `".$network_prefix."_reports_types` WHERE `type_weight`='$weight3'"));
    echo "<td align='center'><nobr>";
    if($pid1 AND $pid1 > 0) {
      echo "<a href='".$admin_file.".php?op=ReportTypeOrder&amp;weight=".$type_row['type_weight']."&amp;pid=".$type_row['type_id']."&amp;weightrep=$weight1&amp;pidrep=$pid1'><img src='modules/$nuke_module_name/images/weight_up.png' border='0' hspace='3' alt='"._NETWORK_UP."' title='"._NETWORK_UP."'></a>";
    } else {
      echo "<img src='modules/$nuke_module_name/images/weight_up_no.png' border='0' hspace='3' alt='' title=''>";
    }
    if($pid2) {
      echo "<a href='".$admin_file.".php?op=ReportTypeOrder&amp;weight=".$type_row['type_weight']."&amp;pid=".$type_row['type_id']."&amp;weightrep=$weight3&amp;pidrep=$pid2'><img src='modules/$nuke_module_name/images/weight_dn.png' border='0' hspace='3' alt='"._NETWORKDOWN."' title='"._NETWORK_DOWN."'></a>";
    } else {

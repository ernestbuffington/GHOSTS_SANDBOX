...
$result = $network_db->sql_query("SELECT `weight` FROM `".$network_prefix."_projects` ORDER BY `weight` DESC");
if(!$result) {
  $weight = 1;
} else {
...
$network_db->sql_query("INSERT INTO `".$network_prefix."_projects` VALUES (NULL, '$project_name', '$project_description', '$project_site', '$priority_id', '$status_id', '$project_percent', '$weight', '$featured', '$allowreports', '$allowrequests', '$date', '$start_date', '$finish_date')");
$projectresult = $network_db->sql_query("SELECT `project_id` FROM `".$network_prefix."_projects` WHERE `date_created`='$date'");
list($project_id) = $network_db->sql_fetchrow($projectresult);
if(implode("", $member_ids) > "") {
  while(list($null, $member_id) = each($member_ids)) {
    $numrows = $network_db->sql_numrows($network_db->sql_query("SELECT * FROM `".$network_prefix."_projects_members` WHERE `project_id`='$project_id' AND `member_id`='$member_id'"));
    if($numrows == 0) {
      $network_db->sql_query("INSERT INTO `".$network_prefix."_projects_members` VALUES ('$project_id', '$member_id', '".$pj_config['new_project_position']."')");        
    }
  }
}

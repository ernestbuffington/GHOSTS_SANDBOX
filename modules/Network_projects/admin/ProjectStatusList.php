...
//echo "<br />\n";
$statusresult = $network_db->sql_query("SELECT * FROM `".$network_prefix."_projects_status` WHERE `status_weight` > 0 ORDER BY `status_weight`");
...
if($status_total != 0){
  while($status_row = $network_db->sql_fetchrow($statusresult)) {
    ...
    list($pid1) = $network_db->sql_fetchrow($network_db->sql_query("SELECT `status_id` FROM `".$network_prefix."_projects_status` WHERE `status_weight`='$weight1'"));
    list($pid2) = $network_db->sql_fetchrow($network_db->sql_query("SELECT `status_id` FROM `".$network_prefix."_projects_status` WHERE `status_weight`='$weight3'"));
    echo "<td align='center'><nobr>";
    if($pid1 AND $pid1 > 0) {
      echo "<a href='".$admin_file.".php?op=ProjectStatusOrder&amp;weight=".$status_row['status_weight']."&amp;pid=".$status_row['status_id']."&amp;weightrep=$weight1&amp;pidrep=$pid1'><img src='modules/$nuke_module_name/images/weight_up.png' border='0' hspace='3' alt='"._NETWORK_UP."' title='"._NETWORK_UP."'></a>";
    } else {
      echo "<img src='modules/$nuke_module_name/images/weight_up_no.png' border='0' hspace='3' alt='' title=''>";
    }
    if($pid2) {
      echo "<a href='".$admin_file.".php?op=ProjectStatusOrder&amp;weight=".$status_row['status_weight']."&amp;pid=".$status_row['status_id']."&amp;weightrep=$weight3&amp;pidrep=$pid2'><img src='modules/$nuke_module_name/images/weight_dn.png' border='0' hspace='3' alt='"._NETWORKDOWN."' title='"._NETWORK_DOWN."'></a>";
    } else {

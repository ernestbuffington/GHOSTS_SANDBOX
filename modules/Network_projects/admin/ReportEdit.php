...
$typelist = $network_db->sql_query("SELECT `type_id`, `type_name` FROM `".$network_prefix."_reports_types` ORDER BY `type_weight`");
...
$statuslist = $network_db->sql_query("SELECT `status_id`, `status_name` FROM `".$network_prefix."_reports_status` ORDER BY `status_weight`");
...
while(list($member_id, $member_name) = $network_db->sql_fetchrow($memberlistresult)) {
    $memberexresult = $network_db->sql_query("SELECT `member_id` FROM `".$network_prefix."_reports_members` WHERE `member_id`='$member_id' AND `report_id`='$report_id'");
...
$membersresult = $network_db->sql_query("SELECT `member_id`, `position_id` FROM `".$network_prefix."_reports_members` WHERE `report_id`='$report_id'");
$memrows = $network_db->sql_numrows($membersresult);
if($memrows > 0){

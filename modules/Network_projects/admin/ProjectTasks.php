...
$pagetitle = "::: "._NETWORK_TITLE." ".$pj_config['version_number']."::: "._NETWORK_PROJECTS.": "._NETWORK_TASKLIST;
...
//echo "<br />\n";
$taskresult = $network_db->sql_query("SELECT `task_id`, `task_name`, `priority_id`, `status_id` FROM `".$network_prefix."_tasks` WHERE `project_id`='$project_id' ORDER BY `task_name`");
...
if($task_total != 0){
  while(list($task_id, $task_name, $priority_id, $status_id) = $network_db->sql_fetchrow($taskresult)) {

...
CloseTable();
//echo "<br />";
$typeresult = $network_db->sql_query("SELECT `type_name` FROM `".$network_prefix."_requests_types` WHERE `type_id`='$type_id'");
list($type_name) = $network_db->sql_fetchrow($statusresult);
...
$typelist = $network_db->sql_query("SELECT `type_id`, `type_name` FROM `".$network_prefix."_requests_types` WHERE `type_id` != '$type_id' AND `type_id` > 0 ORDER BY `type_weight`");

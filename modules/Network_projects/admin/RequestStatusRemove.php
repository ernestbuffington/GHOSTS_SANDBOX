...
CloseTable();
//echo "<br />";
$statusresult = $network_db->sql_query("SELECT `status_name` FROM `".$network_prefix."_requests_status` WHERE `status_id`='$status_id'");
list($status_name) = $network_db->sql_fetchrow($statusresult);
...
$statuslist = $network_db->sql_query("SELECT `status_id`, `status_name` FROM `".$network_prefix."_requests_status` WHERE `status_id` != '$status_id' AND `status_id` > 0 ORDER BY `status_weight`");

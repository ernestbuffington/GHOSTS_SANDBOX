...
$pagetitle = "::: "._NETWORK_TITLE." ".$pj_config['version_number']."::: "._NETWORK_MEMBERS.": "._NETWORK_POSITIONLIST;
...
//echo "<br />\n";
$positionresult = $network_db->sql_query("SELECT * FROM `".$network_prefix."_members_positions` WHERE `position_weight` > 0 ORDER BY `position_weight`");
$position_total = $network_db->sql_numrows($positionresult);
OpenTable();
...
if($position_total != 0){
  while($position_row = $network_db->sql_fetchrow($positionresult)) {
    $pjimage = pjimage("position.png", $nuke_module_name);
    ...
    list($pid1) = $network_db->sql_fetchrow($network_db->sql_query("SELECT `position_id` FROM `".$network_prefix."_members_positions` WHERE `position_weight`='$weight1'"));
    list($pid2) = $network_db->sql_fetchrow($network_db->sql_query("SELECT `position_id` FROM `".$network_prefix."_members_positions` WHERE `position_weight`='$weight3'"));
    echo "<td align='center'><nobr>";
    if($pid1 AND $pid1 > 0) {
      echo "<a href='".$admin_file.".php?op=MemberPositionOrder&amp;weight=".$position_row['position_weight']."&amp;pid=".$position_row['position_id']."&amp;weightrep=$weight1&amp;pidrep=$pid1'><img src='modules/$nuke_module_name/images/weight_up.png' border='0' hspace='3' alt='"._NETWORK_UP."' title='"._NETWORK_UP."'></a>";
    } else {
      echo "<img src='modules/$nuke_module_name/images/weight_up_no.png' border='0' hspace='3' alt='' title=''>";
    }
    if($pid2) {
      echo "<a href='".$admin_file.".php?op=MemberPositionOrder&amp;weight=".$position_row['position_weight']."&amp;pid=".$position_row['position_id']."&amp;weightrep=$weight3&amp;pidrep=$pid2'><img src='modules/$nuke_module_name/images/weight_dn.png' border='0' hspace='3' alt='"._NETWORKDOWN."' title='"._NETWORK_DOWN."'></a>";
    } else {

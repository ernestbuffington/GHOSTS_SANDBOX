...
if($reportrows > 0){
    $reviewresult = $network_db->sql_query("SELECT `report_id`, `report_name`, `project_id`, `type_id`, `status_id` FROM `".$network_prefix."_reports` ORDER BY `$column` $direction LIMIT $start_list, $end_list");
    while(list($report_id, $report_name, $project_id, $type_id, $status_id) = $network_db->sql_fetchrow($reviewresult)){

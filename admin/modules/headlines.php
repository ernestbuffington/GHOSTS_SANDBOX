...
if (isset($_POST['save'])) {
    ...
    $nuke_db->sql_query('UPDATE '.$prefix."_headlines SET sitename='$xsitename', headlinesurl='$headlinesurl' where hid=".intval($_POST['save']));
    nuke_redirect($admin_file.'.php?op=headlines');
} else if (isset($_POST['addHeadline'])) {
    $xsitename = str_replace(' ', '', Fix_Quotes($_POST['xsitename']));
    $headlinesurl = Fix_Quotes($_POST['headlinesurl']);
    $nuke_db->sql_query('INSERT INTO '.$prefix."_headlines VALUES (NULL, '$xsitename', '$headlinesurl')");
    nuke_redirect($admin_file.'.php?op=headlines');
} elseif (isset($_GET['edit'])) {
    $hid = intval($_GET['edit']);
    include_once(NUKE_BASE_DIR.'header.php');
    // GraphicAdmin();
    list($xsitename, $headlinesurl) = $nuke_db->sql_ufetchrow("SELECT sitename, headlinesurl FROM ".$prefix."_headlines WHERE hid='$hid'",SQL_NUM);
    OpenTable();
    ...
    if (isset($_POST['cancel'])) { nuke_redirect($admin_file.'.php?op=headlines'); }
    if (isset($_POST['confirm'])) {
        $nuke_db->sql_query('DELETE FROM '.$prefix."_headlines WHERE hid='".intval($_GET['del'])."'");
        nuke_redirect($admin_file.'.php?op=headlines');
    }
    ...
    $result = $nuke_db->sql_uquery("SELECT hid, sitename, headlinesurl FROM ".$prefix."_headlines ORDER BY hid");
    $bgcolor = $bgcolor3;
    while (list($hid, $sitename, $headlinesurl) = $nuke_db->sql_fetchrow($result)) {

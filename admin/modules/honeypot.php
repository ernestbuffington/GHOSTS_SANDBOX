...
function honeypotstats()
	{	
		...
		$result = $nuke_db->sql_query("SELECT check1, check2, check3, check4, check5, check6, c8opt1, c8opt2, fs9opt1, fs9opt2, headcolor, rowcolor1, rowcolor2, pagebgcolor, pagebordercolor, fontcolor, fontcolor2 FROM ".$prefix."_honeypot_config");
		list($check1, $check2, $check3, $check4, $check5, $check6, $c8opt1, $c8opt2, $fs9opt1, $fs9opt2, $headcolor, $rowcolor1, $rowcolor2, $pagebgcolor, $pagebordercolor, $fontcolor, $fontcolor2) = $nuke_db->sql_fetchrow($result);
	...
	addCSSToHead(''.$hpcss2head.'','inline');
					...
					$result1 = $nuke_db->sql_query("SELECT COUNT(potnum) FROM `". $prefix ."_honeypot` WHERE potnum = '0'");
list($waitscript) = $nuke_db->sql_fetchrow($result1);
$nuke_db->sql_freeresult($result1);
					$result2 = $nuke_db->sql_query("SELECT COUNT(potnum) FROM `". $prefix ."_honeypot` WHERE potnum = '1'");
list($textremoval) = $nuke_db->sql_fetchrow($result2);
$nuke_db->sql_freeresult($result2);
					$result3 = $nuke_db->sql_query("SELECT COUNT(potnum) FROM `". $prefix ."_honeypot` WHERE potnum = '2'");
list($hidden) = $nuke_db->sql_fetchrow($result3);
$nuke_db->sql_freeresult($result3);
					$result4 = $nuke_db->sql_query("SELECT COUNT(potnum) FROM `". $prefix ."_honeypot` WHERE potnum = '3'");
list($customquestion) = $nuke_db->sql_fetchrow($result4);
$nuke_db->sql_freeresult($result4);
					$result5 = $nuke_db->sql_query("SELECT COUNT(potnum) FROM `". $prefix ."_honeypot` WHERE potnum = '4' OR potnum = '5'");
list($sfsspam) = $nuke_db->sql_fetchrow($result5);
$nuke_db->sql_freeresult($result5);
					$result6 = $nuke_db->sql_query("SELECT COUNT(potnum) FROM `". $prefix ."_honeypot` WHERE potnum = '6'");
list($bscheck) = $nuke_db->sql_fetchrow($result6);
$nuke_db->sql_freeresult($result6);
					$result7 = $nuke_db->sql_query("SELECT COUNT(potnum) FROM `". $prefix ."_honeypot` WHERE potnum = '7'");
list($fscheck) = $nuke_db->sql_fetchrow($result7);
...
$result_total = $nuke_db->sql_query("SELECT COUNT(potnum) FROM `". $prefix ."_honeypot`");
list($total_blocked) = $nuke_db->sql_fetchrow($result_total);
$nuke_db->sql_freeresult($result_total);
...
function honeypot()
	{
		global $prefix, $nuke_db, $admin_file;
		$result1 = $nuke_db->sql_query("SELECT botlisting, perpage, pagenumberpos, headcolor, rowcolor1, rowcolor2, pagebgcolor, pagebordercolor, fontcolor, fontcolor2, usefeedback, email FROM ".$prefix."_honeypot_config");
		list($botlisting, $perpage, $pagenumberpos, $headcolor, $rowcolor1, $rowcolor2, $pagebgcolor, $pagebordercolor, $fontcolor, $fontcolor2, $usefeedback, $email) = $nuke_db->sql_fetchrow($result1);
		if($pagenumberpos == 0)
		{
			$page_numberpos = "top";
		} 
		...
		{
			$nuke_db->sql_query('DELETE FROM `'.$prefix.'_honeypot`');
			$nuke_db->sql_query('ALTER TABLE `'.$prefix.'_honeypot` AUTO_INCREMENT = 1');
			$nuke_db->sql_query('OPTIMIZE TABLE `'.$prefix.'_honeypot`');
			Header("Location: admin.php?op=honeypot");
		} 
		else 
		{
			global $prefix, $nuke_db, $bgcolor2, $admin_file;
	...
	echo '<div class="pagination" style="text-align:right;">' , PHP_EOL
		...
		if(isset($_GET['page'])) {
				$currentPage = $nuke_db->sql_escapestring($_GET['page']);
		}else{
			$currentPage = 1;
		}
		...
		// total results
		if(isset($_GET['search'])){
			$nameip = $nuke_db->sql_escapestring($_GET['search']);
			$where = ' WHERE `email` LIKE \'%' . $nameip . '%\' OR `ip` LIKE \'%' . $nameip . '%\'';
			$num = $nuke_db->sql_numrows($nuke_db->sql_query('SELECT * FROM `' . $prefix . '_honeypot`' . $where . ''));
		}else{
			$num = $nuke_db->sql_numrows($nuke_db->sql_query('SELECT * FROM `' . $prefix . '_honeypot`'));
		}
...
// End jquer for Hidden div
		...
		if(isset($_GET['search'])){
			$nameip = $nuke_db->sql_escapestring($_GET['search']);
			$result = $nuke_db->sql_query("SELECT id, username, realname, email, ip, date, potnum, reason FROM ".$prefix."_honeypot WHERE email LIKE '%$nameip%' OR ip LIKE '%$nameip%' ORDER BY id $orderby $limitQ");
		}else{
			$result = $nuke_db->sql_query("SELECT id, username, realname, email, ip, date, potnum, reason FROM ".$prefix."_honeypot ORDER BY id $orderby $limitQ");
		}
		$total_count = $nuke_db->sql_numrows($result);
...
// Hidden Div start
	echo '<div class="addinfo',$row['id'],' hp-bot-box">' , PHP_EOL
	   ...
	   , '<hr>' , PHP_EOL
	   , '<p class="minipot-text"><span class="minipot thick">' , _HONEYPOT_USERNAME , '</span> &nbsp;&nbsp;-&nbsp;&nbsp; ' , htmlspecialchars($row['username'], ENT_QUOTES, _NUKE_CHARSET) , '</p>' , PHP_EOL
	   , '<p class="minipot-text"><span class="minipot thick">' , _HONEYPOT_REALNAME , '</span> &nbsp;&nbsp;-&nbsp;&nbsp; ' , htmlspecialchars($row['realname'], ENT_QUOTES, _NUKE_CHARSET) , '</p>' , PHP_EOL
	   , '<p class="minipot-text"><span class="minipot thick">'._HONEYPOT_IP.'</span> &nbsp;&nbsp;-&nbsp;&nbsp; '.$ip2c_image . $row['ip'].'</p>' , PHP_EOL
	   , '<p class="minipot-text"><span class="minipot thick">' , _HONEYPOT_EMAIL , '</span> &nbsp;&nbsp;-&nbsp;&nbsp; ' , htmlspecialchars($row['email'], ENT_QUOTES, _NUKE_CHARSET) , '</p>' , PHP_EOL
	   , '</div>' , PHP_EOL;
	...
	if (isset($_POST) && $_POST['delete']) {
		...
		for($i = 0; $i < $count; $i++){
			$id = (int) $checkbox[$i];
				...
				if ($id > 0) {
					$resultds = $nuke_db->sql_query("DELETE FROM `".$prefix."_honeypot` WHERE id='".$_POST['checkbox'][$i]."'");
					$nuke_db->sql_query('OPTIMIZE TABLE `'.$prefix.'_honeypot`');
				}
			if($resultds){
...
//-----------------------------
	...
	global $prefix, $nuke_db, $nuke_module_name;
		...
		$nuke_db->sql_query("UPDATE ".$prefix."_honeypot_config SET 
		usehp='$usehp',

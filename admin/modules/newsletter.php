...
global $prefix, $nuke_db, $admin_file, $currentlang;
if (is_mod_admin()) {
    ...
    function newsletter_selection($fieldname, $current) {
        ...
        if (!isset($groups)) {
            ...
            $groupsResult = $nuke_db->sql_query("SELECT group_id, group_name FROM ".$prefix."_bbgroups WHERE group_single_user=0");
            while (list($groupID, $groupName) = $nuke_db->sql_fetchrow($groupsResult, SQL_NUM)) {
    ...
    if (isset($_POST['discard'])) {
        nuke_redirect($admin_file.'.php?op=newsletter');
    } elseif (isset($_POST['send'])) {
        global $aid;
        $row = $nuke_db->sql_ufetchrow('SELECT `adminmail` FROM `'.$prefix.'_config');
        $admin_email = $row[0];
        $admin_name = $aid;
        $headers = '';
        # Common Headers
        ...
        $headers .= "Content-type: text/html; charset="._NUKE_CHARSET." ".$eol;
        ...
        if ($n_group == 0) {
            $query = "SELECT username, user_email FROM ".$nuke_user_prefix."_users WHERE user_level > 0 AND user_id > 1";
        } elseif ($n_group == 2) {
            $query = "SELECT aid, email FROM ".$prefix."_authors";
        } elseif ($n_group > 2) {
            $n_group -= 2;
            $query = "SELECT u.username, u.user_email FROM ".$nuke_user_prefix."_users u, ".$prefix."_bbuser_group g WHERE u.user_level>0 AND g.group_id=$n_group AND u.user_id = g.user_id AND user_pending=0";
        } else {
            $query = "SELECT username, user_email FROM ".$nuke_user_prefix."_users WHERE user_level > 0 AND user_id > 1 AND newsletter=1";
        }
        ...
        $recipients = array();
        $result = $nuke_db->sql_query($query, true);
        set_time_limit(0);
        while (list($u_name, $u_email) = $nuke_db->sql_fetchrow($result, SQL_NUM)) {
    ...
    if (isset($_POST['preview'])) {
        ...
        if (empty($mailcontent)) { DisplayError(sprintf($admlang['global']['not_set'], $admlang['global']['content'])); }
        if ($group == 0) {
            list($num_users) = $nuke_db->sql_fetchrow($nuke_db->sql_query('SELECT COUNT(*) FROM '.$nuke_user_prefix.'_users WHERE user_level > 0 AND user_id > 1'));
            $group_name = strtolower($admlang['global']['all_members']);
        } elseif ($group == 2) {
            list($num_users) = $nuke_db->sql_fetchrow($nuke_db->sql_query('SELECT COUNT(*) FROM '.$prefix.'_authors'));
            $group_name = strtolower($admlang['global']['administrators']);
        } elseif ($group > 2) {
            $group_id = $group-2;
            list($num_users) = $nuke_db->sql_fetchrow($nuke_db->sql_query('SELECT COUNT(*) FROM '.$prefix.'_bbuser_group WHERE group_id="'.$group_id.'" AND user_pending="0"'));
            list($group_name) = $nuke_db->sql_ufetchrow("SELECT group_name FROM ".$prefix."_bbgroups WHERE group_id=$group_id", SQL_NUM);
        } else {
            list($num_users) = $nuke_db->sql_fetchrow($nuke_db->sql_query('SELECT COUNT(*) FROM '.$nuke_user_prefix.'_users WHERE user_level > 0 AND newsletter="1"'));

...
function displayadmins() 
{
    ...
    if (is_admin()) {
        ...
        echo '  </tr>'."\n";
        $result = $nuke_db->sql_query("SELECT `aid`, `email`, `name`, `admlanguage`, `url` FROM `".$prefix."_authors`");
        $countAuthor = 1;
...
{
    global $admin, $prefix, $nuke_db, $multilingual, $admin_file, $admlang;
    if (is_admin()) 
    {
        include_once(NUKE_BASE_DIR.'header.php');
        OpenTable();
        ...
        $row = $nuke_db->sql_fetchrow($nuke_db->sql_query("SELECT aid, name, url, email, pwd, radminsuper, admlanguage from " . $prefix . "_authors where aid='$chng_aid'"));
        $chng_aid = $row['aid'];
        $chng_name = $row['name'];
        ...
        if ($row['name'] != 'God'):
        	...
        	echo '      <table style="width: 100%;" border="0" cellpadding="3" cellspacing="1" class="forumline">'."\n";
        	echo '        <tr>';
            $result = $nuke_db->sql_query("SELECT mid, title, admins FROM ".$prefix."_modules ORDER BY title ASC");
            while ($row = $nuke_db->sql_fetchrow($result)):
 ...
 ******************************************************/
            $chng_aid = substr($chng_aid, 0,25);
            if ($chng_radminsuper == 1) {
                $result = $nuke_db->sql_query("SELECT mid, admins FROM ".$prefix."_modules");
                while ($row = $nuke_db->sql_fetchrow($result)) {
                    $admins = explode(",", $row['admins']);
                    $adm = '';
                ...
                $nuke_db->sql_query("update " . $prefix . "_authors set aid='$chng_aid', email='$chng_email', url='$chng_url', radminsuper='$chng_radminsuper', pwd='$chng_pwd', admlanguage='$chng_admlanguage' where name='$chng_name' AND aid='$adm_aid'");
                if ($adm_aid == $chng_aid) {
                    nuke_redirect($admin_file.".php?op=logout");
                } else {
                    // nuke_redirect($admin_file.".php?op=mod_authors");
                    nuke_redirect($admin_file.'.php?op=modifyadmin&chng_aid='.$chng_aid);
                }
            } else {
                if ($chng_name != 'God') {
                      $nuke_db->sql_query("update " . $prefix . "_authors set aid='$chng_aid', email='$chng_email', url='$chng_url', radminsuper='0', pwd='$chng_pwd', admlanguage='$chng_admlanguage' where name='$chng_name' AND aid='$adm_aid'");
                }
                $result = $nuke_db->sql_query("SELECT mid, admins FROM ".$prefix."_modules");
                while ($row = $nuke_db->sql_fetchrow($result)) {
                    $admins = explode(",", $row['admins']);
            ...
            if ($chng_radminsuper == 1) {
                ...
                $nuke_db->sql_query("update " . $prefix . "_authors set aid='$chng_aid', email='$chng_email', url='$chng_url', radminsuper='$chng_radminsuper', admlanguage='$chng_admlanguage' where name='$chng_name' AND aid='$adm_aid'");
                // nuke_redirect($admin_file.".php?op=mod_authors");
                nuke_redirect($admin_file.'.php?op=modifyadmin&chng_aid='.$chng_aid);
            } else {
                if ($chng_name != 'God') {
                        $nuke_db->sql_query("update " . $prefix . "_authors set aid='$chng_aid', email='$chng_email', url='$chng_url', radminsuper='0', admlanguage='$chng_admlanguage' where name='$chng_name' AND aid='$adm_aid'");
                }
                $result = $nuke_db->sql_query("SELECT mid, admins FROM ".$prefix."_modules");
                while ($row = $nuke_db->sql_fetchrow($result)) {
                    $admins = explode(",", $row['admins']);
        ...
        if ($adm_aid != $chng_aid) {
            $result2 = $nuke_db->sql_query("SELECT sid, aid, informant from " . $prefix . "_stories where aid='$adm_aid'");
            while ($row2 = $nuke_db->sql_fetchrow($result2)) {
                ...
                if ($old_aid == $informant) {
                    $nuke_db->sql_query("update " . $prefix . "_stories set informant='$chng_aid' where sid='$sid'");
                }
                $nuke_db->sql_query("update " . $prefix . "_stories set aid='$chng_aid' WHERE sid='$sid'");
            }
        }
    } else {
        DisplayError("Unauthorized editing of authors detected<br /><br />"._GOBACK);
...
function deladmin2($del_aid) {
    ...
    if (is_admin()) {
        ...
        $result = $nuke_db->sql_query("SELECT admins FROM ".$prefix."_modules WHERE title='News'");
        ...
        $nuke_db->sql_freeresult($result);
        if ($radminarticle == 1) {
            $row2 = $nuke_db->sql_fetchrow($nuke_db->sql_query("SELECT sid from " . $prefix . "_stories where aid='$del_aid'"));
            $sid = intval($row2['sid']);
            if (!empty($sid)) {
                include_once(NUKE_BASE_DIR.'header.php');
                OpenTable();
                ...
                $result3 = $nuke_db->sql_query("SELECT aid from " . $prefix . "_authors where aid!='$del_aid'");
        ...
        nuke_redirect($admin_file.".php?op=deladminconf&del_aid=$del_aid");
    } else {
        DisplayError("Unauthorized editing of authors detected<br /><br />"._GOBACK);
    }
}
 ...
 ******************************************************/
    ...
    case "assignstories":
        $del_aid = trim($del_aid);
        $result = $nuke_db->sql_query("SELECT sid from " . $prefix . "_stories where aid='$del_aid'");
        while ($row = $nuke_db->sql_fetchrow($result)) {
            $sid = intval($row['sid']);
            $nuke_db->sql_query("update " . $prefix . "_stories set aid='$newaid', informant='$newaid' where aid='$del_aid'");
            $nuke_db->sql_query("update " . $prefix . "_authors set counter=counter+1 where aid='$newaid'");
        }
        $nuke_db->sql_freeresult($result);
        nuke_redirect($admin_file.".php?op=deladminconf&del_aid=$del_aid");
    break;
    ...
    case "deladminconf":
        $del_aid = trim($del_aid);
        $nuke_db->sql_query("delete from " . $prefix . "_authors where aid='$del_aid' AND name!='God'");
        $result = $nuke_db->sql_query("SELECT mid, admins FROM ".$prefix."_modules");
        while ($row = $nuke_db->sql_fetchrow($result)) {
            $admins = explode(",", $row['admins']);
               $adm = "";

...
OpenTable();
title(_CB_CONFIG1);
$cbinfo = $nuke_db->sql_fetchrow($nuke_db->sql_query("SELECT * FROM `".$prefix."_nsncb_config` WHERE `cgid`='1'"));
...
sort($cblockslist);
$result2 = $nuke_db->sql_query("SELECT * FROM `".$prefix."_nsncb_blocks` WHERE `cgid`='1' ORDER BY `cbid`");

...
OpenTable();
title(_CB_CONFIG2);
$cbinfo = $nuke_db->sql_fetchrow($nuke_db->sql_query("SELECT * FROM `".$prefix."_nsncb_config` WHERE `cgid`='2'"));
...
sort($cblockslist);
$result2 = $nuke_db->sql_query("SELECT * FROM `".$prefix."_nsncb_blocks` WHERE `cgid`='2' ORDER BY `cbid`");
while($cbidinfo = $nuke_db->sql_fetchrow($result2)) {

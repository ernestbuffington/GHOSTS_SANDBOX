...
$totalselected = $nuke_db->sql_numrows($nuke_db->sql_query("SELECT * FROM `".$prefix."_nsnst_harvesters`"));
if($totalselected > 0) {
  ...
  echo '</tr>'."\n";
  $result = $nuke_db->sql_query("SELECT * FROM `".$prefix."_nsnst_harvesters` ORDER BY `harvester` $direction LIMIT $min,$perpage");

...
$perpage = 100;
$totalselected = $nuke_db->sql_numrows($nuke_db->sql_query("SELECT `ip_lo` FROM `".$prefix."_nsnst_excluded_ranges`"));
if(!isset($min)) {
  $min=0;
  ...
  OpenTable();
  $testmessage = "";
  $result = $nuke_db->sql_query("SELECT `ip_lo`, `ip_hi` FROM `".$prefix."_nsnst_excluded_ranges` ORDER BY ip_lo LIMIT $min, $perpage");
  while(list($xip_lo, $xip_hi) = $nuke_db->sql_fetchrow($result)) {
    $test1 = $nuke_db->sql_query("SELECT * FROM ".$prefix."_nsnst_excluded_ranges WHERE (ip_lo<='$xip_lo' AND ip_hi>='$xip_lo') AND (`ip_lo`!='$xip_lo' AND `ip_hi`!='$xip_hi') ORDER BY `ip_lo`");
    $test2 = $nuke_db->sql_query("SELECT * FROM ".$prefix."_nsnst_excluded_ranges WHERE (ip_lo<='$xip_hi' AND ip_hi>='$xip_hi') AND (`ip_lo`!='$xip_lo' AND `ip_hi`!='$xip_hi') ORDER BY `ip_lo`");
    $test3 = $nuke_db->sql_query("SELECT * FROM ".$prefix."_nsnst_excluded_ranges WHERE (ip_lo>='$xip_lo' AND ip_hi<='$xip_hi') AND (`ip_lo`!='$xip_lo' AND `ip_hi`!='$xip_hi') ORDER BY `ip_lo`");
    $test4 = $nuke_db->sql_query("SELECT * FROM ".$prefix."_nsnst_excluded_ranges WHERE (ip_lo<='$xip_lo' AND ip_hi>='$xip_hi') AND (`ip_lo`!='$xip_lo' AND `ip_hi`!='$xip_hi') ORDER BY `ip_lo`");

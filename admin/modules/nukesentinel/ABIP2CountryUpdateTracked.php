...
$totalselected = $nuke_db->sql_numrows($nuke_db->sql_query("SELECT DISTINCT(`ip_addr`) FROM `".$prefix."_nsnst_tracked_ips`"));
if(!isset($min)) {
  $min=0;
  ...
  $nuke_db->sql_query("UPDATE `".$prefix."_nsnst_config` SET `config_value`='1' WHERE `config_name`='site_switch'");
  $ab_config['site_switch'] = 1;
  $result = $nuke_db->sql_query("SELECT DISTINCT(`ip_addr`) FROM `".$prefix."_nsnst_tracked_ips` LIMIT $min, $perpage");
  while(list($xip_addr) = $nuke_db->sql_fetchrow($result)) {
    $xip_long = sprintf("%u", ip2long($xip_addr));
    list($xc2c) = $nuke_db->sql_fetchrow($nuke_db->sql_query("SELECT `c2c` FROM `".$prefix."_nsnst_ip2country` WHERE `ip_lo`<='$xip_long' AND `ip_hi`>='$xip_long' LIMIT 0,1"));
    if(!$xc2c) { $xc2c = "00"; }
    $nuke_db->sql_query("UPDATE `".$prefix."_nsnst_tracked_ips` SET `c2c`='$xc2c' WHERE `ip_addr`='$xip_addr'");
  }
  $max=$min+$perpage;
  $pagesint = ($totalselected / $perpage);
  $pages = ceil($pagesint);
  ...
  $nuke_db->sql_query("UPDATE `".$prefix."_nsnst_config` SET `config_value`='0' WHERE `config_name`='site_switch'");

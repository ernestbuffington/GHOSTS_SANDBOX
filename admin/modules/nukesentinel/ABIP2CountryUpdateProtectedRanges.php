...
if(!isset($min)) {
  ...
  $result = $nuke_db->sql_query("SELECT `ip_lo` FROM `".$prefix."_nsnst_protected_ranges` LIMIT $min, $perpage");
  while(list($xip_lo) = $nuke_db->sql_fetchrow($result)) {
    list($xc2c) = $nuke_db->sql_fetchrow($nuke_db->sql_query("SELECT `c2c` FROM `".$prefix."_nsnst_ip2country` WHERE `ip_lo`<='$xip_lo' AND `ip_hi`>='$xip_lo' LIMIT 0,1"));
    if(!$xc2c) { $xc2c = "00"; }
    $nuke_db->sql_query("UPDATE `".$prefix."_nsnst_protected_ranges` SET `c2c`='$xc2c' WHERE `ip_lo`='$xip_lo'");
  }
  $max=$min+$perpage;
  $pagesint = ($totalselected / $perpage);
  $pages = ceil($pagesint);

...
list($xcountry) = $nuke_db->sql_fetchrow($nuke_db->sql_query("SELECT `country` FROM `".$prefix."_nsnst_countries` WHERE `c2c`='$xc2c' LIMIT 0,1"));
$test1 = $nuke_db->sql_query("SELECT * FROM ".$prefix."_nsnst_ip2country WHERE (ip_lo<='$longip_lo' AND ip_hi>='$longip_lo') AND (`ip_lo`!='$old_ip_lo' AND `ip_hi`!='$old_ip_hi') ORDER BY `ip_lo`");
$test2 = $nuke_db->sql_query("SELECT * FROM ".$prefix."_nsnst_ip2country WHERE (ip_lo<='$longip_hi' AND ip_hi>='$longip_hi') AND (`ip_lo`!='$old_ip_lo' AND `ip_hi`!='$old_ip_hi') ORDER BY `ip_lo`");
$test3 = $nuke_db->sql_query("SELECT * FROM ".$prefix."_nsnst_ip2country WHERE (ip_lo>='$longip_lo' AND ip_hi<='$longip_hi') AND (`ip_lo`!='$old_ip_lo' AND `ip_hi`!='$old_ip_hi') ORDER BY `ip_lo`");
$test4 = $nuke_db->sql_query("SELECT * FROM ".$prefix."_nsnst_ip2country WHERE (ip_lo<='$longip_lo' AND ip_hi>='$longip_hi') AND (`ip_lo`!='$old_ip_lo' AND `ip_hi`!='$old_ip_hi') ORDER BY `ip_lo`");
$testnum1 = $nuke_db->sql_numrows($test1);
$testnum2 = $nuke_db->sql_numrows($test2);
$testnum3 = $nuke_db->sql_numrows($test3);
$testnum4 = $nuke_db->sql_numrows($test4);
if($testnum1 > 0 OR $testnum2 >0 OR $testnum3 >0 OR $testnum4 >0) {
  ...
  if($testnum1 > 0) {
    ...
    $testmessage .= '</tr>'."\n";
    while($testrow1 = $nuke_db->sql_fetchrow($test1)) {
  ...
  if($testnum2 > 0) {
    ...
    $testmessage .= '<td align="center" width="20%"><strong>'._AB_FUNCTIONS.'</strong></td>'."\n";
    $testmessage .= '</tr>'."\n";
    while($testrow2 = $nuke_db->sql_fetchrow($test2)) {
  ...
  if($testnum3 > 0) {
    ...
    $testmessage .= '<td align="center" width="20%"><strong>'._AB_FUNCTIONS.'</strong></td>'."\n";
    $testmessage .= '</tr>'."\n";
    while($testrow3 = $nuke_db->sql_fetchrow($test3)) {
  ...
  if($testnum4 > 0) {
    ...
    $testmessage .= '<td align="center" width="20%"><strong>'._AB_FUNCTIONS.'</strong></td>'."\n";
    $testmessage .= '</tr>'."\n";
    while($testrow4 = $nuke_db->sql_fetchrow($test4)) {
  ...
  $xtime = time();
  $nuke_db->sql_query("UPDATE `".$prefix."_nsnst_ip2country` SET `ip_lo`='$longip_lo', `ip_hi`='$longip_hi', `c2c`='$xc2c', `date`='$xtime' WHERE `ip_lo`='$old_ip_lo' AND `ip_hi`='$old_ip_hi'");
  $nuke_db->sql_query("UPDATE `".$prefix."_nsnst_tracked_ips` SET `c2c`='$xc2c' WHERE `ip_long` >= '$longip_lo' AND `ip_long` <= '$longip_hi'");
  $nuke_db->sql_query("UPDATE `".$prefix."_nsnst_blocked_ips` SET `c2c`='$xc2c' WHERE `ip_long` >= '$longip_lo' AND `ip_long` <= '$longip_hi'");
  $nuke_db->sql_query("UPDATE `".$prefix."_nsnst_blocked_ranges` SET `c2c`='$xc2c' WHERE `ip_lo` >= '$longip_lo' AND `ip_lo` <= '$longip_hi'");

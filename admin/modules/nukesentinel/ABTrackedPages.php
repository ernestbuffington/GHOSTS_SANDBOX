...
echo '</tr>'."\n";
$result = $nuke_db->sql_query("SELECT `tid`, `page`, `date` FROM `".$prefix."_nsnst_tracked_ips` WHERE `ip_addr`='$ip_addr' AND `user_id`='$nuke_user_id' ORDER BY $column $direction LIMIT $min, $perpage");
while(list($ltid, $page, $date_time) = $nuke_db->sql_fetchrow($result)){
  ...
  echo '<td align="center" nowrap="nowrap"><a href="'.$admin_file.'.php?op=ABTrackedDeleteSave&amp;tid='.$ltid.'&amp;user_id='.$nuke_user_id.'&amp;ip_addr='.$ip_addr.'&amp;column='.$column.'&amp;direction='.$direction.'&amp;min='.$min.'"><img src="images/delete.png" height="16" width="16" border="0" alt="'._AB_DELETE.'" title="'._AB_DELETE.'" /></a></td>'."\n";
  echo '</tr>'."\n";
}
echo '</table>'."\n";

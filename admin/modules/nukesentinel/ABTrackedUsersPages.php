...
$totalselected = $nuke_db->sql_numrows($nuke_db->sql_query("SELECT * FROM `".$prefix."_nsnst_tracked_ips` WHERE `user_id`='$tid'"));
if($totalselected > 0) {
  ...
  echo '</tr>'."\n";
  $result = $nuke_db->sql_query("SELECT `tid`, `user_id`, `page`, `date` FROM `".$prefix."_nsnst_tracked_ips` WHERE `user_id`='$tid' ORDER BY $column $direction LIMIT $min, $perpage");
  while(list($ltid, $luserid, $page, $date_time) = $nuke_db->sql_fetchrow($result)){

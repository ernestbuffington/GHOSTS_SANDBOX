...
$totalselected = $nuke_db->sql_numrows($nuke_db->sql_query("SELECT `ip_addr` FROM `".$prefix."_nsnst_blocked_ips`"));
if(!isset($min)) {
  $min=0;
  ...
  $result = $nuke_db->sql_query("SELECT `ip_addr` FROM `".$prefix."_nsnst_blocked_ips` LIMIT $min, $perpage");
  while(list($xip_addr) = $nuke_db->sql_fetchrow($result)) {
    ...
    if(!$xc2c) { $xc2c = "00"; }
    $nuke_db->sql_query("UPDATE `".$prefix."_nsnst_blocked_ips` SET `c2c`='$xc2c' WHERE `ip_addr`='$xip_addr'");
  }
  $max=$min+$perpage;
  $pagesint = ($totalselected / $perpage);
  $pages = ceil($pagesint);

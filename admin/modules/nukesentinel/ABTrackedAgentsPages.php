...
$totalselected = $nuke_db->sql_numrows($nuke_db->sql_query("SELECT * FROM `".$prefix."_nsnst_tracked_ips` WHERE `user_agent`='$uname'"));
if($totalselected > 0) {
  ...
  echo '</tr>'."\n";
  $result = $nuke_db->sql_query("SELECT `ip_addr`, `page`, `date` FROM `".$prefix."_nsnst_tracked_ips` WHERE `user_agent`='$uname' ORDER BY $column $direction LIMIT $min, $perpage");

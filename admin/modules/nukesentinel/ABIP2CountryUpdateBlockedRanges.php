...
if(!isset($min)) {
  ...
  $result = $nuke_db->sql_query("SELECT `ip_lo` FROM `".$prefix."_nsnst_blocked_ranges` LIMIT $min, $perpage");
  while(list($xip_lo) = $nuke_db->sql_fetchrow($result)) {
    ...
    if(!$xc2c) { $xc2c = "00"; }
    $nuke_db->sql_query("UPDATE `".$prefix."_nsnst_blocked_ranges` SET `c2c`='$xc2c' WHERE `ip_lo`='$xip_lo'");
  }
  $max=$min+$perpage;
  $pagesint = ($totalselected / $perpage);
  $pages = ceil($pagesint);

...
$totalselected = $nuke_db->sql_numrows($nuke_db->sql_query("SELECT * FROM `".$prefix."_nsnst_ip2country` $modfilter"));
if($totalselected > 0) {
  ...
  $countries = $nuke_db->sql_query("SELECT `c2c`, `country` FROM `".$prefix."_nsnst_countries` ORDER BY `c2c`");;
  while(list($xc2c, $xcountry) = $nuke_db->sql_fetchrow($countries)) {
  ...
  echo '</tr>'."\n";
  $result = $nuke_db->sql_query("SELECT * FROM `".$prefix."_nsnst_ip2country` $modfilter ORDER BY $column $direction LIMIT $min,$perpage");
